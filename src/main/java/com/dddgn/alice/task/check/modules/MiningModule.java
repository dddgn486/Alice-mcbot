...
public final class MiningModule implements CheckModule {
    ...
    public List<CheckStep> steps(CheckContext ctx) {
        ...
        return List.of(
                ...
                // ⭐⭐ `D-389`（2026-09-22）：**沿脉传播**的行为级判据 —— 石壳 + 3×2×5 铁矿脉（26 邻接），
                // bot 只能挖进去；进脉之后每个下一个目标都在 1 格内 ⇒ 便宜。
                // 断言 ① 传播真的触发（`veinPropagations > 0`）② 矿簇被挖穿（`mined ≥ MIN_MINED`，
                // 该下限**按实测**抬，见夹具注释）③ 沿脉走之后不再有昂贵搜索被限流。
                // 红臂：注释掉 `MineJob.enqueueVeinNeighbours(...)` ⇒ ① 必红。EXTRA（跑一个真实作业）。
                // ⚠️ **单项预算必须 > 夹具自己的 `BUDGET_TICKS`（4600）**：2026-09-24 实测抓到的坑 ——
                // 预算写成 2800 时，夹具**内部的** 4600 永远轮不到，`PL-1` 的重评能力一跑就被 harness
