...
public final class LineOfSightChecker {
    ...
    /** 目标面内缩距离（可调，见 {@link MiningTuning#losSampleEpsilon()}）。 */
    private static double sampleEpsilon() {
        return MiningTuning.losSampleEpsilon();
    }
    ...
    public static LineOfSightResult checkFromEye(Level level, Vec3 eyePosition, BlockPos target) {
        ...
        for (Vec3 sample : samples) {
            ...
            if (hit.getType() == HitResult.Type.BLOCK && hit.getBlockPos().equals(target)) {
                return new LineOfSightResult(true, null, sample);
            }
            if (firstBlocker == null && hit.getType() == HitResult.Type.BLOCK) {
                firstBlocker = hit.getBlockPos().immutable();
            }
        }
        return new LineOfSightResult(false, firstBlocker, null);
    }
    ...
    public static class LineOfSightResult {
        ...
        public LineOfSightResult(boolean clear, BlockPos firstBlocker, Vec3 successfulSample) {
            this.clear = clear;
        ...
        public String toString() {
            ...
            return "LineOfSight[BLOCKED, firstBlocker="
                    + (firstBlocker != null ? firstBlocker.toShortString() : "null") + "]";
        }
    }
}
