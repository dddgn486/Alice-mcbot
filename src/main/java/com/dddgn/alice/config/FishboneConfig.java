 ...
 *   <tr><td>{@code oreBudgetPerUnit}</td><td>32</td><td>见下（计划写 8 → 本片 16 → 真机实测 1/117 触顶后抬到 <b>32</b>，理由写在字段注释里）</td></tr>
 * </table>
 *
...
public final class FishboneConfig {
     ...
     *
     * <p>⚠️ **2026-09-25 真机实测 ⇒ 16 → 32**（用户原话「矿簇的预算还能放开点吗」）：
     * 本轮 117 个单元里**只有 1 个**触顶（`ore_budget_exhausted 单元 117 已消费 16 个暴露矿（上限 16）
     * ⇒ 丢弃剩余 8 个候选`）—— 也就是说那个单元**实际有 24 个候选**，16 把它砍掉了三分之一，
     * 而 8 个被丢的矿**正是用户看到的"矿簇没挖完"**（本轮 53 找到 / 43 挖到，10 个没挖里有 8 个是它）。
     * 32 > 24 ⇒ 实测这一处够用；触发率 1/117 ≈ 1% ⇒ 抬高的成本可忽略。
     * 复核触发：真机出现"某个单元追矿超过 32 格"或"追矿把一轮作业拖长到明显不合理" ⇒ 回退。
     * ⚠️ **真正的修法不在这个数**：`D-443` 裁定 6 已经把「丢弃 ⇒ 有界延后」判给片 B
     * （触顶只许**延后**、不许**丢**）；本条只是把护栏挪到实测够用的位置。
     */
    public static final int DEFAULT_ORE_BUDGET_PER_UNIT = 32;
