...
import com.dddgn.alice.task.BotPhysicsAssertionFixture;
import com.dddgn.alice.task.SoftPhysicsObservationTest;
import com.dddgn.alice.transfer.TransferFixture;
import com.dddgn.alice.transfer.TransferSelectionFixture;
import com.dddgn.alice.command.TransferSelectionCommandParseFixture;
import com.dddgn.alice.gui.BotInventoryFixture;
import com.dddgn.alice.item.TransferEndpointSelectorEventsFixture;
...
public final class BotSelftest {
    ...
    private static void setup() {
        ...
        boolean selectionPass = TransferSelectionFixture.run(level, surface.offset(28, 0, 0));
        boolean selectorEventPass = TransferEndpointSelectorEventsFixture.run();
        boolean commandParsePass = TransferSelectionCommandParseFixture.run();
        boolean botInventoryPass = BotInventoryFixture.run(level, bot);
        boolean softPhysicsPass = SoftPhysicsObservationTest.run(level);
        ...
        BotLog.info("TRANSFER_SELECTION_FIXTURE_SUITE {} request/state/code/location/source-bot-destination-delta asserted", selectionPass ? "PASS" : "FAIL");
        BotLog.info("TRANSFER_SELECTOR_EVENT_FIXTURE_SUITE {} role/event-write-count/hand-rejection/non-cancel asserted", selectorEventPass ? "PASS" : "FAIL");
        BotLog.info("TRANSFER_SELECTION_COMMAND_PARSE_FIXTURE_SUITE {} valid/malformed ResourceLocation tokens asserted", commandParsePass ? "PASS" : "FAIL");
        BotLog.info("BOT_INVENTORY_FIXTURE_SUITE {} slot-count/busy-flag asserted", botInventoryPass ? "PASS" : "FAIL");
        BotLog.info("SOFT_PHYSICS_OBSERVATION_SUITE {} observation/revalidate/replan asserted", softPhysicsPass ? "PASS" : "FAIL");
        BotLog.info("BOT_PHYSICS_ASSERTION_SUITE {} f1/f3/f4/f5/f2/tickChain asserted", botPhysicsAssertionPass ? "PASS" : "FAIL");
