...
**反向对照的元纪律（D-255 实测补）**：注入违例后**先看判据有没有真的跑起来** —— 注入本身把代码改坏
（括号不平衡等）时 battery 会以 `exit=5`（环境/脚本）收场、**没有判决行**；那是「注入坏了」，**不是判据红了**，
不能记成证据。退出码字典：`0=PASS 1=FAIL 2=DEGRADED 3=无判决 4=起不来 5=环境/脚本`。
