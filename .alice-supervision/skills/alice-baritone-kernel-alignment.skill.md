...
**✅ 切片 B 已落地并真人验收（D-251/D-253，2026-09-16）**：水位支撑例外（只认水面格）+ 浮着完成口径（`TRAVERSE/DESCEND/ASCEND`）+ 段间浮着（settle 不再 `stopMovement()`）+ 水里失败当场判定；`deep_pond_course` 已是执行判据并在客户端 `COMPLETED`（水面 12/11 tick vs 陆地 5 tick）。**（D-248 的回退记录保留在下方，仅供追溯）** \n**切片 B（旧记录）：勘查完成、规划那半已回退（D-248，2026-09-16）**：`canWalkOn` 加 Baritone 那条例外（**下面也是水 ⇒ 算支撑**）在**单跑**下全绿，但 **CORE 里红**（逃生改走更便宜的"破墙 + 升到水面格"路线，最终段支撑被读成 `Air` ⇒ `SEGMENT_FUTURE_BLOCKED`）⇒ 回退，证据在 D-248。**根因随后查清（D-249）**：那格 `Air` 是**同一条计划自己的 `BREAK_AND_ENTER` 清掉的头位格**（清脚位+头位两格，Baritone `MovementTraverse.java:56` 同款），**不是未加载区块** ⇒ 真问题是「**搜索不模拟自己的写入**」（Baritone `CalculationContext` 也没有这种覆盖，它靠执行期 `COST_INF` 重算兜底；Alice 的 `futureTargetBlocked` 用真实世界提前抓到，而重规划从被水推回的起点又算出同一形状）⇒ 教训两条：① **改合法位置集时，生成器 / 会话健康检查 / 计划自洽性三处谓词要一起审**（否则"可规划不可执行"）；② 看到"计划期读到的方块几 tick 后变了"先 grep **自己**的 `[WRITE] break` / `[BreakEnter] cleared`。留在树上的只有：`canWalkOn` 那条例外**有意收窄到"只认水面格"**（假人浮得上去、潜不下去 ⇒ 合法位置集只放执行器真到得了的格子）；`Traverse`/`Descend`/`Ascend` 三处加了"浮着段"的完成口径（脚位到格即成功）——⚠️ **蹚水（下面实心）不要套**，混在一起会让蹚水多出不必要 STALE 重规划。**实测缺口**：从岸上走进深水那一步，动量把 bot 带到池底 ⇒ 今天**游不过去**（常绿登记在 `deep_pond_course+floor`）。**2026-09-16 更新（D-250 实验）**：有了计划自洽校验（见下条 Alice 有意偏离）后，切片 B 的**规划**已能给出自洽计划，**执行**仍卡在「水里跨段保持浮着」（`ASCEND_STALE_START`）⇒ 切片 B 落地前必须先定这一条。
**⚠️ Alice 有意偏离（D-250，2026-09-16，已实施）**：**Baritone 不模拟自己的写入**（`CalculationContext:53-77` 只有真实世界 `bsi` + `allowBreak`/`allowBreakAnyway`；全仓 `blockedBy`/`plannedBreaks`/`simulate` **零命中**），它靠"执行期代价变 `COST_INF` ⇒ 重算路径"兜底。Alice **多一层"计划自我写入自洽性"**：搜完回放计划自己的破坏/放置（口径**直接调执行器**的 `collectBlockers`/`placePos`），发现"后面的边踩在前面挖掉的格子上"就**禁掉那条具体边**重搜（≤3 次）。**为什么必须偏离**：Baritone 那种"执行期重算"在 Alice 会**成环**（逃生起点可能被水推回原位 ⇒ 重算出同一形状；D-248→D-249 实测链路）；**为什么不做在搜索里**：A\* 位置键合并会吃掉按路径过滤（更便宜的挖掘前缀占住节点、干净前缀永不重挂 ⇒ 可解变不可解，D-250 实测 `own_write_support=245`）。判据：电池硬自断言 `selfwrite_unresolved == 0`。
**⚠️ 两个"水里没有落地"的真坑（D-251 实测，写在这里省一轮）**：① **任何"等落地再判定"的守卫在水里都失效** ——
`PathSession.handleFailure` 的空中守卫（`!onGround` 就 return）会让**水里的段失败被静默吞掉**：既不报错也不前进，
空转到 bot 沉底踩到地面才报 STALE（实测深水池空转 ~57 tick）。判据要写成 `!onGround && !inWater` 才等。
② **段间 settle 一律 `stopMovement()` 在水里等于松手** ⇒ bot 下沉 + 被"挖开侧壁后流过来的水"推走 ⇒ 下一段
`STALE_START`（实测灌水坑：10 tick settle 内从浮着的水面格被推到 `341,100,306` 再沉到 `341,99,306`）。
水里要"零水平输入 + 按需按住跳跃"，口径收在 `MovementHelper.shouldHoldJumpInWater`（对照
