...
**切片 B：勘查完成、规划那半已回退（D-248，2026-09-16）**：`canWalkOn` 加 Baritone 那条例外（**下面也是水 ⇒ 算支撑**）在**单跑**下全绿，但 **CORE 里引出未解释的回归**（逃生改走更便宜的"破墙 + 升到水面格"路线，最终段支撑被读成 `Air` ⇒ `SEGMENT_FUTURE_BLOCKED`）⇒ 回退，证据在 D-248；教训：**改动合法位置集时，生成器与会话健康检查的谓词必须一起审**（否则"可规划不可执行"）。留在树上的只有：但**有意收窄到"只认水面格"**（假人浮得上去、潜不下去 ⇒ 合法位置集只放执行器真到得了的格子）；`Traverse`/`Descend`/`Ascend` 三处加了"浮着段"的完成口径（脚位到格即成功）——⚠️ **蹚水（下面实心）不要套**，混在一起会让蹚水多出不必要 STALE 重规划。**实测缺口**：从岸上走进深水那一步，动量把 bot 带到池底 ⇒ 今天**游不过去**（常绿登记在 `deep_pond_course+floor`）。
