...
- **上下文：禁止估算，只读真实数据（用户 2026-09-14 立规）**：
  跑 `bash tools/dsh-context-usage.sh`（读 DSH 会话 JSONL 的 `request/context.contextWindow` 与
  `assistant/message.usage`）。**当前真实值**：窗口 **256,000**；自动压缩阈值 **204,800**（=0.8×窗口，
  来自 `@deepseek-ai/dsh-compaction-basic` 默认，`settings.yaml` 无覆盖）；压缩后保留 **≈40,960**（0.16×窗口）。
  **不得再说"我估计上下文用了 X%"**；旧表述"窗口 1,000,000 / 800,000 压缩"已作废。
