...
- **R1（已证，代码+日志 ⇒ `D-371` 已修）**：`MineJob.select()` 里 `session.advance()` **只在 `selection.picked() == null` 时调用**
...
## 8. 修复进度（2026-09-20 目标轮 1–8 结束时）

| 项 | 状态 | 判据/证据 |
|---|---|---|
| ① 掉刻归因 | ✅ **完成** | 选择 102→126 ms（`D-368` 摊销后 **26 ms** 热缓存）· 搜索墙钟上限 3000→**200 ms**（`D-369`）+ 超 tick 日志 · 菜单 31 ms/次（**不是重复**，待办作废）· LLM 异步（排除）· 分片 13–20 ms（非主因） |
| ② R2 选择排序 | ✅ **完成** | `D-368` 摊销：每次 ≤1 次精算 + 缓存覆盖**全部**候选；夹具 4 条 + **4 注入全红** |
| ③ R3 站位点/路径 | ✅ **诊断完成**（真机确认待测试轮） | 病根 = **到达路径长度**（站位格只差 1 格却 11 段）；夹具 2 几何 + 3 注入全红 |
| ④ 掉落物落入不可进入的洞 | ⏳ **未做**（等用户裁定） | 已归因（纯通行收集器 + 自造 1 格高口袋）；"是否给有界世界修改权"= `D-076` 红线，必须用户决定 |
| ⑤ R1 扫描覆盖 | ✅ **完成** | `D-371` 每次选择推进一格；夹具 3 条 + 门禁 2 注入全红；真机复核 = `分片推进 visited` 应突破 8192 |
| `D-366b` 风险补偿核实 | ⏳ **未做** | 需查新放开的 PILLAR/FALL/DOWNWARD 是否仍经风险层 + 真机观察伤害 |

**本会话"判据太弱"共 4 次**（都已收紧并留档）：`D-366` 方法定义 vs 调用点 · `D-368` 索引 0 vs 排序第一 ·
