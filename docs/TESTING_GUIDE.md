...
| **疾跑+右键** `alice:survival_exit_check`（**细雪冻结**，D-229 新增） | `alice_test:survival_sealed_course`（自动生成封闭石壳）+ `fill … minecraft:powder_snow` | `维生监测 hazard=FREEZING`（`ticksFrozen` 累积到 60 才出现，约 3 秒）→ 持续 10 tick 宽限后 `[Survival] 软危险 hazard=FREEZING … 半径 8 格内**无安全落点** ⇒ 不否决`（+ 一条 `exit=none decision=continue` 事件）→ 全冻（140 tick ≈ 7 秒）后 `[Threshold] 掉血 … hazard=FREEZING`，**每 2 秒 1 点**；**肉眼：bot 在细雪坑里冻住、血量往下掉**（这一档**故意不逃生**：壳里没有出口，乱否决只会让它更惨）。想要"否决 + 逃生"请看上一条（着火）|
