...
## 2.5 ★ 保护区地图（区块勾选，D-314）—— 零参数右键，**不需要场景**
...
**它长什么样（先看这个，再看日志）**：
- 每格底色 = 该区块的**地形色**（客户端从自己的区块数据采样 `MapColor`）；**未加载的区块是灰色**（不假装知道）；
- 已认领格 = **红色描边**；待认领 = 琥珀色；待取消 = 蓝色；你站的区块 = **白色内框**；
- 底部两行：`光标：区块 X, Z · 已认领/未认领/待认领/待取消`、`本视图已认领 N 格`，以及键位提示与「待提交 N」。
...
**已知边界**：本轮**只做了本地小地图式勾选**，没有拖动 / 缩放大地图，也没有客户端磁盘缓存；不集成任何地图 mod。
