## 客户端轮次的**准入尺子**（2026-09-19 用户裁定）

> 起因：`D-342` 的两轮客户端复测**没有提供任何客户端才有的信息**（它是纯服务端逻辑），
> 用户逐字追问「这种重复测试应该加入无头测试，而不是反复测试，**这个测试有我客户端才能获取的信息吗**？」
> ⇒ 之后**默认无头**，客户端轮次必须先回答"这一问"。

**只有下面这些类别才值得占你的客户端轮次**（其余一律先做成无头判据/内核规则）：

| 需要客户端 | 理由 |
|---|---|
| 渲染 / 实体 / 物理 / 碰撞 / 同步（rubber-banding、穿模、悬空、抖动、紫黑贴图） | 服务端日志**看不到**观感与客户端预测 |
| GUI / 容器菜单 / 槽位 / 按键交互 | 只有真人有手 |
| 真实模组交互（Create/Mek/EC… 的方块与物品语义） | 无头模组集与客户端**不完全一致**，且需要真人看行为 |
| 真人观感与主观验收（"符合预期"） | 只有你能给 |

**不需要客户端**（必须做出无头判据，做不出就说明判据没设计好）：
纯服务端判定 / 权限与预算闸门 / 终态与记账 / 提示词与通知 / 夹具自身的断言 —— 这类改动
"**从改一行到知道对不对**"应当在 **1 条命令**内闭环。

**交付时的固定话术**（避免我再把复测当默认动作）：说明"这条线**不需要**你复测（已无头覆盖：`<命令>`）"，
或明确列出"需要你复测，因为 `<客户端才有的信息>`"。
# Alice 测试流程指南（一键优先）
...
| 右键 `alice:survival_full_check`（**维生全套自检**，D-229 新增；含细雪冻结） | 自动：`alice_test:survival_course`，封闭场景/水/雪由夹具自己建 | 起任务后依次出现决策表相关日志 → `hazard=ON_FIRE` + `掉血 … hazard=ON_FIRE` → 入水 `air=` 下降 → **细雪** `维生监测 hazard=FREEZING`（约 3 秒后）→ 无出口 ⇒ `[Survival] 软危险 hazard=FREEZING … 无安全落点 ⇒ 不否决` → 全冻（140 tick ≈ 7 秒）后 `[Threshold] 掉血 … hazard=FREEZING`（每 2 秒 1 点）→ 聊天里一行 `[Survival] SUMMARY checks=55 failures=0 → PASS`。**肉眼：bot 被封在细雪坑里冻住、血量往下掉**，结束后自己回到平台 |
...
## 2.5 ★ 保护区地图（区块勾选 + 地形细化，D-314 / D-315）—— 零参数右键，**不需要场景**

**物品**：`alice:protection_selector`（保护区地图，贴图同守卫）

```
/give @s alice:protection_selector      # 或直接右键手持的那个
```

**一次右键**（对着空气或方块都行）⇒ 打开「保护区地图」界面：**以你为中心的 17×17 区块网格**
（格数按窗口自适应，但**永远是奇数** ⇒ 正中间那格就是**你站的区块**）。

| 操作 | 效果 |
|---|---|
| **左键**点一格 | 该格 = **待认领**（琥珀色描边） |
| **右键**点一格 | 该格 = **待取消**（蓝色描边） |
| 按住**拖动** | 沿路连选（沿用按下时的那个键） |
| **R** | 视图回到你所在区块 |
| **G** | 高度明暗开关（觉得糊就关掉） |
| **关闭界面（ESC）** | **一次性提交**（最多两个包：认领批 + 取消批） |

**它长什么样（先看这个，再看日志）**：
- 每格**不是单色块**：切成 3×3（小窗口 2×2）个子格，各自采自己的列 ⇒ 有地形形状；
  同色再按**高度**加明暗（**墙体/山脊亮、坑洼暗**）⇒ 应能认出自己的基地轮廓；
- **浅灰**=区块未加载 · **深灰**=采样窗口（你脚位 ±40）里没找到地表 · **蓝色系**=水面；
- 已认领格 = **红色描边**（染色很淡，不盖地形）；待认领 = 琥珀；待取消 = 蓝；你站的区块 = **白色内框**；
- **边缘每 4 格**有区块坐标刻度；**悬停**一行显示 `区块 X, Z · 状态 · 地表 <方块> y=<高度>`；
- 打开瞬间：**中心区立刻有地形**，远处几百毫秒内补齐（标题行会显示「采样中 n%」）；
- 底部四行：光标行 / 本视图认领数 / **图例** / 键位与「待提交 N」。

**聊天与日志（AI 会读这个判读）**：
```
[alice] 保护区：认领 3 个区块，本次提交 3 个；当前共 12 个区块
[Protection] batch player=… action=claim offered=3 applied=3 changed=3 rejected=0 code=ok total=12 dim=minecraft:overworld
[Protection] sync player=… dim=minecraft:overworld sent=12 truncated=false total=12
```

**判定**：
- 关闭界面后**聊天必须出现回执**；`changed` = 真正新变化的格数（重复认领是 `0`，正常）；
- 重开界面：刚提交的格应为**红色描边**（= 服务端已确认）；
- **反证（最该试的一条）**：认领你站的那一格，然后用 `alice:target_selector` 右键那一格里的方块 ⇒ bot 应被拒
  （日志 `protected_area`）；
- ⚠️ 若界面一直显示「等待服务端认领数据…」⇒ **记下来**（说明回推与自愈请求两条路都没通）；
...
⭐ **2026-09-18 补齐（`D-321`，源码核过）**：①「结盟」和「让 bot 入你的队」**两条路都挂在 `/ftbteams party`
下面** ⇒ **你自己没有 party 时，上面三条里没有一条能用**（`public` 那个例外是改你自己的设置）。
现在有了 §2.7 的三条命令：`/alice ftb status`（只看）/ `bind`（建队+邀请+入队）/ `unbind`。
...
## 2.6 ★ 假人归属（创建者登记，D-319）—— 三条命令，**不需要场景**

**入口**（都要求权限 2，与 `/alice` 其余子命令一致）：

| 命令 | 用途 |
|---|---|
| `/alice spawn <名字>` | 生成假人；**执行者自动成为它的创建者**（回执里会写出来）|
| `/alice bots` | 列出所有假人：名字 · uuid · **创建者** · 维度 · 坐标 · 忙/闲 |
| `/alice adopt <名字>` | 给**未登记**的假人补创建者（老存档 / 命令方块生成的 / 夹具探针）|

**看什么 / 判定**：
1. `/alice spawn demo` ⇒ 回执应带 `（创建者=<你的名字> (<8位>)）`；
2. `/alice bots` ⇒ 那一行 `创建者=` 是**你的名字**，不是「未登记」；
3. ⭐ **反证（最该试的一条）**：再敲一次 `/alice adopt demo` ⇒ 必须**失败**，并说「创建者已经是 X ⇒ 不改写」（认领是**单向**的）；
4. **落盘**：退出存档再进来 ⇒ `/alice bots` 仍是**同一个创建者**（UUID 在存档里，名字只是登记当时的快照）；
5. ⚠️ **老假人**（这次改动**之前**生成的）⇒ 显示「未登记」，用 `/alice adopt` 认领一次即固化（**不猜、不静默补**）。

**多假人共存（`D-320`）**：以前**两只假人挨着**（同格/相邻格）会让服务端爆栈崩（旋转包 A⇄B 互相转发；`5d63cdf` 引入），
**本轮已修**。判定：`/alice spawn a` + `/alice spawn b`（**故意挨着放**）⇒ ①游戏不崩 ②`/alice bots` 正常回话
③能正常退出存档 ④日志里**不再出现** `StackOverflowError: Sending packet`。⚠️ 真人在客户端看 bot 转头/移动是否依旧正常 ⇒ 仍需你眼验。

**明确不做（本步边界）**：归属**不参与任何权限判定**（"谁都能指挥 bot"**没变**）；
FTB 侧"**继承创建者的队伍身份与权限**"见下面的 §2.7（`D-321`）。

---

## 2.7 ★ FTB 身份继承（D-321）—— 三条命令，**先只读、再显式写入**

**入口**（权限 2；`bind`/`unbind` 必须由**游戏内玩家**执行 —— 命令方块没有 FTB 身份）：

| 命令 | 用途 | 写不写你的 FTB 数据 |
|---|---|---|
| `/alice ftb status` | 只读：你的队伍 + 每只假人的队伍 + 与你的关系（**同队/不同队**）| 不写 |
| `/alice ftb bind` | 把**由你创建**的假人拉进你的 FTB 队伍：没有队就先以你的名义建队 → 邀请 → 入队 | 写（见下方警告）|
| `/alice ftb unbind` | 反向：假人先退、你再退（FTB 会删掉空队伍）| 写 |

**⚠️ 必须先知道的副作用**（`D-321` 已逐条核实源码）：你**还没有队伍**时，`bind` 会以你的名义建一个 party，
而 FTB Chunks 的 `playerJoinedParty` 会把你**已有的认领区块转入该队**（FTB 自己留了"原始认领"，退队时还回；
claim 上限默认按"最大成员"算 ⇒ 不变）。所以它**不在 spawn 时自动做** —— 只在你敲命令时做。

**看什么 / 判定**：
1. 先 `/alice ftb status` ⇒ 应看到两行以上：你 = `自己「<你的名字>#xxxx」…`（还没队）或 `party「…」`；每只假人一行 + `与你的关系=不同队`；
2. `/alice ftb bind` ⇒ 逐条回话：`建队 ✓` → `邀请假人 … ✓（已获得 INVITED 级）` → `假人 … 入队 ✓（…身份=MEMBER）`；
3. ⭐ **幂等（最该试的一条）**：`bind` 再敲一次 ⇒ 必须是 `你已经有队伍 ✓（不新建）` + `已在同一队伍 ✓（重复执行不改任何东西）`，
   **不能**再建一个队；
