...
- **离线门槛**（"改一行到知道对不对"）：`tools/headless-battery.sh core` = **CORE 51 步**（总 **66 项**；`far_path_bench`/`path_retry_bench`/`mine_inventory`/`place_step_diagonal` 属 EXTRA，其中前两个是**测量基准**且改世界 ⇒ 只 `single:` 跑）
