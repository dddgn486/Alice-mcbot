...
- **S-5 维生线（2026-09-15/16）**：D-226 决策表 / D-228 补 `baseTick` / D-229 冻结危险档 / D-236 溺水不再静默 /
  ...
  D-242 水里"规划得到、执行不了"的结论 / **D-243 水里垂直移动**（`PILLAR`/`ASCEND` 水里按住跳跃上浮）/ **D-244 水柱省料分支**（起点/目的地都是水 ⇒ 上浮**不放方块**，完成口径改"脚位到格即成功"；实测整段逃生零放置）。
  D-226/228/229/236 已 `WINDOWS_CLIENT`。
  零参数入口：`alice:survival_exit_check`（右键窒息 / Shift+右键着火）、
  `alice:survival_full_check`（一次右键跑完全部 **118** 条判据 ⇒ 聊天里看 `SUMMARY checks=118 failures=0 → PASS`）。
