...
- **保护区线（2026-09-18，D-313…D-317）**：区块级认领（忽略 Y / 全高度 / 旧格式迁移不静默丢）+ 零参数物品 `alice:protection_selector`
  ...
  FTB 只读兼容**已整体回撤**（`D-318`）；`D-319` 假人归属（创建者登记 + `/alice bots`/`adopt`，**客户端已验 ✓**）；`D-320` 两假人挨着爆栈崩已加闸 + 新 EXTRA 门先红后绿 + **同配置客户端复验 ✓**（旧 jar 133 处爆栈 / 新 jar 87 秒零异常、退档 2.1 秒）；**新增** `D-321` FTB 身份继承（`/alice ftb status|bind|unbind`：写入**只由显式命令**触发、代打 FTB 自己的命令 ⇒ 待 `WINDOWS_CLIENT`）+ `D-322` 步内 spawn/remove 假人崩（迭代活 map 的 CME，已修）；Xaero 联动三项**已登记**。
