...
- **保护区线（2026-09-18，D-313…D-316）**：区块级认领（忽略 Y / 全高度 / 旧格式迁移不静默丢）+ 零参数物品 `alice:protection_selector`
  （网格勾选 + 3×3 子格地形/明暗）+ **FTB Chunks 只读兼容**（外部认领源走唯一入口，理由码 `protected_ftb_claim`，不写回）。
  ⚠️ `D-317`：客户端首帧崩溃已定根因（Java 重载把像素当格号）并修 + 加「不许同名重载」门禁，待复跑。
