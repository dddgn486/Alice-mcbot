...
| **S-9** | 危险伤害**按采样血量差记录** ⇒ 被回血抹平的伤害不可见 | D-261 实测；`previousHealth()` **零生产消费者** | **✅ 观测已落地（2026-09-17，D-271）**：`survival/DamageLedger` + `BotManager.onBotDamage(LivingDamageEvent)` + 电池步 `damage_event_visible`（判据：精确 hits=4/total=4.0；反向对照可红）。**消费未做**（接进维生升级/保险触发/决策快照）⇒ 后半项仍挂在本行 |
