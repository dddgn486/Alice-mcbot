...
| # | 事项 | 判据 / 状态 | 我的评价 |
|---|---|---|---|
| **A1** | ⭐ `LumberJob` 补**终态闩锁**（照 `MineTask.terminalStatus` / `D-175`） | 判据 = `single:lumber_job` 期望 `idempotent=true`（修前 false），**先红后绿**。**代码事实我已核实**：`MineTask:87/328-329` 有闩锁 + `D-175` 注释；`LumberJob` `terminalStatus` **0 命中**（只有 `String terminalReason`） | 建议**先做**（无前置、判据现成、决定 CORE 能否转绿 ⇒ `D-352` 缓存能否生效）；⚠️ "这就是 `idempotent=false` 的根因"仍是**推断**（勘测侧自陈未跑） |
| **A2** | ⭐ **逐步缓存可行性验证**（先验后改机制） | 判据 = 同一提交"全量 vs 逐步缓存"**逐条判决完全一致**（`D-201` 附注一的逐步 diff 现成） | 同意**先验证**；⚠️ 步间若不独立 ⇒ **必须放弃**（否则假绿）。与 A1 scope 不相交，可并行 |
