...
| 13 | ⭐ **区域补种异步化**（用户 2026-09-19 原话要点）：砍完**立刻捡原木**；**等待窗口主动捡区域内树苗**（不止自己砍下的）；"该补的种"先记账（已有 `pendingReplant`），**捡够再补**；身上有苗则砍完立刻补 | ⏳ **需求已定口径**，实现前先出方案（涉及 `RegionLumberJob` + 主动收集 + 拾取闸门口径） |
| 14 | ⭐ **政策待拍板**：LLM 能否**自行**起一个拿到 `L2` 区内写权限的任务（今天等级只按任务类别授予、不看驱动身份；客户端实测里 LLM 在用户基地里自起 `region_lumber` 并砍树，聊天**零提示**） | ⏳ 待用户裁定；另附**可观测性**：LLM 起的任务应在聊天里留一行"谁起的" |
