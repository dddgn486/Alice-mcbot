...
| # | 落地件 | 现状 | 依赖 / 触发 |
|---|---|---|---|
| 1 | `SafeZoneData`：**安全区子集**（必须落在保护区内）+ `internalChunks()`（**四邻腐蚀**的内部区块；1 区块/散点区 ⇒ 空集 ⇒ 退化"进区即到"） | ❌ 未做（只有认领集，无类型） | **无依赖、纯只读几何** ⇒ 可独立开工（最省的第一步） |
| 2 | **返程目标链**：归位点 > 安全区内部 > 保护区内部；`SafeReturnTask` 终点与到达判据从"最近认领格 / 区块边界"改成"最近**内部**格" | ❌ 未做（今天 = `nearestClaimedCell` + `isClaimed` 边界判据，已绿） | 依赖 1 |
...
| 7 | **玩家文案统一**：claim 命令/勾选 UI 的"保护区"= 正名；返程/`/alice status` 侧改口 | ❌ 未做（今天同一份认领集两个名字） | 随 1，或独立小改 |
...
② 内部区块判据在长条/环形区表现不好（到家后仍贴边）⇒ 换判据；③ 任务区随 scope 生灭若导致"中途重规划丢授权" ⇒ 补持久化。
