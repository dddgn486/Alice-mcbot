...
> ⭐ **`survey/22 §1.5①` 的挖矿收集距离窗口**（`D-345`：产物 >32 格被永久退休 ⇒ `product_not_collected`；
> **红证据已在**（`module:mine_drop_range`），**只差修法两族二选一**）。
...
| 17 | **中断点语义待裁定**（`D-338` 附注十三）：① **玩家显式停止**（`immediateStop` 旁路）—— ⚠️ **审计修正**：它**现已记 `STOP` 进事件环**（`D-338` 附注十五）⇒ 「不记事件环」这半句**已过期**；剩下的真问题只有**「不叫 LLM」**（是有意设计还是缺口，待裁定） —— 要不要把"被玩家停了"写进事件环（我倾向：不叫 LLM，但要记）；② 启动前拒绝（`unimplemented` 实体目标 / 修路计划非法）今天不进事件环 | ⏳ 待用户裁定（已落档，未动代码） |
