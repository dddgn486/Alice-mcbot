...
> ⏳ **`J-1` 的退出条件（建议 7，待你拍）**：`J-1` 现在是**开放列表**（三天长 22 行），需要一个"什么时候算完"的句子。
> **提议**：「**鱼骨在真实存档里连跑 15 分钟不失败，且 `SUMMARY` 的 `mined` / `spursAbandoned` / `collected`
> 三个计数互相自洽 ⇒ `J-1` 关闭**」。
...
| 1.4u | **`SUMMARY` 的 `spurs=` 标签误导**（读数，零风险） | 汇总行不再让人误读成"子巷挖完了" | ⏳ **待开工**（`F3`）。`FishboneJob.emitSummary`（`:1318`，实参在 `:1342`）打印的是 `spurBranches()-spursAbandoned / spurBranches()`，即 **未放弃数/总数**；本轮真机实测 `main=1/20` 与 `spurs=12/12` **并排出现**（主巷只走了 1 格、子巷一条都没开挖）⇒ 极易误读。建议改成 `spursAbandoned=0/12`（含义唯一）| 0.1 轮 |
...
| 1.4w | ⭐ **归因口径通用化：`no_valid_standing_point` 不再被改写成 `no_reachable_standing_point`**（`survey/35 §9` 桶3-3「**最省力收益最大的一刀**」） | 「做不了」与「不让做」的读数**分得开**（`survey/35 §5`：现在两级丢失，长得一样） | ⏳ **待开工（排期 = `J-★` 第 2 段 2.1/2.2）**。事实：`MiningPlanner.planDirect` 的真实理由 `no_valid_standing_point`（`:255`，垃圾桶码）被 `:190` **一律改写**成 `no_reachable_standing_point`（只放行 `SEARCH_INCOMPLETE`）⇒ 上游永远看不到"到底为什么站不上去"。⭐ **不用发明**：唯一做对的先例 = `D-329`（`MineJob:160/167`：`40` tick 冷却 + 连 `8` 次认账；`:414-423` 冷却期刻意不工作）⇒ 把那个口径搬过来即可。判据 = 造"遮挡导致无站位"几何 ⇒ 读数里能看见 `no_valid_standing_point`（而不是被总括码吃掉）；**红臂** = 退回改写 ⇒ 红。⚠️ 它是 `1.4i`（归因四分类）的**前置** | 0.3 轮 |
| 1.4x | ⭐ **沙砾/重力方块的「挖掘期暂停」判据同步**（`survey/35 §9` 桶3-4；= 本会话新清单 `K1` 的**前半**） | 挖通通道后**沙砾不会在 2 tick 后把刚挖的格填回**；代价模型**先不动** | ⏳ **待开工（排期 = `J-★` 第 2 段 2.3）**。事实：`pathing/` 内 `FallingBlockEntity`/`avoidBreaking`/`includeFalling` **全 0 命中**（6 处命中全在 `road/RoadBuilder:136,163` + `task/RoadBuildTask:234,256` —— **造路模块已有先例**）；Alice 只移植了 `MovementAscend` 那一条（`AscendExecutionFactory:65-70`）。修法 = **照 Baritone `Movement.java:157-160`**（目标格里有在飞的 `FallingBlockEntity` ⇒ **暂停挖掘**），≈20 行，**不碰成本模型**（成本乘子 `includeFalling` 按 `survey/35 §9` 桶3-5 = **可延后**，见 `J-★` 2.5）。⚠️ 测量陷阱：修复后必须**等一个重力 tick 再复检**（`FallingBlock` 落体延迟 2 tick）—— 立刻复检会读到"已修好"的**假绿**。⚠️ 与 `F1` 同族：`F1` 是**空间**维度判据错位、沙砾是**时间**维度判据失同步 | 0.3 轮 |
