...
| **⭐ 新工作项（独立于 R-2）** | **保护区区块化 + 地图式勾选界面**（客户端 UI 子系统，带协议）⇒ 结构见 `D-305` ①′ + `D-307`；⚠️ 需要一个**客户端轮次**验证 | **1/2 ✓（`D-313`）· 2/2 已实现 ✓（`D-314`）· 3/3 地图细化已实现 ✓（`D-315`）**：1/2 = `SafeZoneData` 改 ⭐ **区块级 2D 认领**（忽略 Y、全高度）+ **旧格式自动迁移**（相交即认领、计数可查、不静默丢）；2/2 = **零参数物品 `alice:protection_selector`** + 17×17 区块网格（左键认领 / 右键取消 / 拖动连选 / 关闭即批量提交）+ **3 包协议**（S2C 快照 / C2S 批量动作 / C2S 空请求自愈；动作包**无维度字段** ⇒ 维度由服务端裁定）；3/3 = **地图细化**（每格 3×3 子格各采自己的列 + **高度明暗** + 渐进入场不占帧 + 边缘区块刻度 + 悬停显示地表方块/高度 + 图例；`G` 键关明暗）。夹具判据 **32 → 57 → 63 条**（协议契约 / 界面几何 / 子格平铺 / 中心向外采样序；**零世界写入**）、反向对照两轮共 6 注入 ⇒ 10 条如期红、`module:protection` **1/1 PASS**、CORE **49/49**（步序 diff = 0 ×3）。⚠️ **界面观感仍未验证**：`WINDOWS_CLIENT` / `USER_ACCEPTED` 待取，入口与判读见 `docs/TESTING_GUIDE.md` §2.5。**FTB Chunks 只读兼容已实现 ✓（`D-316`）**：新 `protection/ClaimSources`（缝 + 注册表）+ `compat/ftbchunks/FtbChunksClaims`（反射软依赖，解析不到即空转）+ `SafeZoneData.protectionReason` **唯一入口**咨询外部源（理由码 `protected_ftb_claim`）+ 快照分两列/界面**紫框**标来源；`mods.toml` 加 `mandatory=false` 的 ftbchunks 声明。判据 63 → **73**（假源离线可判：唯一入口生效 / **不复制进存档** / 快照分列 / 异常不冒泡 / 缺模组空转 / 自清理），反向对照 3+1 注入全红。⚠️ **`available()=true` 那条路从未跑过**（本环境取不到 FTB 工件：CurseForge 有 Cloudflare、官方不在 Modrinth）⇒ 待客户端轮次；⚠️ "FTB 保护是否作用于我们的玩家化假人"仍待实测（验收脚本见 `TESTING_GUIDE.md` §2.5.1）|
