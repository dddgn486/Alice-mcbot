...
| **⭐ 新工作项（独立于 R-2）** | **保护区区块化 + 地图式勾选界面**（客户端 UI 子系统，带协议）⇒ 结构见 `D-305` ①′ + `D-307`；⚠️ 需要一个**客户端轮次**验证 | **1/2 ✓（`D-313`）+ 2/2 已实现 ✓（`D-314`）**：1/2 = `SafeZoneData` 改 ⭐ **区块级 2D 认领**（忽略 Y、全高度）+ **旧格式自动迁移**（相交即认领、计数可查、不静默丢）+ `claim/unclaim` 诊断命令；2/2 = **零参数物品 `alice:protection_selector`** + 以玩家为中心的 **17×17 区块网格**（左键认领 / 右键取消 / 拖动连选 / 关闭即批量提交；地形色客户端自采）+ **3 包协议**（S2C 认领快照 / C2S 批量动作 / C2S 空请求自愈，动作包**无维度字段** ⇒ 维度由服务端裁定）。夹具判据 **32 → 57 条**（新增协议契约 + 界面几何两组，**零世界写入**）、反向对照三注入 ⇒ 6 条如期红、`module:protection` **1/1 PASS**、CORE **49/49**（步序 diff = 0）。⚠️ **界面观感仍未验证**：`WINDOWS_CLIENT` / `USER_ACCEPTED` 待取，入口与判读见 `docs/TESTING_GUIDE.md` §2.5 |
