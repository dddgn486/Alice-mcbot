...
**§6.47 A5 的 LLM 路径客户端验证 ✅（jar `288fc05f…`，2026-09-13 20:39）**
```
...
- **判别实验（下一步）**：`/alice battery full`（35 项 = 整理前的全集）。FULL 绿 ⇒ 缺的是"前置互锁"，
  修法 = 把清场写成**显式前置断言**（每个夹具开局断言：当前菜单是玩家自带菜单 / bot 在起点 / 方块实体状态已复位），
  然后**逐条**把步骤移出 CORE 并每移一条复跑一次；FULL 也红 ⇒ 另有别的原因，再上探针。
