...
2. **细雪冻结没有危险档**：`freeze` 伤害现在会真的发生，而 `HazardType` 只有
   `LAVA_CONTACT/SUFFOCATING/LOW_AIR/ON_FIRE/WATER_CONTACT` ⇒ **否决链覆盖不到冻结**。
3. **`LOW_AIR` 现在产线可达**（空气真的会消耗）—— S-5 那一档从"纸面"变成"活的"，建议后续补一条
   真人可复现的溺水入口。
4. **CORE 里的"治疗来源"未定位**（18→20；单步跑不出现）—— 疑似前序机器/药剂步骤的残留效果。
