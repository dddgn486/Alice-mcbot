...
| 4 | **区域级权限 / 预算授予面** + **任务区声明**（任务自声明，随 `scopeId` 生灭；目标内 `KEEP` / 目标外提权+预算+`TEMP`） | ❌ 未做（今天授予全是**动作级**）；⭐ **粒度已定**（`D-338` 附注四②）：**工作区域 = 方块级**（任务语义）⇒ **任务区 = 其所占区块的最小覆盖**（区块级授权封套）；**单向派生**；目标内/目标外按"是否 ∈ 目标集"判 | 需先拍板第 5 行；**实验载体 = 区域砍伐**（`RegionLumberJob`）|
...
| 8 | **归位点**（玩家设定，返程最高优先） | ✅ **形态已定**（`D-338` 附注四①，2026-09-19 用户）：**每 bot 一个**、**暂时用命令**（`set`/`clear`/`show`，零参数、以执行者站位为准、默认半径 3）；⭐ 它同时是"小基地 / 想要精确落点"的**正解**（不许再去优化内部区块几何） | **本片实现**（插在返程链最前：有归位点 ⇒ 跳过区几何）|
