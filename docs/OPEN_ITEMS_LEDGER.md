...
| **⭐ `FarWalkTask` 的第一个生产调用方（未接）** | 任务本身已实现且经执行器实测（300 格 / 2 跳 / 1111 tick），但**仍无生产调用方** —— 预期第一个是 `D-327` 机制 B「任务失败后返回安全区」或决策层的「去某坐标」目标（两者都还没做）。⚠️ 决策层动词表（`check-goal-vocabulary`）是闭集 ⇒ 加「去坐标」目标要同时改提示词/决策层，属另一件事 |
