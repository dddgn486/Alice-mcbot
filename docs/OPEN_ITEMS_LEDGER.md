...
| 1.4p | ⭐ **「bot 被自己搭的方块拦住」**（真机第四轮；截图 `2026-09-25_23.17.44.png`） | 作业不许把自己的通道格填掉；回程不再因为自己放的方块变成 `UNREACHABLE` | ⏳ **已取证、未修**（`docs/reviews/2026-09-25-真机第四轮-自己搭的方块拦住回程.md`）。链条（逐条锚点）：`collect-drops` 为够一个掉落物，用 `PILLAR/STEP_PLACEMENT` 把圆石放进了 **bot 自己刚挖通、且刚走过** 的通道脚位格 `(-65,54,181)`（= south9 unit 44）⇒ 【① 直接原因】；该格**无保护区**（`ProtectionZones.isWild`）⇒ 按 `D-398 R1/R2` **不记账、不恢复**（`[Ledger] skip … 区外 … 累计 15 次`）⇒ 那块圆石**永远不会被回收** ⇒ 【② 系统原因，改它得先改裁定】；随后 `SPUR_RETURN` 从 `(-66,56,181)` 回 `(-65,54,172)` **`UNREACHABLE`**（`descend_precondition=11`、`nodes=3`、`searchLimit=0`）⇒ `return_failed` ⇒ 作业 FAILED、bot 被留在原地 ⇒ 【③ **未定**：`descend_precondition` 是聚合计数，不打印是哪条子条件拒的 ⇒ 必须先上探针】。⭐ 好消息：本轮 `segment_stall=0`（机制 A 生效，上轮 2 次）。⭐ 另一条弃巷仍是旧账：`target=-65,54,163 footPassable=true`（= `1.4j①` 签名）。候选修法见文档 §4：**A=通道保护 `I5`（`1.4j②` 已裁定，本截图是它的真机证据）/ B=作业自清理自己放的方块 / C=需要填自己通道时干脆不爬 / D=先上探针把 ③ 定死（推荐先 D）** | 0.5 轮 |
