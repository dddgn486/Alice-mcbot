...
| **C4** | ⭐⭐ **新线索（两份报告都没有；我从 CORE 日志取证）**：`lumber_job` 的**真正阻塞点** = ① **站位给不出**：`trunk_too_tall(unreachable=4,hard=2)`（`LumberCandidateSource:107-115`，2 格**硬遮挡且无清障方案**）+ 同窗口 `MiningPlanner planning failed reason=no_reachable_standing_point`；② **破坏被拒**：`[WRITE-REFUSED] … reason=world_unchanged（destroyBlock=false）` + `progress=1.07`（`BlockBreakSession:104-120`，`D-323` 那一类）；放大机制 = 失败一次即 `already_attempted`（`LumberJob:646`）⇒ 本次 job 内**永久排除**该树 | **归属已澄清 = 不是 `Z1` 连带**（`lumber_job=FAIL` 最早 `20260920-234751-core.log`，早两天）；⭐ 若 ① 与 `P5`/`D-391`（被包住的目标给不出站位）**同族**，则这是**那条缺陷类最便宜的可红复现**（`single:lumber_job`，确定性地形 ~1000 tick）；⚠️ ②的来源**未定，不猜**（需探针/反编译源） | **待查** |
