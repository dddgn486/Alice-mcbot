...
| 11 | **区域补种缺料**：自己砍下的树苗在地上被**被动拾取闸门**挡（`[Pickup] blocked … provenance=FOREIGN policy=ASK`）⇒ 区域任务 `tool_missing` 中止 | ✅ **用户 2026-09-20 裁定：让区域任务显式收集自己的掉落物** ⇒ **开工项**（`D-350`）。边界：只收**我方归属**掉落物（`DropPolicy.effectiveProvenance` 的授权面，不许放宽到 `FOREIGN`）；对账契约按 `D-349` 的 `JobKindContract` 声明（区域任务的成功判据 = 补种不缺料） |
