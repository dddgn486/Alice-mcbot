...
| **⭐ `D-337 附注二`：远距离 hop 的每跳代价 = 一次整片洪泛（未修，可选优化）** | 收口实测：粗目标在**边界外不可达** ⇒ A\* 把**已加载区整片**展开完才收手 ⇒ `nodes=20000`（= 上限）/ **147~163 ms**；对照**可达**的 640 格 = `641 节点 / 8~14 ms` ⇒ 分段 hop（`PathRetryRunner.MAX_PARTIAL_HOPS=4`）每跳都付满预算。**可选优化（未做）**：把粗目标**先夹到已加载区域边界**（或给粗目标单独的节点预算），让搜索重归线性；触发条件 = ④ 把 `GoalNearXZ` 接进生产/任务层时一起量。⚠️ **卡顿时长仍未测**（超平坦下同步加载便宜；真实世界数字要用户世界里测）|
