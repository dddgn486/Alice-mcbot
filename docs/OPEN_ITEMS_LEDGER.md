...
| # | 事项 | 判据 | 依赖 | 状态 |
|---|---|---|---|---|
| **RC1** | **条件门落地**（`D-402`）：N1–N4 必要条件 + B1–B3 有界条件 + A1/A2 自动触发点；**不满足 ⇒ 进账本待办**（不是失败、不是丢弃）；作业层统一收尾（成功/失败/超时/取消四路径） | 夹具：① 满足条件 ⇒ 自动收 ② 未加载/越界/会摔/会产生新债 ⇒ **留待办且有归因**（不许静默） | Z1 + 阈值裁定 | 待做 |
| **RC2** | **显式回收入口**（用户 2026-09-22 裁定）：① 玩家显式授权 ② 显式发布**回收任务** ⇒ 才发起回收；**不自动重试、不做启动补收（T3 否决）**；**失败保留账本**；任务内遇未加载区块**不强制加载**（T4 同意） | 夹具：无显式入口 ⇒ 账本**不动**；显式发布 ⇒ 收；未加载 ⇒ 如实报告不加载区块 | 无 | 待做 |
