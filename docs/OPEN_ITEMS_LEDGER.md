...
| **RC1-进度** | 🟡 代码已落：RestoreScopeTask 补第①条保留条件「已加载」⇒ 归因码 chunk_not_loaded，且置于 level.getBlockState 之前（否则未加载区块会被强制同步加载，把回收变成开图）。⚠️ 回归演练未跑完（上次 single:restore_underfoot_safety 被中断）＋三条条件的夹具臂未补 ⇒ RC1 未完成。待续：① 跑夹具确认不回归 ② 补 not_ours / chunk_not_loaded 两臂 ③ 注入即红 | 🟡 进行中 |
| **PL-1** | 产品线：**从脉外打进实心脉**（= Baritone `BuilderProcess` 级能力：清盒子 + 2 格高走廊执行器 + 作业级预算 + ticks 判据）| 设计输入齐备：`docs/plans/2026-09-24-P5a-两格高走廊设计.md`（含 `TunnelCommand:51-84`/`:46` 对照与三条实测事实）| 登记（D 段产品线开工时做）|
