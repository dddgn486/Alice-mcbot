...
- **基-2 决策层进回归电池**（电池 9 → 25 项）；**基-3 S4 事件层**——`WINDOWS_CLIENT`；
...
> ② 本轮 25 项电池里的 `transfer` 步（11:06，`2026-09-13-1.log.gz`）——
...
| D-169 电池自杀 | K-3 退出电池（25 → 23）+ `fixture_not_top_level` 前提断言 + 手工入口两模式 | 电池 `(25/25) ticks=3507 → PASS` | `WINDOWS_CLIENT` |
...
**§6.13 第四次电池（2026-09-13 晚）**：25 项 **22/23**，唯一失败 = `transfer` 步的 `end_to_end`
...
**§6.15 2026-09-13 收尾复测（客户端 12:42）**：电池 **(25/25) ticks=3301 → PASS**、
