...
| 7 | **玩家文案统一**：claim 命令/勾选 UI 的"保护区"= 正名；返程/`/alice status` 侧改口 | 🟡 **一半已落地**（2026-09-19）：`/alice protection list` 现在打印「当前位置：保护区=…/安全区=…」+「保护区（父类）/ 安全区（子类）：summary」；返程侧文案仍写着"安全区"（属第 2 件一起改） | 第 2 件落地时一并收口 |
...
④ ⚠️ **四邻腐蚀下 2×2 区块（32×32 格）及以下没有内部区块** ⇒ 若用户基地就是那么大，"向中心靠"看不到效果
（退化成"进区即到"）⇒ 改**八邻**（`SafeZoneData.internalChunks` 一处）。
