...
| **A2′** | ⭐ **一次引导跑多个点名步**（`single:<步名>[,<步名>…]`）：`HeadlessBattery.java:138` 现在 `setOnlySteps(List.of(name))` **只收一个名字** | 判据 = 与**逐条 solo 判决一致**（**对照就是 `A2` 那张现成的表**）；收益 = 省 N−1 次引导（实测每次 **20–26 s**：41 步从 ~17 min → ~5 min） | ⭐ **推荐**（小改动 + 判据现成 + **不跳过任何执行** ⇒ 不引入"陈旧判决"风险；对 `write_policy` 反而**更强**：前缀人口变大、更不容易空集真）⇒ 因 `A2` 作废而**上浮**，待用户裁 |
