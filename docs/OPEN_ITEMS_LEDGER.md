...
| 1.4v | ⭐⭐ **Baritone 功能面缺口清单**（用户 2026-09-26：「做一次梳理，到底还有哪些 Baritone 功能我们没实现」） | 手里有一份**可复核的**"Baritone 有、Alice 无"全集，避免每次凭记忆列 | ✅ **已落盘**：`docs/reviews/2026-09-26-Baritone功能面缺口清单.md`。**规模**：Baritone `src/main` 155 + `src/api` 161 = 316 文件（两棵树：`reference/baritone/` = MC 1.21.4 · `reference/baritone-1.20.1/` = 1.20.1，**引用以后者为准**）；Alice `src/main/java` 510 文件。**方法**：8 区域并行只读审计（`process`/`movement`/`command`/`behavior`/`cache_selection`/`schematic`/`utils`/`api_root_settings`），每区域逐类枚举 Baritone 侧 + 在 Alice 侧 grep 对应物；**每条"缺失/部分"都写明搜过的词与 0 命中**（可逐条复核），**"刻意不做"必须给 `D-编号`**。**产物** = **158 条功能点**：已实现 17 / 部分 69 / 刻意不做 14 / **缺失 58**；原始区域产物在 `run/baritone-gap/*.md`（`run/` 已 gitignore）+ 合并器 `run/baritone-gap/merge.py`。**质量加固**：跨区域同题 7 个文件已标注（§0 各计一次、§1 已去重）· `utils` 与 `schematic` 重复 1 条已折叠 · 30 条带行号引用在 1.20.1 树上**复核 0 问题** · 抽查 9 条头条声明（7 条字面 0 命中、2 条差异查清后仍成立，需收紧范围措辞）。⭐ **§1 只列影响产品线的 21 条**（K1–K9 内核 / M1–M5 挖矿 / L1–L2 伐木 / C1–C2 合成传输 / O1–O5 命令面 / V1 观测），**§2 是"明确不做/不在产品线"清单**（`D-编号` + 理由，防止"为对齐而实现"）。**§1.7 建议排期**：K1 重力方块族（**同时是"局部扫描自动复位"方案的主要敌人**）→ M5（`1.4r` 真缺陷）→ K2 `GoalAdjacent`（一条改动松开挖矿站位/伐木站位/施工器三处手搓）→ K6 `PathEvent` + V1 `PathRenderer`（路径中途的观测前提）。⚠️ **同时发现三条文档/数字问题**：① `D-418` §三 断言⑤ 正文"今天 Alice **没有配置面**"**已过期**（门禁实跑 = `配置面 = 存在 12 处（已断言不含底线名）`）⇒ **已在 `AI_DECISIONS.md` 就地更正**（保留原口径 + 注明以门禁实跑为准）；② `api_root_settings` 提醒：`cache_selection` 区域 9 条 `缺失` 多是**同一个已裁定延后项**（`D-044`⑤/`D-067`⑧ 异步快照层）的分身 ⇒ **不许算成 9 个待办**；③ `mine_reach_probe`（EXTRA）在**新世界**里必然 `start_standable=false / realSearches=0/17` ⇒ 它应改成 `CheckStep.skippable`（无真机地形时 **SKIP 而非 FAIL**，`HANDOVER.md:123` 早有"结论不作数"的登记）—— 0.1 轮，未做 | 1 轮（已落）|
...
| 1.4s | **`I5` 破坏面**（草案 `S2` 的原文口径：「施工期间**别的写入者不许破坏** I1/I2 的格」） | 通道的**地板/支撑**不再被自己的追簇或 `collect-drops` 的 `PATH_ACCESS` 挖掉（A1 处境） | ⏳ **未落地**。裁定原文见 `docs/plans/2026-09-25-通道施工器草案.md` §4 `I5` + §7 `S2`（红臂 = "去掉保护 ⇒ 地板被挖掉 ⇒ 红，用 A1 的真实破坏序列造场景"）。⚠️ 与本次落地的**放置面**是同一不变式的另一侧（"这条通道在施工期间必须保持可通行"），载体现成（`TaskTargetProtection` 破坏面 + `FishboneTemplate.cellSet()`），**但破坏侧要做"豁免鱼骨自己挖穿通道"的例外**（不然连通道都挖不出来）—— 这个例外的口径需要裁定 | 0.5 轮 |
