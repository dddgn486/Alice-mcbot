...
| **J-10 掉落物归属与收集授权（D-138 提案）** | 现在只认"我方破坏事件配对"⇒ 漏掉**我方行为的间接后果**（树叶衰减掉树苗/木棍、仙人掌/甘蔗被移除支撑后弹出）与**玩家派活**（捡玩家授权区里的东西）；同时"默认谁都捡"危险 ⇒ 需要 `DropProvenance` + `DropPolicy(AUTO/NOTIFY/ASK/IGNORE)` + `CollectGrant`（`anyDrops` 退役）。设计见 `DECISION_LAYER_DESIGN.md` §3.1，待用户裁定 4 个问题后开工 |
