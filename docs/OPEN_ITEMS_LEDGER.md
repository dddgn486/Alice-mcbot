...
| **PL-1** | 产品线：**从脉外打进实心脉**（= Baritone `BuilderProcess` 级能力：清盒子 + 2 格高走廊执行器 + 作业级预算 + ticks 判据）| 设计输入齐备：`docs/plans/2026-09-24-P5a-两格高走廊设计.md`（含 `TunnelCommand:51-84`/`:46` 对照与三条实测事实）| ⏳ **开工清单已定档（2026-09-24）**：`docs/HANDOVER.md` 断点④（三条已实测事实 + 两条硬判据 + 三条真陷阱 + 建议的第一切片「只做执行器、先测成本再测能力」+ 成本算术 ≈9 ticks/内联格 + 边界）。**本轮因上下文预算不足未开工** （它要动 `MineTask` 执行结构 + 跑 2–4 min/次的夹具 + CORE 逐步 diff）—— 下一轮从这里直接动手，不必重新取证 |
