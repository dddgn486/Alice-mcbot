...
**本次修复带出的后续项（按我的优先级）**：
1. **bot 的血只减不增（值得排期）**：伤害现在是真的（火焰/冻结/溺水/摔落），但**自然回血仍然没有**
   —— 它在 `Player.tick()→FoodData.tick()` 里，那半没补 ⇒ bot 掉血后永不恢复（吃药水/瞬间治疗有效，
   因为效果现在会 tick）。这让 **D-228 的选项 A（补 `doTick()`，覆盖食物/饥饿/自然回血/效果计时）**
   从"以后再说"变成"值得排期"；代价是物理推进顺序/次数会变（需重跑 CORE + 真人复测）。
...
3. **`LOW_AIR` 现在产线可达**（空气真的会消耗）—— S-5 那一档从"纸面"变成"活的"，建议后续补一条
   真人可复现的溺水入口。
4. **"治疗来源"未定位**（2026-09-15 晚更新：**真人客户端也出现** —— 冻结相位里 `19.0 → 20.0`，
   而夹具刚在相位前 `removeAllEffects()` ⇒ 有东西在**持续重新施加**；Alice 侧无任何治疗代码）
   ⇒ 最可能是 **Regeneration 类效果**（补 `baseTick` 后药水效果才开始真的 tick）。
   **一条命令可核实**：`/data get entity tango active_effects`。
5. **行为变化需知会**：药水效果**现在会正常计时并到期**（此前永不失效）。
