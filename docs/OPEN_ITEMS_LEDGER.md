...
| **V-4** | **Baritone 对照实验未跑** | **❌ REAL-GAP（仅剩"没跑"，不是"没准备"）**：datapack 函数**都在**（`run/world-pristine/datapacks/alice_test/.../contrast_fall|contrast_trace_fall|contrast_pillar.mcfunction`），缺的只是**一次客户端实跑**（需客户端装 Baritone + `#goto`）。**✅ 2026-09-17 已补齐"读数口径"标注**（V-4 原本要求的那条）：`BARITONE_CONTRAST_TESTING.md` **§6-6** 写明 Alice 在 FALL 场景有两条额外守卫（`no_deep_fall` / `fall_recover_guard`，Baritone 没有），并提醒 **不要**开 `DESCEND_OVERSHOOT_GUARD`（默认关 = Baritone 原样，开了又多一条不可比因素）；矩阵两条对照行也加了指针。⇒ **剩余动作在用户侧**：跑 4 次（2 场景 × 各 1～3 遍取最小）并回填 §5 记录表 |
