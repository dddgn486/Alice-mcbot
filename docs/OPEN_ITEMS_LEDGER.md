...
| 序 | 事项 | 硬前置 | 依据 |
|---|---|---|---|
| 3.1 | 通道**补路动作 + 归因码**（= `D-450` 的落地） | 0.4 | `§3.5`「三层已齐、只缺判据」；⚠️ 须显式声明「**鱼骨是区外任务**」（保护区内语义相反，会被 `RestoreScopeTask` 收走） |
