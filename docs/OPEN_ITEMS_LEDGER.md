...
## §11 当前工作队列（2026-09-22 用户裁定顺序）

> ⚠️ **2026-09-26：本表已降级为历史归档（只读）** —— 唯一排期来源 = `### J. 工作排期表` 下的
> **`J-★ 2026-09-26 排期`** 块（`survey/35 §10` 建议 1：两张表并存 ⇒ 同一条待办可能两份不一致，`F2` 就是活例）。
> 本表保留原样（表头规则「**只许追加/改状态，不许凭记忆重写**」照旧），不再更新顺序。
...
#### J-★ 2026-09-26 排期（**唯一执行顺序出处**；`survey/35 §10` 建议 1/2/3/4/6/7 的落地）

> ⚠️ 本块只回答「**按什么顺序做**」。每条的状态/判据仍在原行（`1.4x` / `J-x`）—— **id 不变、顺序单独一处**。
> **成本三段**（建议 3）：**实现**（写码 + 夹具）/ **验证**（门禁红臂 + 电池 + CORE）/ ⭐**客户端**（要你操作 + 同步 jar）。
> ⚠️ **真机轮 = 新发现的来源，成本必须单列、不计入实现轮** —— 证据：`1.4a`–`1.4v` 三天新增 22 行，
> **绝大多数是"发现"而不是"实现"**（`1.4k` 取证 · `1.4p` 取证 · `1.4r` 确认 · `1.4v` 审计）。
> **状态词表**（建议 4）= `§11` 表头那套**验证等级**：`IMPLEMENTED / COMPILES / SERVER_TESTED / WINDOWS_CLIENT / USER_ACCEPTED`；
> 行内 `⏳/✅/⚠️/📋` 只是视觉提示。⛔ **先看 `J-4`（明确不做）再读本块**（建议 6）。
> ✅ **`J-1` 的退出条件（建议 7，用户 2026-09-26 裁定 A：原文采用）**：`J-1` 是**开放列表**（三天长 22 行），
> 现按用户拍板收口为：「**鱼骨在真实存档里连跑 15 分钟不失败，且 `SUMMARY` 的 `mined` / `spursAbandoned` /
> `collected` 三个计数**与日志逐条对得上（`spursAbandoned` 能对上前 12 条支洞的逐条终态）⇒ `J-1` 关闭**」。
> ⚠️ 键名口径：`1.4u` 已把误导标签 `spurs=` 改成 **`spursAbandoned=<放弃数>/<总数>`**（分子就是放弃数）——
> 本句用的是**改后**的键名（改前那句 `spurs=` 会被读成"未放弃数"，对不上账）。
>
> **✅ 用户 2026-09-26 对「挡下一轮的 ① 组四条」的裁定（逐条，全部 = 推荐项）**：
> ① **先跑真机、AI 并行做第 2 段**（`J-★` 第 1 段优先；理由是 `R8`：不测就叠改造 ⇒ 真机红了无法归因）；
> ② `D-450` 的补格额度**与 `C8` 共用同一份**（`bridgeBlockBudget()`，单一出处）；③ `J-1` 退出条件用上面那句原文；
> ④ **采纳"切两刀"**（`survey/36 §9-Q`；登记为 `D-452`：第一刀 = 补路动作 + 归因码 + 记录图 + 沿单元返程；
> 第二刀 = 脱离→挖簇→回归，**本轮不做**）。

**第 0 段 · 文书收口**（0.3 轮，**纯文档 / 一行注释，不碰行为**）—— `survey/38 §0` 四件 + `survey/35 §9` 桶3-11

| 序 | 事项 | 状态 |
|---|---|---|
| 0.1 | `TaskZoneRegistry:24` 措辞冲突修掉（原文与 `D-338` ① 直接冲突，已实际误导过一轮勘测） | ✅ 已落（2026-09-26） |
| 0.2 | `D-448` 锚点任务定义与定位 + 两条附注（`survey/26` 更正 + 三轴新发现） | ✅ 已落 |
| 0.3 | `D-449` 第一层收口条件（照 `D-430` 形状四段） | ✅ 已落（⭐ 含两处 ⏳ 待拍） |
| 0.4 | `D-450` 通道成品规格 `I1`/`I2`/`I3` + "补格"额度出处（= `F2` / `1.4t` 收编） | ✅ 已落（额度是否与 `C8` 同一份 ⏳ 待明确） |
| 0.5 | `D-451` `I5` 破坏面豁免口径（= `1.4s` 口径） | ✅ 已落 |
| 0.6 | 排期表整治（`survey/35 §10`） | ✅ 已落（只留一处 ✅ · 执行顺序=本块 ✅ · 成本三段 ✅ · 状态词表 ✅ · `J-4` 前置指针 ✅ · `J-1` 退出条件 ⏳ 待拍） |

**第 1 段 · 真机先行**（**1 客户端轮**）—— `survey/36 §8-序 0` + `R8`（自称"最要紧的实操风险"，**唯一一条顺序性建议**）

| 序 | 事项 | 内容 |
|---|---|---|
| 1.1 | ⭐ **先把当前 jar 测掉** | 入口 = `alice:fishbone_job`（零参数右键）；jar 含 `D-446`（`I5` 放置面）+ `F1`（单格判据）。⚠️ **不测就叠架构改造 ⇒ 真机红了不知是 `I5` 还是新状态机**（`R8`）；⚠️ 第二步：主巷有一格空 ⇒ 记 `SUMMARY` 的 `advance`/`cell=` 停在哪（`F1` 的验收） |

**第 2 段 · 最便宜的一刀**（0.5–1 轮，**不叠新逻辑、不碰成本模型**）—— `survey/35 §9` 桶3 的先做三条 + 两条 0 成本项

| 序 | 事项 | 台账 | 依据 | 成本 |
|---|---|---|---|---|
| 2.1 | `SUMMARY spurs=` 误导标签 → `spursAbandoned=0/12` | `1.4u` | 桶3-1（**改一个字符串表达式**） | 0.1 |
| 2.2 | ⭐ **归因口径通用化**：`no_valid_standing_point` 被 `MiningPlanner:190` 一律改写成 `no_reachable_standing_point` ⇒ **搬 `D-329` 现成口径**（`MineJob:160/167` 先例） | 新行 | 桶3-3「**最省力收益最大的一刀**」；`1.4i` 的前置 | 0.3 |
| 2.3 | ⭐ **沙砾「挖掘期暂停」判据同步**（Baritone `Movement.java:157-160` 现成公式；≈20 行；**只在挖掘期**、不碰成本模型） | = `K1` 前半 | 桶3-4 | 0.3 |
| 2.4 | `D-362` 保护安装点从构造器挪到**首 tick** + 结构门禁 | `1.4r` | 桶3-2（**已确认**缺陷） | 0.2 |
| 2.5 | `K1` **定性质**：拆「站位形态（`assumeVerticalShaftMine` 必做）」vs「成本乘子（`includeFalling` 可延后）」 | `K1` 后半 | 桶3-5（**0 行代码** ⇒ 把 `K1` 从 0.5–1 轮降级为"只补判据同步"） | 0 |

> ✅ **第 2 段落地记录（2026-09-26）**：2.1 / 2.2 / 2.3 / 2.4 **全部已落**（`COMPILES` + 门禁 + 电池），
> 逐条证据见 `1.4u` / `1.4w` / `1.4x` / `1.4r` 四行的"落地"小节；2.5 见 `K1` 行（0 行代码，纯定性）。
> ⭐ **一条新门禁 + 一条新电池步**：`tools/check-protection-install-point.py`（`1.4r` 的结构判据，自带 6 条合成红臂）
> 与 `mine_falling_pause`（`1.4x` 的行为判据，**MAIN ⇒ 进 CORE**，四臂含两个负对照）。

**第 3 段 · 通道施工器第一刀**（≈2 轮）—— `survey/36 §8` 的依赖链，**第二刀切掉**

| 序 | 事项 | 硬前置 | 依据 |
|---|---|---|---|
| 3.1 | ✅ **已落地（2026-09-26，`D-454`，见 `1.4z-e`）** —— 实测改写：「补路动作」由**既有** `D-440` 走位承担，「归因码」= `channel_floor_missing:<cause>`；还剩「沿单元返程」在 3.3 | 0.4 | `§3.5`「三层已齐、只缺判据」；⚠️ 须显式声明「**鱼骨是区外任务**」（保护区内语义相反，会被 `RestoreScopeTask` 收走） |
| 3.2 | 施工**记录图**（3 态 = 未施工 / 已施工无变化 / 已施工已被修改；靠服务端事实更新） | 0.4 | `§5`：**不需要落盘**（`FishboneJob` 全仓 0 处 `SavedData`/`serializeNBT` ⇒ 架构上已自然满足） |
| 3.3 | **沿单元的正/反向返程走法**（替代现在的自由寻路 `RETURN`） | 3.2 | `§6.1`：这是本次改动里**唯一必须新增**的机制，其余都是重组现有件 |
| 3.4 | ⏳ 才谈 `D-446` 回退 / `1.4s` 挂起 | 3.2 + 3.3 | `§4.1`：`1.4s` 是**预防式**且自带难裁例外（须豁免鱼骨自己挖穿通道），而记录图是**治疗式**、只需事后一次比对 |
| 3.5 | ⛔ **不做**：第二刀「脱离施工 → 挖簇 → 回归」 | 3.1–3.4 | `§7.1`（第一刀拿到收益且 `R1`/`R2`/`R3` **全不触发**；第二刀必须逐条有判据与红臂） |
| 3.6 | ⭐ **收尾的正确形态 = 收在「单元/支巷边界」**（替代"每挖一格一趟"） | 3.2 记录图（"这一单元收过了没有"要有记录） | `1.4y` 的量测判决：逐格收集占 **25% + 8%**，但**不能**改成"按年龄批量"（夹具实测红：收尾 COLLECT 挖地形 ⇒ `C5` 白名单外 4 条 + `C1` 模板格被填）。判据 = 真机 `collects` 趟数降一个量级 **且** `outside=0` 不变、`unrepaired=0`。⚠️ 同时裁定"逐格收集**能不能改世界**"（`allowWorldModification=true` 是真机 18 格 `collect-drops:PATH_ACCESS` 的来源，且与 `D-375` 的口径相抵） |
| — | ⚠️ 记录图落地的两条硬要求：`SUMMARY` 必须报**残留**（`unrepaired=<n>` 等价字段，失败终态时记录也丢）+ 读数**不许误导** | — | `§5` / `§9-P`；把 `1.4u` 的教训写进判据 |

**第 4 段 · 架构债**（并行推进，⚠️ **不许冒充鱼骨进度**）—— `survey/35 §12-A/B` + `§9` 桶1/桶3

| 序 | 事项 | 依据 |
|---|---|---|
| 4.1 | ⭐ **站位选优的架构取舍**：**留 A、把 B 交出去**（`K2` `GoalAdjacent` = 解开**追簇复杂度爆炸**的钥匙） | `§8.7` / `§12-A`；⚠️ `§9.4` 诚实边界：**本次改动对鱼骨完成度无可测提升**（真正挡住鱼骨的是桶2-1/2/3/4） |
| 4.2 | **退化顺序**改成「② 目标级一次搜索（把 `planEnterTarget` 提上来 + 补 `GoalAdjacent`）→ ③ 站位枚举 A/B」，**单向不可逆**（防 A→B→A 震荡） | `§7.5`；⚠️ 配套代价：Goal 化后"允许破坏/放置"授权须从规划器改挂到目标（`D-076`），**不是免费的** |
| 4.3 | 三处"已有简单解法"：`generateCandidates` **加短路** / `StandingPointEvaluator` **合并（删代码）** / `MiningPlan.isExecutable()` **删或变真门** | 桶3-8 / 3-9 / 3-10 |
| 4.4 | 桶1 的两条真缺陷：⭐ **模式 B 没有 LOS 门**（`selectBestApproach:455-460` 只挑成本最小、**LOS 只存不算**；`isExecutable()` 全仓只被日志用）/ B **截断后结论可信度**（直线排序 + 6 次上限） | `§8.3` / 桶1-2 / 桶1-3 |
| 4.5 | 顺手项：`1.4n` yaw 合并（`grep "atan2(-dx, dz)"` 现返回 **12**）/ `1.4o` 伐木场景顺序（**不改生产**，测试固定"先砍高树"）/ `1.4i` 片 B（归因四分类 + `abandonSpur` 升档 + 矿上限"有界延后"） | 桶3-6 / 3-7 / `1.4i` |

**第 5 段 · 阶段一（`survey/39`）**（**必须先拍收口标准 + 三条硬前置**）

| 序 | 事项 | 依据 |
|---|---|---|
| 5.1 | **共用件七组**：⭐ **锚点框架**（固定/跟随两态 + 半径 + 脱离返回；❌ 无载体）· **产物存放**（❌ 现状"背包满就收工"）· 掉落物管理**提成通用服务** · 范围内扫描**三份合一** | `39 §5`（= `D-449` 收口条件 ①，**唯一能单独卡住收口的**） |
| 5.2 | **三条硬前置**：**实体感知通道**（❌ 零 —— `perception/PerceptionCategory` 四个枚举**全是方块类**；挡**击杀/养殖/钓鱼**三种逻辑；⏳ **是否做通用地基待拍**）· **农业口径**（⚠️ 与 `D-449 §②`「农业不在产品线」**冲突 ⇒ 必须裁**）· **击杀归属**（挂账未裁） | `39 §6` |
| 5.3 | **7 逻辑 × 范围框架**（挖掘 · 砍树 · 种植/收获 · 击杀 · 养殖 · 钓鱼 · 存储交互 × 区域/锚点(固定·跟随)/鱼骨）+ 两条范围不变量 | `39 §4`；⚠️ 待确认：清单里**没有**区域挖掘/区域击杀/区域钓鱼/锚点存储交互（故意不列还是漏了？） |
| 5.4 | **产出契约**扩展（`JobKindContract` 加"产物"栏 —— 排产需要它、验收需要 `successCriterion`）+ `L13` 自我知识（= `D-419 §四` 明写"未做"的那半句） | `39 §8` / `§9` |
...
| 1.4j | ⭐ **片 C：`P1` 判据唯一化 + `I5` 通道保护**（`D-443` 裁定 5/8） | A1 类"半成品格"不再出现（`-65,52,182` 那种"空气+头位实心+无地板"）；本作业自己的追簇/捡东西**不再挖穿通道必需格** | ⏳ **待开工**。内容：① "这格算不算通"**唯一一份判据**（脚位可穿 ∧ 头位可穿 ∧ 地板在），`FishboneJob:527 bodyPassable` 与 `:647 canWalkThrough` 两处合一（`D-443` #5：判据就地放 `job/fishbone/`，接口/包等第二个消费者）；② `I5` = **作业形状驱动**的通道必需格集合（行走层 + 净空 + 支撑格），在**已有的候选过滤形态**处消费（对照 `LumberCandidateSource:71`/`MineCandidateSource:449`），共享任务按作业作用域取用（`D-443` #8，**不挂 `ZoneAuthority`**）。判据 = 把本轮破坏序列（`break -66,51,182` / `-65,51,182` / `-65,52,182`）造成场景 ⇒ 绿；**红臂** = 退回 `bodyPassable` ⇒ 红 · 删保护 ⇒ 破坏序列复现 ⇒ 红。⚠️ **2026-09-25 真机第三轮把①的理由钉死了**（取证见 `1.4k` / `docs/reviews/2026-09-25-真机第三轮-根因取证.md` §2）：`bodyPassable` 问单格 ⇒ 3/3 弃巷。⭐⭐ **2026-09-26 真机第五轮又给了 4/4 的证据，而且症状比"弃巷"更狠**（取证见 `docs/reviews/archive/2026-09-26-真机第五轮-自检报告-空气与通道成品规格.md`）：4 次启动**每次都死在单元的 `cell=2/2`（头位格）**，探针 `footPassable=true`（= 在**挖空气**）⇒ `no_valid_standing_point` ⇒ 主巷 ⇒ **整作业失败**（不是弃巷）。两个下游症状：① 用户报「**要起点旁边有一堵墙才能启动**」= 「前方第一格必须是实心」，因为头位格实际被问成「头位 + 天花板」而天花板恒实心 ⇒ 头位格**永远不被判成已通**；② 用户报「主巷有一格空的就放弃」= 那一格**已经是空气、本不需要挖**。⇒ ✅ **① 已落地（2026-09-26，`D-447` / `F1`）**：`isAlreadyPassable` = **单格** `canWalkThrough(cell)`（判据提升为 `public static` 供夹具复用同一处出处，先例 `advanceRefusalIsHard`），并**同时钉死 `I3` 口径**（半成品格「脚位空 + 头位实」⇒ 脚位跳过、**头位格照常挖**）。证据：新电池步 `corridor_skip_predicate` 绿臂 `checks=9 failures=0`（`FRESH new=[false/false]` 反证照常挖 · `DUG new=[true/true] old=[true/false] probe=[true/true]` = **真机签名逐字复现** · `HALF new=[true/false]` = `I3`）；**红臂**（退回 `bodyPassable`）`failures=2`，正好是 `DUG 必须两格都跳过` 与 `HALF 必须挖头位`；集成 `fishbone_slice1=42/0` · `fishbone_slice2=88/0`（计数一字未变）· **CORE PASS（246 s）**。⚠️ 原判据形态保留在 `D-447` 里供追溯。**仍待开工的是本条 ②**（`I5` 破坏面，见 `1.4s`）。⚠️ 测量陷阱一条：夹具首版把 bot 传送进实心石（`ORIGIN`=FRESH 脚位石）⇒ 维生监测 `SUFFOCATING` 打断电池（`no_verdict exit=3`）⇒ 夹具自己的前提不许被自己破坏（先建盒 → 凿站位 → 最后传送）。并**同时钉死 `I3` 口径**（半成品格「脚位空 + 头位实」必须**继续挖头位**，不许被跳过）。⚠️ 实测口径：`single:fishbone_slice1/2` 必须保持 42/0 · 88/0；回归判据**必须跑 `core`** | ✅ 0.5 轮（已落）|
| 1.4t | ⭐ **通道的「成品规格」（`I1` 连通 / `I2` 地板存在 / `I3` 不留半成品格）**没有任何一层负责 | 主巷的"空位"要么被**补上**、要么被**如实放弃并归因**，不许落成一次"不可能完成的挖掘" | ✅ **已落地（2026-09-26，`D-454`）** —— ⭐ **落地形态与 `D-450` 的字面不同**：实测证明「补」**今天已经存在**（`finishUnit` 的 `workCellNotStandable` 走位 + `PLACE_STEP_AND_TRAVERSE` ⇒ 走的过程中把缺的地板补上，且 requester 前缀 `fishbone` ⇒ 本来就在同一份额度里）⇒ 只落地**归因**与**额度分项**；明细 + 红绿证据见 **`1.4z-e`**。额度与 `C8` = **同一份**（已拍，且由 `1.4z-e` 的臂⑦实测钉住）。以下为裁定原文与架构事实（保留供追溯）：架构事实（`docs/reviews/archive/2026-09-26-真机第五轮-自检报告-空气与通道成品规格.md` §3.4）：① **形状是硬的** —— `FishboneTemplate` 纯几何（本轮 `units=404 cells=808`）；② **怎么到那一格完全交给内核** —— 逐格 `MineTask` + `MiningPlanner`（模式 A/B/进入）+ `StandingPointSelector`（本轮探针 `profile=standableOnly approach=PLACEMENT_ALLOWED`）；③ **但作业只下发「挖掉这一格」，从不下发「补齐这一格」**（全仓无"补通道格"的调用点）⇒ 空位既不被补也不被跳。本轮真机实证：`belowSolid=false`（`(-75,70,233)` 缺一格地板）⇒ `faceStandable=0/6` ⇒ 落 `no_reachable_standing_point`（**这个码把"地板没了"伪装成"站位找不到"**）。两条路需**一次裁定**：**(a) 补**（`I2`/`P0.5`，额度沿用 `C8` 的 `max(16,单元数/10)`，用尽 ⇒ 如实放弃）或 **(b) 如实放弃**（前瞻窗口判 `big_cavern_ahead`/`bridge_budget_exhausted`）。判据：造"主巷地板缺 1 格"几何 ⇒ (a) 补上且 `advance` 前进 或 (b) 归因码正确；**红臂 = 两个都不做**（= 今天的行为）| 0.5 轮 |
| 1.4u | **`SUMMARY` 的 `spurs=` 标签误导**（读数，零风险） | 汇总行不再让人误读成"子巷挖完了" | ✅ **已落地（2026-09-26）**（`F3`）。`FishboneJob.emitSummary`（`:1318`，实参在 `:1342`）打印的是 `spurBranches()-spursAbandoned / spurBranches()`，即 **未放弃数/总数**；本轮真机实测 `main=1/20` 与 `spurs=12/12` **并排出现**（主巷只走了 1 格、子巷一条都没开挖）⇒ 极易误读。建议改成 `spursAbandoned=0/12`（含义唯一）。⭐ **落地**：`emitSummary` 的 `spurs={}/{}` 与紧挨着的重复字段 `abandoned=` **并成一个** `spursAbandoned={}/{}`（分子 = 放弃数）。✅ **键名 = 与用户的契约，改一边必红**：契约门禁 `tools/kernel-predicates.py` 的 `rule_fishbone_live_log_shape` **实测先红**（收尾行键表里钉着 `spurs=`/`abandoned=`）⇒ 同步改成 `spursAbandoned=` 后绿；另同步计划 `§8` 的取样说明 + `FishboneJobItem` 的"你要看什么"。⚠️ 真机读数的**形状变了**（旧键名 grep 会得 0 行，而 0 是歧义的）⇒ 第 1 段的测试卡要用新键名 | 0.1 轮 |
| 1.4v | ⭐⭐ **Baritone 功能面缺口清单**（用户 2026-09-26：「做一次梳理，到底还有哪些 Baritone 功能我们没实现」） | 手里有一份**可复核的**"Baritone 有、Alice 无"全集，避免每次凭记忆列 | ✅ **已落盘**：`docs/reviews/2026-09-26-Baritone功能面缺口清单.md`。**规模**：Baritone `src/main` 155 + `src/api` 161 = 316 文件（两棵树：`reference/baritone/` = MC 1.21.4 · `reference/baritone-1.20.1/` = 1.20.1，**引用以后者为准**）；Alice `src/main/java` 510 文件。**方法**：8 区域并行只读审计（`process`/`movement`/`command`/`behavior`/`cache_selection`/`schematic`/`utils`/`api_root_settings`），每区域逐类枚举 Baritone 侧 + 在 Alice 侧 grep 对应物；**每条"缺失/部分"都写明搜过的词与 0 命中**（可逐条复核），**"刻意不做"必须给 `D-编号`**。**产物** = **158 条功能点**：已实现 17 / 部分 69 / 刻意不做 14 / **缺失 58**；原始区域产物在 `run/baritone-gap/*.md`（`run/` 已 gitignore）+ 合并器 `run/baritone-gap/merge.py`。**质量加固**：跨区域同题 7 个文件已标注（§0 各计一次、§1 已去重）· `utils` 与 `schematic` 重复 1 条已折叠 · 30 条带行号引用在 1.20.1 树上**复核 0 问题** · 抽查 9 条头条声明（7 条字面 0 命中、2 条差异查清后仍成立，需收紧范围措辞）。⭐ **§1 只列影响产品线的 **24** 条**（K1–K9 内核 / M1–M5 挖矿 / L1–L2 伐木 / C1–C2 合成传输 / O1–O5 命令面 / V1 观测），**§2 是"明确不做/不在产品线"清单**（`D-编号` + 理由，防止"为对齐而实现"）。**§1.7 建议排期**：K1 重力方块族（**同时是"局部扫描自动复位"方案的主要敌人**）→ M5（`1.4r` 真缺陷）→ K2 `GoalAdjacent`（一条改动松开挖矿站位/伐木站位/施工器三处手搓）→ K6 `PathEvent` + V1 `PathRenderer`（路径中途的观测前提）。⚠️ **同时发现三条文档/数字问题**：① `D-418` §三 断言⑤ 正文"今天 Alice **没有配置面**"**已过期**（门禁实跑 = `配置面 = 存在 12 处（已断言不含底线名）`）⇒ **已在 `AI_DECISIONS.md` 就地更正**（保留原口径 + 注明以门禁实跑为准）；② `api_root_settings` 提醒：`cache_selection` 区域 9 条 `缺失` 多是**同一个已裁定延后项**（`D-044`⑤/`D-067`⑧ 异步快照层）的分身 ⇒ **不许算成 9 个待办**；③ `mine_reach_probe`（EXTRA）在**新世界**里必然 `start_standable=false / realSearches=0/17` ⇒ 它应改成 `CheckStep.skippable`（无真机地形时 **SKIP 而非 FAIL**，`HANDOVER.md:123` 早有"结论不作数"的登记）—— 0.1 轮，未做 | 1 轮（已落）|
| 1.4w | ⭐ **归因口径通用化：`no_valid_standing_point` 不再被改写成 `no_reachable_standing_point`**（`survey/35 §9` 桶3-3「**最省力收益最大的一刀**」） | 「做不了」与「不让做」的读数**分得开**（`survey/35 §5`：现在两级丢失，长得一样） | ✅ **已落地（2026-09-26）**（排期 = `J-★` 第 2 段 2.2）。事实：`MiningPlanner.planDirect` 的真实理由 `no_valid_standing_point`（`:255`，垃圾桶码）被 `:190` **一律改写**成 `no_reachable_standing_point`（只放行 `SEARCH_INCOMPLETE`）⇒ 上游永远看不到"到底为什么站不上去"。⭐ **不用发明**：唯一做对的先例 = `D-329`（`MineJob:160/167`：`40` tick 冷却 + 连 `8` 次认账；`:414-423` 冷却期刻意不工作）⇒ 把那个口径搬过来即可。判据 = 造"遮挡导致无站位"几何 ⇒ 读数里能看见 `no_valid_standing_point`（而不是被总括码吃掉）；**红臂** = 退回改写 ⇒ 红。⚠️ 它是 `1.4i`（归因四分类）的**前置**。⭐ **落地（比"搬 `D-329`"更彻底）**：`standableOnly` 早返回改成「**腿给出了理由就原样上抛**，只有腿什么都没说时才用总括码兜底」（`directReason != null && !directReason.isEmpty() ⇒ return direct`）⇒ `search_incomplete` 与 `no_valid_standing_point` **同一条规则**保护，不再是一串例外。⚠️ **`D-329` 门禁实测先红**（它原来钉死的是字面子串 `SEARCH_INCOMPLETE.equals(direct.failureReason())` ⇒ 把"只放行一种码"固化成了合格形状）⇒ 已把该规则升级为**更强的两条**：① 必须 `return direct;`（原样上抛）② 兜底码必须受 `.isEmpty()` 保护；**两条臂实测都红**（只放行 SEARCH_INCOMPLETE ⇒ 红"兜底码不受保护"；无条件改写 ⇒ 红"不再原样上抛"）。电池：`single:fishbone_slice1` PASS（398 tick · 42 判据）· `single:fishbone_slice2` PASS（2235 tick）| 0.3 轮 |
| 1.4x | ⭐ **沙砾/重力方块的「挖掘期暂停」判据同步**（`survey/35 §9` 桶3-4；= 本会话新清单 `K1` 的**前半**） | 挖通通道后**沙砾不会在 2 tick 后把刚挖的格填回**；代价模型**先不动** | ✅ **已落地（2026-09-26）**（排期 = `J-★` 第 2 段 2.3）。事实：`pathing/` 内 `FallingBlockEntity`/`avoidBreaking`/`includeFalling` **全 0 命中**（6 处命中全在 `road/RoadBuilder:136,163` + `task/RoadBuildTask:234,256` —— **造路模块已有先例**）；Alice 只移植了 `MovementAscend` 那一条（`AscendExecutionFactory:65-70`）。修法 = **照 Baritone `Movement.java:157-160`**（目标格里有在飞的 `FallingBlockEntity` ⇒ **暂停挖掘**），≈20 行，**不碰成本模型**（成本乘子 `includeFalling` 按 `survey/35 §9` 桶3-5 = **可延后**，见 `J-★` 2.5）。⚠️ 测量陷阱：修复后必须**等一个重力 tick 再复检**（`FallingBlock` 落体延迟 2 tick）—— 立刻复检会读到"已修好"的**假绿**。⚠️ 与 `F1` 同族：`F1` 是**空间**维度判据错位、沙砾是**时间**维度判据失同步。⭐ **落地**：`MineBlockRunner.tickBreak()` 开头加暂停闸（**不 tick `breakSession`** ⇒ 不推进度、不吃那 60 s 预算、不重复扣账；**返回 `MINING` 而不是失败**）+ 只读出口 `pausedForFallingBlock()`（与 `mineInPlace()`/`supportSkipped()` 同族）；判据盒子逐字用 Baritone 那个 `1.1` 放宽。⭐ **新电池步 `mine_falling_pause`（MAIN ⇒ 进 CORE）四臂**：暂停（**直接证据** = 走了暂停分支 + 窗口内一格没挖动）/ 恢复（清掉落体后必须**挖穿** ⇒ 暂停不许永久）/ 两个**负对照**（落体在目标**上方 2 格**、**侧向 2 格** ⇒ 不许暂停）。⚠️ **夹具实测踩到的两处硬事实**（`javap -c` 读官方映射字节码）：① `FallingBlockEntity(Level,double,double,double,BlockState)` 是 **private** ⇒ 只能用 `fall(...)`；② `fall(...)` 会把该格写成"流体遗留方块" ⇒ 必须立刻还原，且实体**载空气状态**（`tick()` 首分支 `blockState.isAir() ⇒ discard()`）⇒ 它一 tick 静默消失、**落地成块污染场景的机会为 0**。⚠️ **诚实边界**：夹具把落体**钉**在格子里 ⇒ 它验的是**判据本身**，**不是**原版"沙砾 2 tick 才落"的时间线 —— 那条仍要真机观察（本行那条测量陷阱）。⚠️ **另一条边界（Baritone 同款）**：暂停**没有自己的超时**（Baritone 也没有）—— 若某个落体**永久**赖在目标格里，本格会一直让路，直到**作业级**的 `maxTicks` 收工。今天不接受"为了防呆而给暂停加时限"：时限会变成新的假绿来源（到点就放弃 = 又把沙砾填回的坑放回来）。 | 0.3 轮 |
| 1.4z-e | ⭐⭐ **`F2` 落地（`D-454`）：地板缺格的「归因」+ 额度分项**（`D-450` 的落地形态按实测改写） | 真机那个把「地板没了」伪装成「站位找不到」的码消失；搭路额度「用尽」能看出是谁吃掉的 | ✅ **落地（2026-09-26）**。**取证先于动刀**（三条新夹具臂，`fishbone_slice2`）：① ⭐ `D-450` 的「补」**今天已经存在** —— `finishUnit()` 的 `workCellNotStandable`（`D-440`）让作业走进刚挖完的那一格，那次走位用 `PathRequest.withPlacement`（`PLACE_STEP_AND_TRAVERSE`）⇒ 逐字 `[WRITE] place 3761, 79, 2400 minecraft:cobblestone by=fishbone-chase:attempt0:STEP_PLACEMENT`，requester 前缀 `fishbone` ⇒ **本来就在同一份额度里** ⇒ 第一版实现（`Phase.CHANNEL_FLOOR` + `PlaceTask` 主动补格）**整体回退**（同一件事的第二个实现 = 反补丁红线）；② ⭐ 真机 `places=0` 的机理链 = **缺地板 ⇒ 挖出来的方块掉进洞里 ⇒ 拿不到一次性方块 ⇒ 补不了 ⇒ 站位候选恒空** （旁证：想用「洞挖深」造「没原料」造不出来 —— 原版拾取 AABB 外扩 1 格 / 垂直 0.5，而挖掘站位必然贴着目标 ⇒ 头位格掉落物在 10 tick 拾取延迟里正好落进那只扩充盒；夹具改用**干草盒**（掉小麦 ⇒ `findPlaceableSlot` 选不到）才造出来）；③ **落地两条**：(i) `FishboneJob.standingFailureCode` —— 仅当「站位类失败 ∧ 该单元支撑格真的缺」同时成立 ⇒ 主因码 `channel_floor_missing:<cause>`，cause 事实优先（`bridge_budget_exhausted` / `no_throwaway_blocks` / 原样上抛），判据出处 = 新 `MiningPlanner.isStandingPointRefusal` + `FishboneJob.channelFloorMissing`；(ii) `D-450` 推论 = `SUMMARY` 增 `places=<搭桥>/<补地板>`（按**位置**分，复用 `FishboneTemplate.units()`）+ `unrepaired=`；④ ⭐ **顺带修一条既有缺陷**（`silent-measurement-failure` 类）：`placementsUsed()` 原先**不按作业时间窗过滤**，而 `WriteAudit` 是**进程级**环形缓冲（512）⇒ 电池同进程连跑 8 臂时**上一条臂的放置会吃进本次额度** ⇒ 改成按 `gameTimeAtStart` 过滤（与 `emitSummary` 同一口径）。**证据**（全部 `ALICE_BATTERY_NO_CACHE=1`）：`fishbone_slice2 PASS(140s)` **8 臂 / `checks=131 failures=0`**，三臂读数逐字 —— `FILLED channelFloorRepairs=5 unrepaired=0 逐格可站=5/5 终态=template_complete` · `BUDGET channelFloorRepairs=16（= max(16,40/10) ⇒ **补地板与搭桥共用同一份额度**）unrepaired=0 仍缺=4/20 终态=main_unreachable:bridge_budget_exhausted`（⭐ 顺带证明：额度用尽是在**单元开头的推进门**停的，码本来就是计数事实 ⇒ 这一支不需要地板归因）· `NO_MATERIAL channelFloorRepairs=0 unrepaired=1 仍缺=5/5 终态=main_unreachable:channel_floor_missing:no_throwaway_blocks`。**两条红臂**：R1 撤掉归因映射 ⇒ **只有** `NO_MATERIAL` 臂红（3 条），终态退回 `main_unreachable:no_valid_standing_point`（真机那个伪装形态），其余 7 臂仍绿；R2 拿掉 `finishUnit` 的 `workCellNotStandable` ⇒ `FILLED` 红 5 条 + `BUDGET` 红 3 条 + `ORE_VEIN_CHASE` 红 1 条（= `D-440` 的已知依赖）。回归：`fishbone_slice1` · `core` · `check-all`。⚠️ **仍未做**：真机（`WINDOWS_CLIENT`）· 若出现「走位补路补不到但作业仍需推进」⇒ 回 `D-450` 字面形态（作业主动下发补格） | 1 轮 |
| 1.4z-d-fixture | ⭐⭐ **(d) 的专属夹具落地**：`fishbone_slice2` 新增臂 `SALVAGE_ON_FAIL` | 钉死「**失败路径上 `collected` 必须已结算 + 地上不许滞留产物**」；并**逼出 (d) 的第二半**（见下） | ✅ **落地（2026-09-26）**。**臂设计**：复用 `slice2` 的**默认（支巷）模板** + 一盏顶棚露头矿（`SALVAGE_ORE_UNITS={1}`） + **把作业预算掐到 160 tick**（实测：矿在 **+20** tick 被挖掉、它那趟周期收集 **+40** 就完成、整模板 **~240** 才跑完 ⇒ 160 必然以 `goal_timeout` 失败**且**已收到产物）。断言四条：① 前提"作业**真的失败**"（`goal_timeout`）② 前提"真的挖到过产物"（`oreMined ≥ 1`）③ ⭐⭐ `collectedProducts ≥ oreMined`（**已结算且对得上**）④ ⭐⭐ **地上没有剩余产物落物**（覆盖"追簇结束的周期收集"与"失败前的打捞"两条路）。**夹具立刻教了我两件事（都改了生产代码）**：**(i) `collected=0` 的真因不是"没打捞"，是"一次收集都没跑过"** —— 产物在 +40 就被周期收集收走 ⇒ 地上没有产物 ⇒ `hasProductDropOnGround()` 为假 ⇒ 打捞根本不触发 ⇒ 若只在收集档里结算，`collectedProducts` **永远停在 0**。⇒ 修法：`beginReturn()`（失败与成功的唯一入口）**无条件结算读数**（背包增量是绝对值 ⇒ 幂等）。**(ii)** 本臂**故意中途失败** ⇒ 共享判据 `C1`「场景与期望表逐格一致」对它**不适用**（模板必然没挖完），已按臂豁免并写明理由（本臂改考"失败路径的收尾行为"）。**证据**（全部 `ALICE_BATTERY_NO_CACHE=1`）：`fishbone_slice2 PASS(100s)` —— 打捞臂读数 `collected=1 ores=1 地上剩余产物=0 终态=goal_timeout`、臂 `PASS（本臂新增失败 0 条）`；**红臂**（撤掉 `beginReturn` 里的无条件结算）⇒ **精确红在那一条**（`collected=0 ≥ ores=1`，本臂新增失败 **1** 条，其余全绿）· `core PASS`。⚠️ **仍未钉住**：「打捞**确实被触发过**」——在 B 之后产物通常在追簇结束的周期收集里就收走了 ⇒ 打捞是**兜底**，要确定性触发需要一个"周期收集收不到、打捞能收到"的场景（与 `(b)` 水域终态 / `(c)` 维生接管一起考虑）。 | 0.5 轮 |
| 1.4z-a/d | ⭐⭐ **挖到水 ⇒ 硬拒（a）** + **回家前打捞产物 / 结算读数（d）** | (a) 直接消灭「挖穿 → 瀑布灌进通道 → 被冲离站位 → 整轮作业作废」；(d) 失败路径不再把产物留在地上、`collected` 不再是「没算过」 | ✅ **落地（2026-09-26，真机驱动）**。**真机那一幕（逐字）**：`[Fishbone] no_progress 连续 401 tick 没推进（unit=39/404 cell=0 mined=47）⇒ 如实失败` · `SUMMARY main=3/20 spursAbandoned=0/12 mined=47 ores=32/32 uncollected=0 collected=0/32 collects=8 worldChangesInside=79 outside=0 ticks=1920 → FAIL`。**为什么终态是 `no_progress` 而不是「水」**：`PathRetry` 反复 `plan status=REACHED cost=7.01 … goal=-154,45,257` → 立刻 `replan reason=STALE code=DESCEND_STALE_START / ASCEND_STALE_START`（**水流把 bot 推离站位 ⇒ 计划一起步就作废**）⇒ 401 tick 里 `unit/cell/mined/skipped/oreMined` 一个都不涨 ⇒ 通用停滞后卫开火。⚠️ 维生层**看见了但按设计不管**：`维生监测 hazard=WATER_CONTACT duration=220` + `[SurvProbe] verdict=IGNORE escapeWrites=true`（`air` 掉到 **2**、`eyeInWater=true` = 曾经没顶）—— 台账早登记过这一档是「**溺水/着火只报不拦**」。**（a）改动**：接进**既有唯一判据** `FluidRiskPolicy.miningRefusal`（岩浆那条已在用，`MiningPlanner:170` 已在调，硬拒绝码已被 `MineTask.isHardTargetRefusal` 认）⇒ 加 `fluid_risk_water`：**只拦「会把水引进来的那一格」**（目标本身当前不是流体 · **上方**是水 · **水平四邻**任一格是水；⚠️ **不含 `below`**（水不会往上流）；**已经泡在水里的区域不拦** —— 否则水下什么都挖不了）。鱼骨的两档处置**不用改**（支巷 ⇒ `abandonSpur`、主巷 ⇒ `main_blocked:` 如实失败，既有代码已按硬拒绝分流）。**（d）改动**：`beginReturn()`（**失败与成功唯一入口**）先看「作用域里还有没有**产物**落物」⇒ 有就先跑一趟 `CollectKind.SALVAGE` 打捞（产物清单 + `worldMod=false`，只做一次 `salvageDone`），**且三档收集都结算 `collectedProducts`** —— 改前只有收尾档结算，于是失败返航时打的 `collected=0/N` 是「**没算过**」不是「一件没捡」。**证据**（全部 `ALICE_BATTERY_NO_CACHE=1`）：新增电池步 **`fluid_mine`**（MAIN ⇒ 进 CORE，同时把此前**只挂游戏内物品、没进电池**的流体夹具接进回归）—— 四臂：A 岩浆（plan+run 硬拒）· **C 水（上方）** · **D 水（水平邻格）** （C/D 都 `status=FAILED reason=fluid_risk_water cleared=0 gained=0`）· B 正对照 `DONE`（**不误伤正常挖掘**）；`fluid_mine PASS(31s)` · **红臂**（`FluidRiskPolicy` 里把水的两条判据短路）⇒ **精确红在水两档**（`C/D ⇒ 缺陷`、`plan_water_side=PLANNED(未拒绝!)`），**岩浆臂与正对照保持绿** · `core PASS`。⚠️ **未做**：(d) 的**专属夹具**（需要「鱼骨作业失败 + 地上仍有产物」的场景，本轮只有编译 + 无回归；夹具缺口已记在 `J-★`）；水域「站位把不住」是否要给 `PathRetry` 专门终态（`D-450` 归因码那一批）；维生层在水里/溺水时是否接管（需用户裁定的破例）。 | 0.5 轮 |
| 1.4y-B/C | ⭐⭐ **批量收集（B）+ 同格不重磨（C）落地**：收集从「每挖一格一趟」改成「**一段活干完收一次**」 | 把真机「挖一格就跑好几格去等又回来」的**结构**去掉（A 只砍掉了石头那一半） | ✅ **落地（2026-09-26，承 `1.4y` 的账）**。**B**：① 逐格收集关掉（`FishboneJob` 两处 `MiningBudget.forTarget(..., false)`；只跳过「拾取」段，`enterRestoreOrDone()` 的支撑块建拆照旧）；② **批量收集挂在三处结构边界**（`requestBatchCollect`：`spur_done` 支巷完工 · `chase_done`/`vein_done` 追簇队列清空），真正切相位只在 `tick()` 里**唯一一处** `batchCollectDue()`（守卫：`current`/`oreTask`/`collector` 全空 ⇒ 不打断半截挖掘；只在 `EXCAVATE`/`SPUR_RETURN` 切）；③ 两档收集都 **`allowWorldModification=false`**（`D-375`：够不着如实退休，不挖地形）+ 产物清单（`D-453`）；④ 周期收集收完**回工地**（`collectPhase()` 分流），`product_not_collected` 判定**只在收尾那次**做；⑤ `SUMMARY` 新增 `collects=`（周期收集趟数）；⑥ 安全下限 = 最老**产物**落物年龄 ≥ `COLLECT_BEFORE_DESPAWN_TICKS`（`3600` = 原版 `6000` − 余量 `2400`，**派生**，见该常量注释）。**C**：`CollectDropsTask` 的 case 2（「模型说已在拾取范围、等满 `PICKUP_WAIT_TICKS` 却没进包」）**漏了** 3-b/D2 的 `excludeFailedGoal()` —— 真机逐字 7.4 s 白跑簇就是**同一格等两轮**（`reanchor cluster_anchor=-144,50,242 reanchors=1/2` ×2 → `retire pickup_timeout` `delta=0 ticks=148`）⇒ 补上同一处置（换次优格，或如实退休）。**证据**（全部 `ALICE_BATTERY_NO_CACHE=1`）：`fishbone_slice2 PASS(87s)` —— ⭐ **同一个「关掉逐格收集」上次单独上是红的**（`C5` 白名单外 4 条 + `C1` 模板格被填），这次**绿**，差别就是 `worldMod=false` + 产物清单；实测 `collects=2`（`why=spur_done` 两次）· `COLLECT 完成（周期 #N）⇒ 回 SPUR_RETURN` · `outside=0` · `collect_skipped=90` · `fishbone_slice1 PASS(40s)`（无支巷 ⇒ `collects=0`，只收尾，正确）· `collect_offcenter_retry PASS(36s)`（C 没破坏「到位却够不到 ⇒ 换格」那条）· `collect_slot_approach PASS(44s)`（`D-375` 不挖地形）· `pickup_gate PASS(37s)` · `core PASS`。⚠️ **代价（诚实）**：单趟收集预算仍是 `DEFAULT_TOTAL_BUDGET_TICKS=600`（30 s）⇒ 落物多时一趟收不完，剩下的等**下一个边界**或收尾；`MineTask` 的 `collect_skipped` 会每格一行（真机日志噪声，未处理）。 | 0.5 轮 |
| 1.4z | ⭐⭐ **主动拾取清单（A 刀）落地**：石头族不再"值得专门跑一趟" | 把"追簇之后挖一格就跑好几格去等又回来"的量级砍掉（**先取证、再动刀**） | ✅ **落地（2026-09-26）**。**取证**（子巷 65 s 窗口，19:09:15–19:10:20）：**只挖掉 4 格**，却起了 **12 个收集簇（7 个 `delta=0` 白跑）**，收集占 **81%** 时间，每簇中位 **6.10 s**；逐字一例（7.4 s 白跑）= `簇起 anchor=-144,50,242` → `planned movements=6 from=-145,49,247` → 走 6 段（ASCEND+5×TRAVERSE）→ `reanchor ×2` → `retire pickup_timeout` → `cluster_done delta=0 ticks=148`。**根因** = 逐格收集的候选集是**全作用域**落物（`refreshCandidates` = `scope.liveDrops()` 全量），于是按"最近的落物"**逐个点名叫**（锚点散在 7 个格：`-142,50,242`/`-143,49,247`/`-143,50,242`/`-144,49,247`/`-144,50,242`/`-145,49,247`/`-147,49,247`）；而那批落物 **61% 是石头族** ⇒ 纯负担。**改动**（`D-453`）：`CollectDropsTask.activePickup`（`Predicate<ItemStack>`，`null`=全部 ⇒ 既有调用方逐字不变）+ `refreshCandidates` 过滤 + `MineTask` 透传 + `FishboneJob` 传 `PRODUCT_FILTER::matches`。**证据**：`pickup_gate` 新增三臂 —— ① 清单**外**（圆石）+注入清单 ⇒ `地上剩=1 / 背包增量=0 / **bot 一步没动**`；② 清单**内**（粗铁）⇒ `delta=1` 收得到；③ **负对照**（同一颗圆石、**不**注入清单）⇒ `delta=4` 被收走（⇒ 臂①的红不可能来自"场景本身收不到"）；夹具前提自证 `粗铁∈清单=true · 圆石∈清单=false`（走**生产过滤器**入口）。`pickup_gate PASS(38s)` · **红臂**（`refreshCandidates` 里短路过滤）⇒ **精确命中臂①**（`active_blocked_remaining=0 / moved=true`）· `fishbone_slice1 PASS(40s)` · `fishbone_slice2 PASS(99s)` · `core PASS`。⚠️ **夹具教训**：`liveDrops()` 只认**我方破坏产生**的落物（`itemOrigins` 非空）⇒ `addFreshEntity` 造的落物**必须换候选源**（`D-344` 接缝）+ 给采集授权，否则收集器第一 tick 就 `finish("done")`，验出**假结论**（第一版三条臂全 `collected=0/0 clusters=0`）。**遗留**：B（关掉逐格收集 + 改在**单元/支巷边界**收 + 批量收集 `allowWorldModification=false`）与C（不逐锚点硬磨）见 `J-★` **第 3 段 3.6** | 0.5 轮 |
| 1.4y | ⭐⭐ **追簇之后"挖一块就跑一趟、在空洞里发呆"的根因取证 + 两条方案的量测判决** | 把"收尾成本"从猜测变成**量出来的账**；并**证明** `(b)` 与 `(a)` 的朴素形态都不成立 | ✅ **取证完成（2026-09-26 真机 19:04:34–19:10:25 / 345 s / 6900 tick）**。**账**（从 `latest.log` 逐条聚合）：相位 = `EXCAVATE 55% · IN_*（追簇）42% · SPUR_RETURN 3%`；**拾取簇 199 趟 / 1746 tick = 25%**；为拾取走位 518 tick = **8%**；挖方块本体 1788 tick = 26%；**规划 100 次调用合计 24 ms = 0%**（不是规划慢）；`controller_stop_movement` 2103 tick。⭐ **识别层没问题**（`skipped=15` 正确跳过已通格 · 同一 `unit:cell` **零重复** · `spursAbandoned=0` · 零硬失败）⇒ 症状**不在识别层**，"是没能正常识别吗" = **否**。**⛔ 方案 b（退休状态跨格继承）被量测否掉**：那 28 个"重复退休"簇每个只花 `ticks=1`（合计 ≈28 tick = **0.4%**），且实测同一件物品第 5 次就被收走了（`entity_gone`）⇒ "永久写死"会把**本来够得着**的落物留在世界上、伤 `collected`。**⛔ 方案 a（按年龄批量收集）被夹具否掉**：按 `(a)` 实现（关掉逐格收集 + 用 `ItemEntity` despawn 常数派生的到期触发 + 周期/收尾两档 `COLLECT`）后 `fishbone_slice2` **红**：`C5 破坏 35 条里白名单外 4 条`（全是 `by=collect-drops:…:PATH_ACCESS` 在 `3764-3765,80-81,2397`）+ `C1 模板 24 格被填` ⇒ **逐格收集是承重的**：它是"**趁东西还在脚边就捡**"；推迟以后落物已落进缝里 ⇒ 收尾的 COLLECT 只能**挖地形**去够 ⇒ 等于拿 fishbone 的核心不变式（`outside=0`）换 25% 的时间，**不换**。⇒ **已回退**（`git checkout`；工作树干净、`fishbone_slice2` 复跑绿）。⭐⭐ **剩下的真 lever（待裁定）**：逐格收集现在**允许改世界**（`CollectDropsTask(..., allowWorldModification=true)`，两处：`MineTask.enterCollection():507` 与 `FishboneJob.startCollect():1186`）—— 真机那 **18 格 `collect-drops:PATH_ACCESS` + 10 次 `PICKUP_DETOUR` + 2 次 `SESSION_CANCELLED`（挖 4 格去够一件东西 → 中途取消 → 重来）** 就是它；而 `D-375` 自己写着「够不着就是够不着，如实退休比挖穿地形诚实得多」。⚠️ 它挂在**共享** `MineTask` 上（`MineJob`/`LumberJob`/`RegionLumberJob` 同路）⇒ 改它必须跑 6+ 步电池。⚠️ 顺带实测：`ItemEntity` despawn 常数 = **6000**（`javap -c` 实测 `sipush 6000`），而作业预算 `maxTicks` 默认 **86800** ⇒ 差 14 倍，"只在收尾收一次"**必然**丢产物。⭐ **`(a)` 的正确形态 = 收在「单元/支巷边界」**（那一刻 bot 还在落物附近，且走廊里的落物不会漂走）⇒ 归 `J-★` **第 3 段"沿单元返程"** | 0.5 轮（取证已花；落地待裁） |
...
| 1.4m | ⭐ **机制 A：下落/下行前先把身体挪到格中心**（`D-445` 裁定三的落地；用户「先把 A 改动做了我再测」） | 「向下挖却在洞沿上没掉下去」不再发生（真机形态：`onGround=true` + `forward=0.00` + 145 tick 超时） | ✅ **落地（2026-09-25 深夜）**：`DownwardExecution` 补上 `FallExecution:106-116` / `DescendExecution:107-116` **早就有**的那一支 —— 方块已破后：`onGround` 且 `水平离心 > 0.2` ⇒ `faceCellCenter` + `setForward(1.0)` 走过去，够近才原地等掉落，**已离地不给水平输入**。0.2 的依据 = `格半宽 0.5 − AABB 半宽 0.3` = Baritone `MovementDownward:90`。**唯一出处**：`MovementHelper.horizontalDistanceToCenter`/`faceCellCenter`（新增）+ `isSettledAtFootPos` 改成调同一个口径（判定与纠正同源）。**判据 = 运行时**（挂在已有的 `place_step_descend_clearance` 步上，`edgeCrawlContract`）：自建洞沿几何 + bot 摆在**离心 0.3** + 真 tick `DOWNWARD` 到终态 + 三条前提自证。实测：修复后 `SUCCEEDED（7 tick 落到 3000,-61,4040）checks=31/0`；**红臂**（改回只 `stopMovement()`）`TIMEOUT（离心=0.300 onGround=true input=forward=0.00）checks=31 failures=1` —— **把真机那两条 145 tick 的形态离线确定性复现**，还原后 sha 一致。⚠️ **未做**：机制 B（结算点瞬移校准，给拾取模型那 0.49 用）；真机轮未跑。⚠️ **同日深夜修正**：第一版按 Baritone 做成**无条件**（`onGround` + 离心 > 0.2）⇒ **CORE `lumber_job=FAIL`**（其余 40 步 PASS；`git stash` 掉改动后 CORE PASS ⇒ 确认回归）。根因：「离心」在 ALICE 是**正常状态**（`COLUMN` 容差不要求居中）⇒ 无条件给输入会改掉**所有**正常下落的落点；伐木步按 `reason=nearest` 选树，落点一动第三棵就从 @29 变成 @28（7 格云杉）⇒ 砍完高树后砍不动接下来的树 ⇒ `partial_quota`。⇒ **触发收窄为因果谓词** `MovementHelper.supportedByNeighbourCorner`（本列脚下已空 **且** AABB 按一次重力步 `0.0784` 往下探会与邻列脚下一层的实心方块真相交）⇒ 正常下落**零水平输入**。收窄后（全部 `--no-cache` 真跑）：rim `SUCCEEDED（7 tick）32/0` · 红臂 `TIMEOUT 32/1` · **CORE PASS（228 s）** · `fishbone_slice1/2 = 42/0 · 88/0`。⚠️ **两个测量陷阱**：① 单跑一步**不是** CORE 那一步的基线（入口状态不同）⇒ 回归判定必须跑 `core`；② 电池有**判决缓存** ⇒ 红臂必须 `ALICE_BATTERY_NO_CACHE=1` 且先确认日志时间戳是本轮的。 | 0.5 轮 |
| 1.4p | ⭐ **「bot 被自己搭的方块拦住」**（真机第四轮；截图 `2026-09-25_23.17.44.png`） | 作业不许把自己的通道格填掉；回程不再因为自己放的方块变成 `UNREACHABLE` | ✅ **已修（2026-09-25 深夜，`D-446` / 本行 → 见 `1.4q`）** —— 用户裁定「先治根因，让他不会在作业区放方块」⇒ 落地 `I5` 放置面。**原取证保留**（`docs/reviews/2026-09-25-真机第四轮-自己搭的方块拦住回程.md`）。链条（逐条锚点）：`collect-drops` 为够一个掉落物，用 `PILLAR/STEP_PLACEMENT` 把圆石放进了 **bot 自己刚挖通、且刚走过** 的通道脚位格 `(-65,54,181)`（= south9 unit 44）⇒ 【① 直接原因】；该格**无保护区**（`ProtectionZones.isWild`）⇒ 按 `D-398 R1/R2` **不记账、不恢复**（`[Ledger] skip … 区外 … 累计 15 次`）⇒ 那块圆石**永远不会被回收** ⇒ 【② 系统原因，改它得先改裁定】；随后 `SPUR_RETURN` 从 `(-66,56,181)` 回 `(-65,54,172)` **`UNREACHABLE`**（`descend_precondition=11`、`nodes=3`、`searchLimit=0`）⇒ `return_failed` ⇒ 作业 FAILED、bot 被留在原地 ⇒ 【③ **未定**：`descend_precondition` 是聚合计数，不打印是哪条子条件拒的 ⇒ 必须先上探针】。⭐ 好消息：本轮 `segment_stall=0`（机制 A 生效，上轮 2 次）。⭐ 另一条弃巷仍是旧账：`target=-65,54,163 footPassable=true`（= `1.4j①` 签名）。候选修法见文档 §4：**A=通道保护 `I5`（`1.4j②` 已裁定，本截图是它的真机证据）/ B=作业自清理自己放的方块 / C=需要填自己通道时干脆不爬 / D=先上探针把 ③ 定死（推荐先 D）** | 0.5 轮 |
| 1.4q | ⭐⭐ **`I5` 放置面：作业自己的通道层格不许被放方块**（`D-446`；真机第四轮根因的收口） | 作业不再能把自己那条路填掉；`SPUR_RETURN`/`RETURN` 不会因为**自己放的一格**变成 `UNREACHABLE` | ✅ **落地（2026-09-25 深夜）**。① **形状真源** = `FishboneTemplate.cellSet()`（= `C1`/`C5` 那份，**不另抄几何**），保护集合 = **通道层格**（脚位格 + 头位格）；**支撑格刻意不保护**（那是"补地板"的合法目标，`I2`）。② **闸门一处**：新 `BlockInteraction.placementRefusal`（与 `breakRefusal` 严格同形）⇒ **规划期剪枝**（`appendPillar`/`appendPlaceStepAndTraverse`，`D-106 Slice B` 纪律）+ **执行期最后一道闸门**（`placeAt`，排在触及检查之前）⇒ 新 `PlaceResult.CHANNEL_DENIED`、**不消耗物品与预算、世界未被改写**；计数 `place_channel_reserved`（`record` + `recordTotal`）。③ **载体沿用** `TaskTargetProtection`（`D-362` 的作用域：按 botId、任务立/撤、predicate、fail-closed）⇒ 新 `beginChannel` + `placementRefusalFor`，**不挂 `ZoneAuthority`**（2D 无 Y，`D-443` 裁定 8 已排除）。④ **安装点在 `FishboneJob.prepare()`**（不能在构造器，见 `1.4r`）。**判据**：新电池步 `channel_place_guard`（`ChannelPlaceGuardCheckTask`；自建 6×9×6 实心盒 = 构造保证孤立）。绿臂 `checks=23 failures=0`：A 组复现真机（填圆石 ⇒ `UNREACHABLE nodes=3 movements=[]`，与真机 `descend_precondition=11 nodes=3` 同形）、B 组（保住空气 ⇒ `REACHED movements=[FALL@脚位格, TRAVERSE@目标格]`，**零世界写入**）、C 组（脚位/头位=`channel_layer_reserved`；**支撑格/口袋=`null`** 反证不误伤；`placeAt=CHANNEL_DENIED` 且 `worldChanged=false`）、D 组（**负对照**：撤掉作用域 ⇒ `PILLAR` 落通道层格 `hits=1`；装上 ⇒ `hits=0`）、E 组（**用户的问题「他还能找到别的路上去吧」= 实测**）：负对照 = 撤掉作用域 ⇒ `ascent=REACHED hits=1 movements=[PILLAR@通道脚位格, ASCEND@口袋]`（与真机 `cost=6.67 movements=2` 同形）· 装上后 = **`SEARCH_LIMIT` hits=0** ——⚠️ **是 `SEARCH_LIMIT` 不是 `UNREACHABLE`**，按 `D-329` **不许读成「证明上不去」**：诚实读法 = **便宜的替代路（挖穿石头）规划器在预算内没找到 ⇒ 可达性未知**。⇒ **诚实的结论**：这个**封闭形状**里「I5 之前那条便宜上法」被拿掉了；要上去只剩「挖穿石头」这类贵路（离线未在预算内找到）。**代价**：需要垫脚才够得着的掉落物会被如实 `retire`（真机那轮那 3 件**本来就没捡到**：`retire stack=3`）⇒ 净收益 = 作业从 FAILED 变 PASS，而**实际拿到的东西一件不少**。⚠️ 真机（矿洞里空间更多）可能有别的路，**离线几何不能替它保证**；要不要为「上去」补一条路是独立取舍，待裁定。**红臂**（`placementRefusal ⇒ null`，`--no-cache`）：`failures=6 verdict=FAIL`，其中 `C:placeResult=PLACED worldChanged=true`（**真的把圆石写进了自己的通道格** —— 与真机同形）、D 组 `guarded hits=1`；还原后 sha 逐字一致。⚠️ **两个测量陷阱**（已写进 `D-446`）：① `FALL` 边要求手里**一次性方块 ≥ 落差**（`fallRecoverable`）⇒ 夹具首版给 1 个圆石 ⇒ 量到"没材料"而非"落点被堵"、**判据假红一次**；② 夹具**组间不许依赖副作用**（C 组真跑 `placeAt`，闸门失效时会真写进去 ⇒ 下一组从实心格出发、负对照退化成 0）⇒ D 组开头重新确立几何前提。 | 0.5 轮 |
| 1.4r | ⭐ **`D-362` 的任务保护作用域在生产上**确认**是空的**（构造器里装、`beginTask` 里清） | 生产侧「清障不得吃任务目标」必须真的生效（或如实撤销这条护栏） | ✅ **已确认（2026-09-26，运行期逐字，不只代码级）** —— 临时探针（`TaskTargetProtection.begin/end` 打印调用栈，验证后已删，工作树无残留）。**两条路径的差别是关键**：① **电池 `single:*` 看不到它** —— `RegressionBatteryTask` **直驱**子任务（`:877 current = step.factory().get()` + `:735 current.tick()`），构造器装的作用域**活到作业终态**（全轮只有 `MineJob.finish` 撤销过一次）⇒ 夹具绿；② **走会话的路径**（`CheckHarness` → `beginSelfCheckTaskInOpenScope` → `session.beginTask`）**逐字复现"装完即清"**：`run/headless-logs/20260926-122538-module_mining.log` 里 `破坏面装上 owner=mine ← MineJob.<init>(MineJob.java:285)` 紧跟 `end 调用（清到=mine）调用方 = BotSession.beginTask(BotManager.java:1998)`，随后 `MineJob.finish(:1001)` 的撤销已是**空操作** ⇒ 整条作业**保护为空**。生产入口同形（`assignJob:664 create → :674 beginTask`、`assignMineJob:690→:695`）⇒ **生产侧护栏一直是空的**，而电池因直驱而看不到（= `skill §6.9.1 ③` 的形态，也是"夹具绿、生产死"的**结构性盲区**：任何挂在 `BotSession.beginTask` 上的初始化，电池都不会验证）。**修法已定（A 案，用户 2026-09-26 拍板方向）**：把两处安装点从**构造器**挪到**首 tick**（`MineJob` 用现成的 `scopeStarted` 首 tick 块；`LumberJob` 加一个 flag；`FishboneJob` 本来就在 `prepare()` = 同一形状）⇒ `beginTask` 的清空天然落在装之前，顺序不可能再错；零接口改动、零夹具行为变化（直驱路径同样在首 tick 装 = 与今天等价）。判据 = **结构门禁**（构造器体内不许出现 `TaskTargetProtection.begin*`，且必须出现在 `tickOnce()`/`prepare()` 体内 + 人口下限防"抽空即假绿"；先例 `D-425`「死码删除的判据只能是静态门禁」）。⚠️ **修法已落地（2026-09-26，第 2 段 2.4）**：安装点从构造器挪到**首 tick**（`MineJob.tickOnce` 的 `scopeStarted` 块 · `LumberJob.tickOnce` 新增 `protectionStarted` 块 · `FishboneJob.prepare()` 本来就是同一形状 ⇒ 零改动）⇒ `beginTask` 的清空天然落在装之前，**顺序不可能再错**；直驱路径（电池）同样在首 tick 装 = 与今天等价 ⇒ **零夹具行为变化**。判据 = 新门禁 `tools/check-protection-install-point.py`（**自带 6 条合成红臂**：构造器 / `if` 嵌在构造器里（第一版的假绿形态）/ 静态块 / 注释假命中 / `tickOnce` / 类名别名；另实测**真树红臂**：临时加一个"构造器里装"的探针文件 ⇒ 精确报出文件:行 ⇒ 删掉还原）；断言 = ① 任意安装点不许在构造器里（**解析必须跳过 `if`/`for`/匿名类帧**，否则 `if` 嵌构造器会假绿）② `job/` 下的安装点必须在 `{tickOnce, prepare}` 里 ③ 人口下限（三个生产作业各 ≥1 · 全仓 ≥3 · 扫描 ≥400 文件）+ 已挂 `tools/check-all.sh` | 0.2 轮 |
| 1.4s | **`I5` 破坏面**（草案 `S2` 的原文口径：「施工期间**别的写入者不许破坏** I1/I2 的格」） | 通道的**地板/支撑**不再被自己的追簇或 `collect-drops` 的 `PATH_ACCESS` 挖掉（A1 处境） | ⏳ **未落地**。裁定原文见 `docs/plans/2026-09-25-通道施工器草案.md` §4 `I5` + §7 `S2`（红臂 = "去掉保护 ⇒ 地板被挖掉 ⇒ 红，用 A1 的真实破坏序列造场景"）。⚠️ 与本次落地的**放置面**是同一不变式的另一侧（"这条通道在施工期间必须保持可通行"），载体现成（`TaskTargetProtection` 破坏面 + `FishboneTemplate.cellSet()`），**但破坏侧要做"豁免鱼骨自己挖穿通道"的例外**（不然连通道都挖不出来）—— 这个例外的口径需要裁定 | ✅ **口径已定（`D-451`，用户 2026-09-26 选 A：按「**已完工格**」保护，不按调用者身份、不按相位）**；⚠️ **是否落地、以及是否改为治疗式**（`survey/36 §4.1`：`1.4s` 是预防式且自带难裁例外，而施工记录图只需事后一次比对 ⇒ 可能不必做）⇒ 属**排期问题**，见 `J-★ 第 3 段 3.4` | 0.5 轮 |
| 1.4o | ⭐ **砍完高树之后伐木 job 砍不动下一棵树**（机制 A 扰动暴露的潜伏缺陷） | 伐木 job 不再依赖「高树恰好排在最后」；`nearest` 选树的位置敏感不再是隐藏缺陷的遮羞布 | ⏳ **待开工（本轮副产物，未修）**。证据（`run/headless-logs/20260925-224452-core.log`）：选树顺序 `@20→@19→@28`(7 格云杉)`→@33→@29` 时，**@33 与 @29 都是 `chopped=0/4 failed=4`** ⇒ `partial_quota`；而基线顺序 `@20→@19→@29→@28` 全成（@29 是同一棵树，基线 4/4 成功）。⇒ **砍完那棵云杉之后 bot 处在无法再砍任何树的状态**（当时日志里它在 x=28 处连放 `place 28,66 … 28,74` = 爬柱）。CORE 一直绿只因那棵高树**恰好被排在最后** ⇒ 那一步是**位置敏感的脆判据**。⚠️ **不要去改判据/改场景**（那是把缺陷藏起来）；判据建议 = 固定「先砍高树」的场景顺序（两种顺序都要能跑完）；红臂 = 退回当前实现 ⇒ 第一种顺序红 | 0.5 轮 |
| 1.4n | **同式 yaw 公式合并**（`horizontalDistanceToCenter`/`faceCellCenter` 的既有 11 处副本） | 12 处同一段"朝格中心转向"只剩一处 ⇒ 下一个"改了甲、判定器读乙"不可能再发生 | ⏳ **待开工（低优先，故意不与修复混在一起）**：`D-445` 落地时新增了唯一出处，但 `TraverseExecution`/`DescendExecution`/`FallExecution`/`PillarExecution`/`AscendExecution`/`DiagonalExecution`/`BreakAndTraverse`/`BreakAndEnter`/`PlaceStepAndTraverseExecution`/`RoadBuildTask`/`BlockInteraction` 里仍是各自一份（`grep -rn "atan2(-dx, dz)"` 现在仍返回 12）。**为什么没顺手做**：这些路径行为正确且正被电池覆盖，混进"修下落"里会让红/绿归因不清（`AGENTS.md`「不许补丁堆叠」）。判据 = 合并后 `grep` 只剩唯一出处；红臂 = 留一处副本 ⇒ 红 | 0.5 轮 |
