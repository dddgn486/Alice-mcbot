...
| 组 | 项数 | 影响业务？ | 备注 |
|---|---|---|---|
| §1 安全底座（风险 / 维生 / 写入准入） | 9（新增 S-9） | ✅ 4 项（S-1/S-2/S-3/S-4），常驻任务后上升 | 风险清单 9 条断言**至今全部成立**；**2026-09-16 全表复核**：S-1…S-5 已落地（S-5 的基-1 接线+判据已就位，仅两个最高等级无产出者=有意保留）；**真缺口剩 S-6/S-7/S-8**（RiskSwitches 全局静态、两守卫无开关、`policyVersion` 恒 0 且零读者）—— 前两条等 S1 冻结点裁口径，第三条优先级低；另新发现一项（`FluidRiskPolicy` 只覆盖目标格 6 邻格）见 S-4 行–S6 **0/6** |
...
| **S-9** | **危险伤害按"血量差"记录 ⇒ 被回血抹平的伤害不可见**（D-261 实测暴露，2026-09-16 新登记） | 本文件 §5.10（D-230）+ D-261 | **属实、未修**：`SurvivalSystem`/`BotEventLog` 的 `hazard=…` 危险事件是**按采样血量差**写的 ⇒ 当"净血量不变"时（实测：火焰 1 点/20t 与 `aiStep()` 无条件回血 1 点/20t **永久抵消**，食物清零也关不掉）⇒ 伤害在系统里**完全不可见**，按伤害累计/致命性做的任何决策都会漏计 | **修法（需单独立项，属行为/口径变更）**：危险伤害改由**伤害事件**观测（Forge `LivingHurtEvent`/`LivingDamageEvent`），与血量解耦；本轮只登记不修 |
