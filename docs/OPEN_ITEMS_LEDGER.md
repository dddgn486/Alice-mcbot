...
| 1.4h | ⭐ **片 A：`P0.5` 能力注入 + `C8` 三件 + `A14`↔`C8` 对账**（`D-443` 裁定 1a/1b/1c/7a/7b） | 支巷不再因"**挖掘站位走不回来**"被放弃（真机 `spur_abandoned` 次数下降）；通道**不许被搭成一座长桥**（大矿洞该放弃就放弃） | ⏳ **待开工**。内容：① `MiningProfile` 加"接近能力"字段（默认 = `PathRequest.of` ⇒ 其它消费者零变化）、`MiningPlanner:367` 改成问 profile、鱼骨那处调用点传 `withPlacement`；② `C8` 三件 = 单段悬空 ≤ `maxGapLength`(4) + 前瞻无地板 ⇒ `big_cavern_ahead` 放弃/失败 + 额度用尽 ⇒ 如实放弃（取代 `A14` 的"退回纯通行、坑留着"）；③ 预算合成一个名字 `bridgeBlockBudget` = `max(16, advanceCells/10)`。判据 = 夹具造 A2 处境（bot 高 3 格）⇒ 绿；**红臂** = 接近集退回 `of` ⇒ 红 · 大矿洞超限 ⇒ 放弃，窗口调无限 ⇒ 红 | 1 轮 |
