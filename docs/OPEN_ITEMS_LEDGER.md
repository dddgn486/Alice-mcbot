...
| 1.4r | ⭐ **`D-362` 的任务保护作用域可能在真机上根本没生效**（构造器里装、`beginTask` 里清） | 要么确认它一直没生效（那 `D-362` 的护栏在真机上是空的），要么修正安装点 | ⏳ **代码级已核、缺一行运行时确认**。事实：`MineJob:285` 与 `LumberJob:231` 都在**构造器**里 `TaskTargetProtection.begin(...)`；而调用方是 `session.beginTask(new MineJob(...), …)` / `session.beginTask(job, …)`（`BotManager:718/792` 等）⇒ **Java 参数先求值** ⇒ `beginTask` 函数体里的 `TaskTargetProtection.end(bot)`（`BotManager:1998`，与 `begin` 同一 commit `a8b9679`）**在同 tick 把它清掉**。⇒ 真机上"清障不得吃任务目标"很可能是**空的**，而夹具 `ClearGuardCheckTask` 在自己 `prepare()` 里装（任务已 begin ⇒ 生效）⇒ **夹具绿、生产死**（skill §6.9.1 ③ 的典型形态）。**最小核实**：在 `BotManager.beginTask` 的 `end` 之后加一行 `BotLog.info("[D-362] protection_active={}", TaskTargetProtection.active(bot))`，跑任一 `mine_*` 步看它是 false；确认后把两处安装点挪到**任务开始后**（学 `FishboneJob.prepare()`/`ClearGuardCheckTask`）。⚠️ **本次没顺手改**（它与 `I5` 不是同一件事，且属 `D-362` 的回收范围）| 0.2 轮 |
