...
| **P2c** | ⭐ **`FALL_NOT_ON_GROUND`：起点不落地的 `FALL` 边「规划得到、执行不了」**（`D-428` §六，2026-09-24 登记，**本片一行没改**） | 判据草案 = ① 夹具量「计划里存在起点浮空/水柱顶格的 `FALL` 边」② 该边两侧准入比对（`validate` 必拒）③ 修后两侧一致；**先要用户拍方向** | `D-428` §六 | **待用户拍板**（推荐 = **先不改**：保留守卫 + `TIMING` 分类；复核触发 = 日志出现 `FALL_NOT_ON_GROUND` / 实测到浮空起点的 `FALL` 边 / 用户要求「水里也能自己掉下去」；要改则取「放宽执行侧」，与 `D-427` 对 `PILLAR` 的处置同形，且**必须先有夹具**） |
...
| # | 事项 | 目标 | 判据 | 成本 | 依赖 |
|---|---|---|---|---|---|
| 0.1 | `P2c` 裁定（`D-428` §六） | 决定"起点不落地的 `FALL` 边"怎么办 | 一句话即可：**先不改**（推荐，0 成本）/ 放宽执行侧（须先做夹具） | 0 或 1 轮 | 用户 |
...
#### J-1 能力线主战场（**3–6 轮 + 1 客户端轮**）：三天挖矿任务线 ③ 鱼骨 → ① 跟随 → ② 探洞

> ⏸ **本节以下（J-1/J-2/J-3）的排期 2026-09-24 起挂起**：用户去找**勘测员**谈计划，**根据勘测报告再决定之后的排期**（原话）。⇒ 下一轮开工前先看勘测报告（`survey/` 新号 + 我方核对）。下表保留为**候选池**（不是已定档的排期）。

| # | 事项 | 目标（用户可见） | 判据 | 成本 |
|---|---|---|---|---|
| 1.1 | 鱼骨切片 1（`D-386` 口径已定档 + `docs/plans/2026-09-21-鱼骨挖矿计划.md`） | bot 按鱼骨模板开挖，看得见推进 | 新夹具：矿石脉场景 + `mined`/覆盖率读数；**先红后绿**（关掉模板生成 ⇒ 红） | 1 轮 |
| 1.2 | 鱼骨切片 2–4（设计文档里的分批） | 鱼骨在真实矿洞里跑通 | 每片一条夹具判据 + CORE 逐步 diff | 2–3 轮 |
| 1.3 | ⭐ **客户端验收轮** | 你在真实存档里看 bot 挖一段鱼骨 | `latest.log` 的 `SUMMARY`（mined / 用时 / 拒绝归因）+ 你的肉眼结论 | **1 客户端轮** |
| 1.4 | ① 跟随 / ② 探洞（按 1.3 的结论取舍） | 另外两条挖矿任务线 | 同 1.1 的夹具口径 | 各 1 轮 |
...
| # | 事项 | 触发条件 | 成本 |
|---|---|---|---|
| 3.1 | 云端迁移二期（`S31-4` **世界母本版本化** = 报告没覆盖的真风险；`S31-3` 云上 CORE 不是免费） | 你真要在云上长期跑 | 1–2 轮 |
