...
| **B3** | 能力清单从代码生成 + 双向防漂移门禁（记忆库的前置） | ⏳ **未做**（用户 2026-09-23 选定的一批里唯一剩项）。⭐ 方案已写死在 `D-418 §五`：**不引入第五个真相源**，而是**聚合已有单一出处**（`CheckModules.knownIds()` / `CURATION` 步表 / `MachineMap` / `JobKindContract` / `MovementType` / `RiskSwitches.KNOWN`）成人读清单，门禁照 `machine-map.py`：解析不到即红 + 双向 + 人口断言 | ⏳ **下一步**（`A2′` 与之无依赖，二选一由用户定） |
