...
| 13 | ⭐ **区域补种异步化 + 可配置拾取清单**（用户 2026-09-19 **细则已定**）：砍完**立刻捡原木**；"等待窗口"= **现在的巡查退避/等生长**（不新开窗口）；窗口内**主动扫区域内地上的东西**，**一次不设上限、扫到区域内捡完为止**，可**反复扫描等待**；⚠️ **补种与扫描不许撞车**（串行互斥，扫的时候不补、补的时候不扫）；"该补的种"先记账（已有 `pendingReplant`），捡够再补；身上有苗则砍完立刻补（现行为保持）；**不许硬编码树苗** ⇒ **可配置清单**（例：树苗 / 树枝 / 苹果…） | ⏳ 待实现（实现前先出方案：`RegionLumberJob` 的窗口状态机 + 主动收集 + 拾取闸门口径 + 配置出处） |
...
| 19 | ⭐ **客户端反向测试（2026-09-19 19:00–19:07 那轮 + 19:28 复验）的结论 + `D-339` 落地**：✅ 保护区里**玩家自己**发起照旧干活（`chopped=0→5`、`STEP_PLACEMENT` **真垫了方块**、`scaffoldLeft=0`）· ✅ **封顶生效**（LLM 自起那轮 5 棵树全 `zone_break_not_allowed`）· ✅ **事件环可取证**（`bot_report` 快照 `recentEvents` 里 `type=STOP` + `droppedTriggers=3`）· ✅ **`D-339`：夹具终态不再交给 LLM**（`fixture_terminal_silent`；反向对照**恰 1 红**；`module:llm` + CORE **51/51**）· ✅ **客户端复验通过**（19:28 新包：同场景同一失败 ⇒ 全会话 `[Goal]` **仅** `trigger_dropped reason=fixture_driver` 一行，**零** LLM 请求；用户判"符合预期"）· ✅ **口径①已定**（用户：一次性作业在自己认领区被拒**是设计行为、是反例、不开豁免**） | ⏳ **残余口子（未闭）**：夹具驱动任务**运行中**的 `event:PROGRESS` 仍会唤醒 LLM ⇒ 要一起掐需把 `driver` **在任务启动时固定**（今天终态读全局位 = "最后写入者胜"，运行中一次 LLM 决策就会把夹具任务的归属改写成 `llm`）。另：**`L2` 要不要也加"每 `scopeId` 区内放置上限"**（`L1` 有 ≤8）= 待用户裁定 |
