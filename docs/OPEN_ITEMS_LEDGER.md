...
| # | 事项 | 判据 | 依赖 | 状态 |
|---|---|---|---|---|
| **RC1** | ⭐ **按 `D-403` 简化**：**保留自动拆除**；三条保留条件（已加载 / 现场仍是我方 / 不摔 bot）；**不满足 ⇒ 当场放弃 + 必带归因**（不许静默）；**不做**"待办/分批/显式回收入口" | 夹具：不满足条件 ⇒ `skipped` + 带码 notes（C2 已绿：`underfoot_unsafe`） | 无 | 待做（简化版） |
...
| # | 事项 | 判据 | 依赖 | 状态 |
|---|---|---|---|---|
| **P3** | **掉落物"本作业范围内"收集授权**（`provenance=GRANTED_AREA`）—— 用户已裁定方案 | 夹具：作业声明半径内、且属该作业目标产物的掉落物 ⇒ 收；**范围外/玩家丢的 ⇒ 仍不碰** | 无 | 待做 |
| **P5** | ⭐ **`D-391` 收口**：被同族矿石+石壳包住的目标给不出站位/进入方案（已排除 A2 截断、排除搜索预算）+ **死候选剔除**（`candidates` 曾涨到 1037 ⇒ 每次选择全量复检） | 新探针先补读数；收口后把 `mine_vein_propagation` 的完成度下限**从 12/30 抬到 ~28** | 无 | 待做 |
| **P2** | **Movement 审查切片**（对齐 Baritone；`Ascend` 已做）：`Descend`/`Downward`（用户报**未复现**，暂不做回归）→ `Diagonal` → `Traverse` → `Pillar` → `Fall`；每片 = Baritone `文件:行` 对照 + 差异表 + 判据 | 每片一条红/绿判据；同时把 `EXECUTOR_REFUSAL_CLASSES` 里**未指名的 26 个能力类码逐个指名**（`CAPABILITY_UNRESOLVED_BUDGET` **只许减**） | `D-396` 尺子 | 待做 |
| **P2b** | `D-394` 遗留：**上升 Movement 的其他可能隐患**（用户改记口径）+ **段级 `resync` 重放同一条计划无次数上限** | `resync` 加界 + 判据（不许无限重放） | 无 | 待做 |
...
| **C5** | ⭐ **夹具的第三方前提**（`D-409 §五` 的教训）：世界母本 = 玩家真实存档副本 ⇒ 夹具**继承玩家的 FTB 认领**；而"夹具自带前提"这条纪律**只覆盖了我方保护区**（`FixtureZone`），第三方那一半**没人管** ⇒ 踩进去只会得到**误导性的内核失败码**（`no_reachable_candidate`）而不是"前提不成立" | **✅ 完成（`D-410`）**：新增 `task/FixtureThirdParty`（逐区块问 **FTB 自己的**裁决函数；不改世界；fail-open 同 `D-326`）+ `task/PremiseGateTask`（**首 tick 之前**求值的闸门 —— 用 `skipWhen` 不行，它是**终态之后**才求值，任务仍会打出误导码）；接线 = `LumberModule` 3 步（`lumber_failure`/`lumber_job`/`region_maintain`）；判据 = `single:lumber_job` ⇒ **`SKIP ticks=0` + `DEGRADED`**、`[Premise]` **逐字点名 4 个被拦区块**（与认领文件恰好一致）、误导码 **0**、用时 **72 s→21 s**；⭐ **CORE 逐步 diff = 只此 1 条变化**（`lumber_job: FAIL→SKIP`，42 步列表不变，4552→3578 tick） |
| **C6** | 把伐木课程**搬出认领区**（如整体 `z-=32` ⇒ `z∈[171,199]`），让 `lumber_job` **能真的过** | ⚠️ 场景有自记的"树冠通道红线"（`y=65 z=212..214` 是唯一通道）⇒ 搬迁会改几何，**必须单独一轮 + 夹具重验**。⭐⭐ **且 `D-410` 顺带查出：搬迁是必要条件、但"搬完就绿"不成立** —— 同样移开认领的两次运行结果不同（`FAIL partial_quota ticks=904` vs **`PASS ticks=593`**）⇒ 配额 4 棵 vs 可用候选 5 棵（1 棵是设计上的"期望被拒对照"）是**刀尖**，而 `already_attempted`（`LumberJob:646`）让**失败一次 = 永久少一棵** ⇒ 搬迁必须**连"配额 vs 候选数"一起处理**，否则只是把一个假红换成一个 flaky | **待用户拍板** |
| **A2** | ⭐ **逐步缓存可行性验证**（先验后改机制） | 判据 = 同一提交"全量 vs 逐步缓存"**逐条判决完全一致**（`D-201` 附注一的逐步 diff 现成） | ⭐ **升级为本轮唯一"乘法级"杠杆**（缓存命中只取决于它，不取决于 A1′）；⚠️ 步间若不独立 ⇒ **必须放弃**（否则假绿） |
| **A3** | ⭐ **击杀来源归属**（`survey/29 §2.1`：击杀产物落 `FOREIGN` ⇒ 捡不起且静默） | 判据 = 新夹具：生成动物 → 击杀 → 断言 `provenance != FOREIGN`（**先红**）。**代码链我已核实**（两个 matcher 都只遍历 `recentBreaks`）+ **反证检查**：`src/` 无任何 `LivingDropsEvent`/`LivingDeathEvent` 消费者 ⇒ 无补偿路径 | 同意"**归属先行**"（先只登记、不写攻击）；端到端仍须夹具复现 |
| **A4** | 假绿夹具几何重做 | = `§11` `Z1` 尾项（`restore_underfoot_safety`） | 已有，无需新增 |
| **B1** | 「下一步表」最小版（五列 + 一条门禁 + `tools/next-step.sh`） | 判据 = `exit` 指向不存在的步 ⇒ 红；在飞项 scope 相交 ⇒ 红 | ⚠️ **我持保留**：它引入第五条真相源，且**不降低成本本身**（降成本的是 A1/A2）⇒ 建议排在 A1/A2 之后；**待用户拍板** |
| **B2** | 「底线不许进配置面」门禁（+ 必选/可选守卫分界） | 断言底线集合不出现在玩家可写入口 | 便宜、能红 ⇒ 可做 |
| **B3** | 能力清单从代码生成 + 双向防漂移门禁（记忆库的前置） | 照 `machine-map.py` 模子：单一出处 + 解析不到即红 | 便宜 ⇒ 可做 |
