...
| 4 | **区域级权限 / 预算授予面** + **任务区声明**（任务自声明，随 `scopeId` 生灭；目标内 `KEEP` / 目标外提权+预算+`TEMP`） | 🟡 **一半已落地**（2026-09-19，`D-338` 附注六）：⭐ **几何 + 锁定层** = `protection/TaskZoneRegistry`（工作区域**方块级** ⇒ 任务区**区块级最小覆盖**，单向派生；可覆盖保护区父类、**不得**覆盖安全区 ⇒ **报错且不裁剪**；随 `scopeId` 生灭、`NO_SCOPE` 拒绝、命令层无写入口 ⇒ 结构性锁定）+ 生产接线（`RegionLumberJob` 首 tick 解算、冲突 ⇒ `task_zone_conflict` 如实失败、终态/作用域收尾自动解除）+ 只读面（`/alice protect list` 三态、`/alice region info` 预检）+ 门禁 `task_zone`（EXTRA，50 判据，**两次反向对照**先红后绿）。⏳ **仍未做**：把它当**授权封套**接进闸门（目标内 `KEEP` / 目标外 `TEMP`+预算）—— 依赖第 5 行；⚠️ 冲突语义（今天=任务失败）待拍板 | ⏳ 剩下一半需先拍板第 5 行 |
