...
| 1.2 | **复评（0 轮）**：用 1.1 的读数定**切片 2 的落物/几何** | —— | 决定：场景要不要加地板（`≈130 → ~15 tick`）**且**是否接受作废 `D-389`/`D-391` 的历史读数。⚠️ **输入已到手（`D-433` §三）**：鱼骨每轮规划 **0.55 ms**、**41 tick/格**（1 格宽实心石巷、地板实心 ⇒ 本片**没走到**"落物掉进悬空"那条路）⇒ 切片 2 的夹具**必须显式选**：要么自建**无地板**场景复现 `≈130 tick/格` 的收集代价，要么把"落物可能掉落"写成 `C2` 的例外条款 | 0 |
