...
| **Z4** | ⭐ **空集断言清单**（`Z1` 的连带，`Z2` 只做了"可见"）：10 组"用账本证明我没写世界/没残留"的判据在野外**人口为 0** ⇒ 空集恒真（**假绿且不报错**） | 逐条选三种修法之一：① 夹具 `FixtureZone.protect(...)` 让**人口回来** ② 判据改读**世界事实** ③ 明确标 `n/a（区外⇒无义务）` 并**印出人口**（`Z2` 已给的能力） | `Z2`（已给可见性） | 待做（清单 + 证据 = `docs/reviews/2026-09-23-Z2-账本闭合与空集假绿.md §6`；已知最刺眼的一条：`craft_table` 的 `no_world_write` **实测 `temporaryBlocks=0`**） |
