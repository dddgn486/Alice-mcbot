...
>
> **下一步**（压缩后从这里接）：
> 1. ⭐ **量"直挖边为什么没被采用"**（`D-373` §四的未解机制）：B 实验里 100× 预算找到的路是
>    **88 段**，而"直挖 4 下 + 横 5"理论只要 ≈10 段 ⇒ A\* 返回的不是最便宜的路
>    ⇒ **下一步只做一件事：把返回路径的动作类型直方图 + 总成本打出来**（探针加一行统计，
>    一次 `single:mine_reach_probe` 73 s 即可）。**先量后猜，不许先加守卫/调权重。**
>    证据与已排除项：`docs/reviews/2026-09-21-B-深矿可达性判据实验.md`
> 2. ✅ **真机第五轮已完成，A1 止血达成**（2026-09-21 12:34–12:35，日志仍在 `[fixed-client]/logs/latest.log`）：
>    `[SearchTick]` 25 次拒绝（全部 `已发起=1`）· 超预算搜索 **每 tick ≤1 次**（第四轮 376 条/12 波）·
>    **tick 速率 0.4 → ≈17.9 TPS**（`tick=11257→11580` = 323 tick / 18 s）· `Can't keep up` 降到 2 次 ·
>    挖掘仍在推进。**逐条数字见 `docs/reviews/2026-09-21-掉落物在洞里被瞬退.md` §6。**
> 3. ⭐ **真机暴露的存量缺陷（新的待办）：掉落物被"瞬间退役"**——bot 进了拾取阶段、发了寻路、
>    走了 1 格，但寻路 `SEARCH_LIMIT` 后**不做任何重试**就 `retire MOVEMENT_FAILED`，
>    `SUMMARY reason=done collected=0/2`，产物静默丢失（用户手动推 bot 才捡到）。
>    事实/几何反推/4 层假设/修复候选见 `docs/reviews/2026-09-21-掉落物在洞里被瞬退.md`；
>    **等用户回答该文 §7 的 4 个物理问题后再定方向**（`reanchor=0`、`goal_shift=0` 是两个硬指认）。
>
