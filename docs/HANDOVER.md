...
> **⏭ 本轮客户端待验证（2026-09-18，最新）**：**保护区 2/2（`D-314`）已实现并已同步到固定客户端** ——
> 零参数物品 **`alice:protection_selector`** 右键 ⇒ 以你为中心的 **17×17 区块网格**（左键认领 / 右键取消 /
> 拖动连选 / 关闭即批量提交；协议 3 包 · 服务端权威）。**看什么、`[Protection]` 日志怎么读、反证怎么做**，
> 见 `docs/TESTING_GUIDE.md` **§2.5**。已装工件 `alice-1.0.0-1.20.1.jar`，内容摘要
> `JAR_CONTENT_SHA256=458d15b08c83a735d509ee2e532986819cc51b4ea50138e3ea0d370786ca838e`（同步于 2026-09-18 12:54；
> jar **不可字节复现** ⇒ 认"同一版代码"要用 `tools/jar-content-hash.sh`，实测与 `build/libs` 一致 ✓）。
> 离线侧已绿：`module:protection` **57/57** · CORE **49/49**（步序 diff=0） · `check-all.sh` **17 PASS + 0 WARN**（含无头断言）。
> ⚠️ 界面观感（地形色像不像 · 点击与拖动手感 · ESC 与按 T 的行为）**日志证明不了** ⇒ 只等你这一轮。
