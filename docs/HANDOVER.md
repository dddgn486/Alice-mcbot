...
## 6. 压缩后的推荐路线（用户 2026-09-15 指定"由 AI 推荐一条"）

> 断点 commit = `d06c76c`（工作树干净，已推送 `github/master`，客户端 jar = `35fa4580…`）。
> **M 线四项（M1/M2/M3/M4）+ §9#4 负例全部完成，且都到 `WINDOWS_CLIENT`**（本文件 §1/§4）。

**推荐：走「维生最小件」（离线可验证、不需要用户在场）**

1. **`LOW_AIR` / `ON_FIRE` 纳入 `SurvivalSystem.shouldInterrupt` 否决** —— 今天这两类是"**只报不拦**"，
   即**溺水与着火不救**（`HazardType` 五种里只有 `LAVA_CONTACT`/`SUFFOCATING` 会中断）。
2. **给 `HazardState.previousHealth` 找第一个读者**（掉血 ⇒ `DANGER` 事件）—— 今天它是**只写不读**，
   掉血**不触发任何动作**。
3. **给维生加一个 BASELINE 电池步** —— 维生是**唯一"零电池步"的子系统**（`RegressionBatteryTask` 里
   `Survival` 命中 0）⇒ 只能真人验。注意 `event_thresholds` 那步是 **EXTRA**，`core` 不跑。
4. （顺带）`SurvivalExitTask` 终态落一行，把"选了落点却没走到"变可观测（`nearestSafeRefuge` **不验可达**）。

**为什么推荐它**（四条）：① 它是"长作业能活着回来"的必要件，而缺口是**真实的**（溺水/着火不救、掉血无反应）；
② **全离线可验证** ⇒ 正好对上项目唯一指标"从改一行到知道对不对"；③ 顺带把维生从"零门禁"变成有门禁；
④ **不需要你拍板**，压缩后可以自动推进（M 线就是这么推完的）。它与已完成的 **M2（长作业周期复评）**
是同一条"自主中段"的链条。

**之后的下一个决策点（要你参与）**：**作业区抽象 / 粗目标 / "到达"** —— `survey/08` §3 把"短线→长线"
归结为"**把到达从 5 格放大到 60 格**"，且 §3.5 指出**真正的差别是 `GoalFoot` 太精确**（`GOAL_NOT_LOADED`），
而**滚动重规划的骨架已经在跑**（`PathRetryRunner` + `PathSession`）。`survey/08` §10 明确**不回答**
"到达算法"，因为**需要真实存档地形做判据** ⇒ 这一步要你参与 + 客户端。

**明确不是承重件（按需再做，别插队）**：**M4b**（`tree[].lastFailure`，需逐 Job 定义"哪个子阶段失败"）、
**M3b**（`write_budget_exhausted` / `stale_target` 两条映射**照词表写但未被观测**）、
**`decision_contract` 的档位**（自称"任何改动都跑得到"却挂 EXTRA，属策展裁定）、**台账 §5.6 风险议题**（B/C 候选）。
