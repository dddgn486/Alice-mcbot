...
# 断点⑭：真机入口的起点改成 **bot 自己的脚位**（`D-441`）—— 下一轮仍是你的客户端验收（2026-09-25 下午）

## 1. 收工状态（逐条可核）

| 项 | 值 |
|---|---|
| 触发 | 你的原话：「把给我的测试工具修一修，**就以 bot 位置为起点**，之前 bot 想到我这个位置，**又推不动我**」 |
| 事实（真机取证） | `latest.log` 13:18:31.502 `[FishboneJobItem] 启动 … start=-54, 46, 199` = **玩家站的那一格**；物品又把 bot **传送**进同一格 ⇒ bot 与玩家抢一格，而玩家**不可被推**（fake player 挤不动真人）⇒ 它站不稳（"站得正"是就地挖的硬前提）、也回不到起点 |
| ⚠️ 诚实边界 | 那两次 `main_unreachable:no_reachable_standing_point`（`faceStandable=1/6`）**不能**全归因于玩家 —— 唯一可站的邻面是**目标正上方**，而模式 A 按设计排除它 ⇒ 地形成分仍在。**"bot 自己那格为何没被接受为候选"仍未证实**（`D-441 §一.3`，若再现要补探针） |
| 本轮交付 | ① 起点 = `MovementHelper.footCell(bot)`（**物品不再传送 bot**）；站不住 ⇒ **当场拒绝开工**（判据 = 与 `PREPARE` 同一个 `canStandCentered`）；② 选**离你最近**的 bot；③ 一只都没有才生成，且**不生成在你那一格**；④ 你若与它抢同格（脚位/头位）⇒ 拒绝 + 提示让开一格；⑤ 方向仍 = 你的朝向（零参数契约不变）；⑥ `/alice come` 顺带改成送你**旁边**一格（同一个缺陷的第二处现场） |
| 裁定/记录 | **`D-441`**（事实 / 7 条规则 / 代价 / 判据与注入臂 / 复核触发）；台账 **`1.4f` ✅**；计划 §8 的入口口径已同步改掉 |
| 绿 | `single:fishbone_slice2` = **`checks=83 failures=0`**（81 → 83，新增 2 条起点判据）· `single:fishbone_slice1` = PASS · **CORE 电池 = PASS（242 s）** · `check-all` = `pass=21 warning=2 failed=0` · kernel-predicates = PASS |
| 红臂 | **R1**（`originFor` 去掉"站不住就拒绝"）⇒ 夹具实测 `FAIL failures=1`，失败项指名「悬空时 `originFor` 必须返回 null」✓；**门禁 3 臂注入**（① 加回 `teleportTo` ② `footCell`→`blockPosition` ③ 绕过 `originFor` 的结果）⇒ **3/3 全红** ✓ |
| 工件 | `build/libs/alice-1.0.0-1.20.1.jar` · `sha256=cdf237c8780f6014e6bb122414d263fdd5a0f31d961253fff561d06cf5bafa43` · 已同步进固定客户端 `mods/`（同一值；`sync-windows-artifact.sh` 的陈旧工件护栏放行 = 源码不新于工件） |

## 2. ⚠️ 仍待你拍（上一轮就问过：`D-440 §五`）

往下追 **2 格**时最底那一格**物理上挖不掉**（挖掘器拒绝站在"正在挖的那一格"上）+ 坑里的掉落物可能收不回
（终态 `product_not_collected`）。两条：① 允许"挖脚下"（改站位约束，影响所有 `MineTask` 调用点）；
② 把"坑里够不着"从 FAIL **降级为如实上报**（`uncollected=n` + 仍 DONE —— 会动 `D-436 §一.2`）。
**没拍之前夹具停在 1 格深。**
...
```
① 重启固定客户端（jar 已同步）
② /alice come          ← 叫 bot 到你**旁边**（现在它不会塞进你那一格了）
   （或用遥控器 alice:bot_remote_control 把 bot 开到你想要的位置）
③ /give @s alice:fishbone_job，朝你想挖的方向右键
   ⇒ 聊天会回：bot 名 + **起点 = 它自己脚下** + 离你几格 + 方向 + 形状
   ⇒ 若它站不住 / 你就站在它那两格里 ⇒ **当场拒绝**并说明原因（不再白跑一轮）
④ 站旁边看就行（**别挡在它正前面**），重点看：
   ① 支巷不再被放弃（不该出现 spur_abandoned）
   ② [Pillar] placed / [WRITE] place … by=fishbone-… = 它自己把挖掉的地板补回来
   ③ 矿簇是否被挖干净（洞壁 / 洞顶 / 地板）
```
...
## 11. 2026-09-26 补记⑦：**Baritone 功能面梳理落盘**（用户「做一次梳理，到底还有哪些 Baritone 功能我们没实现」）

- **产物** = `docs/reviews/2026-09-26-Baritone功能面缺口清单.md`（**158 条功能点**：已实现 17 / 部分 69 /
  刻意不做 14 / **缺失 58**）。**§1 只列影响产品线的 **24** 条**（`K1–K9` 内核 / `M1–M5` 挖矿 / `L1–L2` 伐木 /
  `C1–C2` 合成传输 / `O1–O5` 命令面 / `V1` 观测）；**§2 = 明确不做/不在产品线清单**（带 `D-编号` 或理由）。
- **方法（可复跑）**：8 区域并行**只读**审计；每区域逐类枚举 Baritone 侧、在 Alice 侧 grep 对应物；
  每条「缺失/部分」写明**搜过的词 + 0 命中**、「刻意不做」必须给 `D-编号`。原始区域产物 =
  `run/baritone-gap/<区域>.md`（**`run/` 已 gitignore**）+ 合并器 `run/baritone-gap/merge.py`。
  ⇒ 教训一条：**大审计不要让子代理回大 JSON**（第一次 workflow 返回值在 **50 KB 处被截断**，8 区域只回来 3 个）
  ⇒ 改成"各自写文件 + 只回 ≤8 行摘要"。
- **引用树**：`reference/baritone/` = MC 1.21.4（mod_version 1.13.1）· `reference/baritone-1.20.1/` = 1.20.1。
  **以后者为准**；30 条带行号引用已在 1.20.1 树上复核（文件缺失 0 · 行号超界 0）。
- **§1.7 建议排期**：`K1` 重力方块族（**同时是"局部扫描自动复位"方案的主要敌人**：挖通 → 沙砾 2 tick 后
  落回 ⇒ 修复立刻假绿）→ `M5`（`1.4r` 真缺陷）→ `K2` `GoalAdjacent`（一条改动同时松开挖矿站位、伐木站位、
  通道施工器三处手搓）→ `K6` `PathEvent` + `V1` `PathRenderer`（路径中途的观测前提）。
- **同批三条文档/数字更正**：① `D-418` §三 断言⑤ 正文「今天 Alice **没有配置面**」**已过期** ⇒ **就地更正**
  （门禁实跑 = `配置面 = 存在 12 处（已断言不含底线名）`）；② `cache_selection` 的 9 条 `缺失` 多是
  `D-044`⑤/`D-067`⑧ 同一延后项的分身 ⇒ 不算 9 个待办；③ `mine_reach_probe` 应改 `CheckStep.skippable`
  （新世界里它必然读数无效；`HANDOVER.md:123` 早有"结论不作数"登记）。
- **`1.4r` 升级为"已确认"**（运行期逐字，非仅代码级）：电池**直驱**子任务 ⇒ 看不到；走会话的路径下
  `MineJob.<init>(:285)` 装 → `BotSession.beginTask(:1998)` 清，作业全程**保护为空**（日志
  `run/headless-logs/20260926-122538-module_mining.log`）。修法已定（首 tick 装）+ 结构门禁，**未落地**。
- **仍未做**：`1.4r` 修法落地 · `F2`（缺地板 补 vs 放弃，**已裁定 A：补 + 用尽即如实放弃**）·
  `F3`（`1.4u` 标签）· `I5` 破坏面（`1.4s`，**已裁定 A：已完工格才受保护**）· `module:mining` 里
  `mine_reach_probe` 的 `skippable` 化。

## 12. 2026-09-26 补记⑧：**勘测 35–39 读毕 + 排期重排（`J-★`）+ 文书收口五件**

- **来源**：远端 8 个新提交（`survey/35`～`39` + 补遗，2561 行）⇒ `git pull --ff-only` 已拉取（`0c0db24`）。
  ⭐ `survey/38` 标题就是「**交付主工作流马上做**」，`survey/35 §10` 是「工作排期表整理建议」，
  `survey/39` 是「阶段一任务清单与收口标准」—— 三份都直接冲我来的。
- ⭐ **排期已重排并只留一处**：台账 `### J. 工作排期表` 下新增 **`#### J-★ 2026-09-26 排期`**（六段：
  0 文书收口 / 1 **真机先行** / 2 最便宜的一刀 / 3 通道施工器第一刀 / 4 架构债 / 5 阶段一）；
  `§11 当前工作队列` **降级为历史归档（只读）**（`survey/35 §10` 建议 1：两张表并存 ⇒ `F2` 就是活例）。
  同时落地建议 2/3/4/6/7：**执行顺序单独一行**（id 不变）· **成本三段**（实现/验证/⭐客户端，
...
- **文书收口五件（0.3 轮，纯文档 + 一行注释）**：
  ...
  - `0.3` ✅ **`D-449`** 第一层收口条件（照 `D-430` 形状四段：判据 `J1`/`J2`/`J3` · 明确不收 · 只接受三种输入 ·
    解除条件四条）；⚠️ **两处 ⏳ 待你拍**（`J2` 是否含"会动/一次性目标"；"如实报不可维持"是否算 `J3`）。
  - `0.4` ✅ **`D-450`** 通道成品规格 `I1`/`I2`/`I3` + 补格额度（= `F2` 收编）—— 修掉勘测侧抓到的
    **三处口径不一致**（`HANDOVER` 写"已裁定 A"、`AI_DECISIONS` 无条目、台账 `1.4t` 仍写"待裁定"）。
  - `0.5` ✅ **`D-451`** `I5` 破坏面豁免口径 = **按格状态**（已完工格才受保护）。
- **两条 0 成本/低成本的下一轮开工项**（`survey/35 §9` 桶3 的三条先做 + 两条 0 成本）：
  `1.4u`（改一个字符串表达式）· `1.4w`（**归因口径通用化**，`survey/35 §9` 桶3-3 自称"最省力收益最大的一刀"，
  口径 `D-329` 现成）· `1.4x`（**沙砾挖掘期暂停**，照 Baritone `Movement.java:157-160`，≈20 行，不碰成本模型）·
  `1.4r`（安装点挪首 tick）· `K1` 定性质（**0 行代码**：拆"站位形态必做"vs"成本乘子可延后"）。
- ⚠️ **`survey/36` 的两条纠偏（必须记住）**：① **`1.4s` 可能可以不做了**（预防式 vs 治疗式）；
