...
>
> ### ⏳ 等你的：`D-350` 客户端验证（**用户可见行为变化**，压缩后请把结果给我）
> - **看什么**：开一个**区域任务**（`region_lumber`，现有命令/物品零参数）在有树的林地跑一段 ⇒
>   **在不缺苗的时候**，bot 也会主动去把**自己砍出来的落在地上的树苗**捡回来（以前只在"欠树且手里没苗"时才捡）。
> - **判据（日志）**：出现 `[Job] maintain 扫描判定 … → ENTER`（旧行为在这里恒为 `NO_DEFICIT`）；
>   出现 `[Job] maintain sweep 结束 … 实际入包≥1`；**不应**出现 `sweep_no_progress`（非必需零收获只应退避）。
> - **顺带观察**：`[Pickup] blocked item=oak_sapling … provenance=FOREIGN policy=ASK` 是否**明显变少**
>   （扫描期间有 `CollectGrants` SESSION 授权，被动拦截不该再是唯一结局）。
> - **我需要你给的**：用了哪个入口（命令/物品）+ **预期 vs 实际** + 复现频率 + `latest.log` 片段
>   （过滤 `[Job] maintain`、`[Pickup]`、`[CollectGrants]`）。
> - ⚠️ 若看到 bot **反复空跑去捡捡不到的东西** ⇒ 那是 `D-350` 的退避没生效，**立即告诉我**（是回归）。
>
> ### 环境坑（今天踩过，压缩后照这个查）
...
> 1. ⭐ 上面那次客户端验证（`D-350`）。
> 2. **17-② 小项**：启动前拒绝（`unimplemented` 实体目标 / 修路计划非法）**补记事件环**（口径 = 与"玩家停止"一致：**记、不叫 LLM**）。
> 3. **`check-all` 结果缓存**（源码指纹一致 ⇒ 复用上次绿 CORE，~10 min → ~1–2 min；缺失即真跑，不许静默复用）。
> 4. `D-351`（LLM 记忆/上下文）= **仅登记**：真需求；四道前置（入通道 / 记忆库形状 / ⭐快照预算（世界事实优先）/ ⭐写入口只能提议）；
>    **触发 A（推荐）= 决策层队列 `GoalRecord` 落地之后**（与 `history` 共用落盘+裁剪+读入快照）；知识库/本地配置/记忆库**三类必须分开**（已登记）。
> 5. `D-333`（守卫冻结）与 `D-337` 附注四（`FarWalkTask`）各自挂着**触发条件**；`RoadBuilder` 已标 🅿️ **陈旧未验证**（要完整复查再决定）。
> 6. `survey/22`+`21` 复读剩余：② 收集器过滤面 → **核实为不做**（已落台账 + 触发条件）；③ 已交付；⑤ **不做**（冻结，已补开封条件）；⑥ 已处置。
>
> ### 过程教训（会再犯的，写死）
