...
> - ⚠️ 本轮 jar **已包含 `D-378`/`D-379`**（都是夹具/闸门，**不影响**水中自救这条路径）；
>   但 `D-379` 会**改变采掘时的路线选择**（不再生成「破开一个没有地板的中间列」的破坏通行）
>   ⇒ 若你在同一轮里顺手挖矿，看到 `break_traverse_no_mid_support_fluid/_dry` 计数是**预期行为**。
>
