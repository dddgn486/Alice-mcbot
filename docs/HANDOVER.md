...
>
> **下一步**（压缩后从这里接；⭐ **2026-09-21 重新整理后的规划**，全文见
> `docs/reviews/2026-09-21-掉落物在洞里被瞬退.md` §0 + §8）：
>
> ⭐ **根因变了（第五轮日志 + 存档只读取证 + 代码直读，已定位到一行谓词）**：
> **图不完整 —— "脚位可通行 + 头位被挡"的格子在整张图里没有任何入边**（= 一格高夹缝的形状）：
> `TRAVERSE` 要 `canStandCentered`（含头位）⇒ 拒；`BREAK_AND_TRAVERSE` 不适用；
> `BREAK_AND_ENTER`（`SurfaceMovementProvider:169`）**只查脚位** ⇒ 早退不生成。
> 而 `BreakAndEnterExecution.collectBlockers:69-78` 本来就会把头位收进待破列表
> ⇒ **唯一拦路的是一行谓词**。对照 Baritone `MovementTraverse.java:57`（`positionsToBreak={to.above(), to}`）
> 与 `:109-118`（cost 里给 `y+1` 计价）⇒ **这是相对 Baritone 的内核缺口，此前从未登记**。
> 用户原话即答案：夹缝里的煤**"只挖一格方块就能走过去"** —— 那**一格**就是缺失的边要破的头位方块；
> 用户"**朝东**推一下"= 补最后 0.1 格（与 `botBox`/`inRange=false` 的几何反推一致）。
>
> **计划 = 两条轨道**（待用户拍板顺序）：
>
> **G 轨道（一致性；2026-09-21 用户裁定"这不是小事"）** —— 全文见
> `docs/reviews/2026-09-21-寻路核心语义一致性审计.md`（第一遍机械化普查）：
> - 事实：同一份准入契约在**三层各写一遍**（生成器 / `*ExecutionFactory` / `*Execution`，9 类 × 3 = 27 处），
>   91 个 `canWalkThrough(` 调用点，**无单一真相源、无"三者一致"门禁**；散文承诺语 10 处全无门禁；
>   **6 条架构红线只有 D-076 有可执行门禁，D-036 零门禁**——而本次缺口正是 D-036 意义上的能力缺失。
> - ⭐ **缺的是一条逆向不变式**：现有门禁只保证**健全性**（可规划 ⇒ 可执行），**没有完成性**
>   （执行器/物理允许 ⇒ 生成器必须给边）⇒ 完成性缺口**永远不会变红**，只能靠真人踩到。
> - **G1 谓词分层**：`canWalkThrough` → `cellPassable` + 新增 `bodyPassable(foot)`；
>   `kernel-predicates.py` 禁止"边生成器裸用单格谓词"（除非带 `.above()` 或 `// single-cell-ok:` 理由）
>   ⇒ 新增同类缺口**构建红**。
> - **G2 完备性差集** ⭐：独立**朴素 BFS**（不看生成器）vs 规划器（允许全集）⇒ 差集必须空或登记在案。
>   **唯一能发现未知缺口**的门禁。与 P0 一起做（P0 提供第一个可验证场景）。
> - **G3 红线↔门禁清单**：`AGENTS.md` 每条架构红线必须带 `[gate: 脚本]` 或 `[未门禁: 原因+复核触发]` ⇒ 缺标记构建红。
> - 三者都必须挂 `tools/check-all.sh`，**上线当天用基线/白名单只拦新增**，**严禁**做成"永不失败的警告"（= 静默绿）。
>
> **P 轨道（修复）**：
> - **P0 ⭐ 图补全**：`appendBreakAndEnter` 早退判据改成「脚位 **且** 头位都可通行」；夹具 = 数据包场景
>   「1 格高夹缝 + 我方掉落物」断言 `collected=1/1` + 规划器级 `REACHED` 且破的是头位；
>   反向对照 3 个（改回脚位必红 / 脚位被挡行为不变（`[BreakEnter] cleared 429/430/431` 是天然对照）/
>   真不可达仍如实 `unreachable`）。**不改授权面 ⇒ 不碰 `D-076`。**
> - **P1 记账**：`MiningPlanner.selectBestApproach` 把 `SEARCH_LIMIT` 当"本轮还没评价"
>   （不计入 `MAX_APPROACH_PLANS`、不并成 `no_reachable_candidate`、上抛成已有的 `search_incomplete`）
>   —— 防 A1 把"被拒"变成"永久跳过矿石"（本轮铁证：25 次拒绝全 `已发起=1`；`436,82,229` 从未被挖却 `already_attempted`）。
> - **P2 拾取加固**（P0 修好后本次场景应自然解决）：判据统一 / 失败可 `reanchor` / 朝物品走一步 / `best==null` 补日志。
> - **P3 复测 B**：P0 后**只用 1× 预算**重跑 `single:mine_reach_probe`（≈73 s）⇒ 若可达则改 `D-373` 结论
>   （"预算不够" → "图缺口迫使预算当拐杖"）。**我原计划的"直挖边直方图"被 P0 取代**（同一个问题，更前一层）。
> - ✅ **真机第五轮 A1 止血达成**（12:34–12:35）：`[SearchTick]` 25 次拒绝全 `已发起=1` · 超预算搜索每 tick ≤1 次
>   （第四轮 376 条/12 波）· **0.4 → ≈17.9 TPS**（`tick=11257→11580` = 323 tick / 18 s）· 挖掘仍在推进。
>
> **客户端 jar `9fb54dad216c1d8e`（A1+A2）仍是当前客户端状态**；P0/P1 需要新 jar。
>
> ## 现状速览（压缩后以此为事实，细节见下列路径，不要从对话里找）
> **源码状态**：`D-373`（A1/A2 + B 实验）已实现/验证/推送；工作树干净；远端只有 `github`。
> **客户端 jar `9fb54dad216c1d8e`（A1+A2 已同步）**。
> **A1 = `SearchTickBudget`**（每 tick 搜索总账，形状照 `WriteBudget`）：唯一强制点 `CorePathPlanner.plan`；
> 唯一搜索出口 `searchAndRecord`；超限 ⇒ 诚实 `SEARCH_LIMIT` + `tick_search_budget_exhausted`；
> **主判据 = "烧预算的搜索次数"（默认 1）**，兜底 = 总毫秒 400ms / 次数 32；
> 选择期成本场只记毫秒不吃主判据额度。**A2 = `MiningPlanner.MAX_APPROACH_PLANS=3`**（有界穷举，检查必须在 `planPath(` 之前且为 `break`）。
> **病灶定量（真机第四轮日志复算）**：单 tick 连发 **13** 次全预算搜索 ⇒ 2.4 s/tick ⇒ **0.4 TPS 持续 57.6 s**；
> `[Search] 超 tick 预算` **376 条**（最大一波 336 条）；`tick 1588` 之后 60 s 无 PROGRESS；
> `NO_PROGRESS_WINDOW_TICKS` 生产默认 **0（关）**。**A2 的代价已实测**：13 个候选里几何最近的 2 个连 2M 节点都不可达
> （第 3 个才可达）⇒ "由近到远"没有预测力，cap=3 这次是**运气**。
> **B 实验结论**：**解释甲成立（能到，不是"没有路"）**，但要 **1.9M 节点（95×）/ 单次 14.9 s / 88 段路径**
> ⇒ **不是"调大预算就能用"**；`survey/24 §2.3`（octile 对"目标在下方"几乎无方向信息）与 `survey/25` 的 ③鱼骨获得实测支持。
> **验证等级**：A1/A2 = `IMPLEMENTED`+`COMPILES`+`SERVER_TESTED`（CORE 119 PASS，仅剩已知 `lumber_job`；2 注入全红）；
> **尚未 `WINDOWS_CLIENT`**。门禁 `pass=18 failed=0`。
>
> ---
>
> ## 上一版断点（保留：勘测报告清单与四轮事实，供追溯）
>
> > 用户原话：「新的**勘测报告**出来，**前后修正有点多**，**先一口气读完，再整理信息**，
> > 然后针对现状给出**后续的规划**」。
> > ⇒ 压缩后按此顺序做：**① 完整通读新勘测报告（不要碎片化浏览）→ ② 整理信息 → ③ 针对现状给规划**。
> > **要读的清单（2026-09-21 由远端取回，本地=远端=`1e4e922`）**：
> > - `survey/24-搜索预算与水平探索设想-20260921.md`（提交语：搜索超预算**定性（记账缺失）**+
> >   每 tick 预算/动作守卫建议 + Baritone 对照 + **水平探索＝目标预筛**）
> > - `survey/25-挖矿三路线-跟随探洞鱼骨-20260921.md`（提交语：挖矿三路线 + **四待拍板项** +
> >   **口径更正：世界修改记账仅限保护区及其子类**）
> > - `survey/26-车万女仆任务形态勘查与锚点模式-20260921.md`（提交语：源码级勘查 + 锚点=一套任务的取值 +
> >   凝矿兰可能是第一个落点；含"补遗一：凝矿兰查证 11x7x11/每100tick一块/17500魔力/概率表 + 低频轮询 + 纪律D1-D3"）
> > - 另 `survey/README.md`（索引，51 KB）
> > ⚠️ 上面只是**提交语摘要**，**不许当正文结论用**；必须**完整通读**后再整理。
> > ⚠️ 其中"**世界修改记账仅限保护区及其子类**"可能与昨日 `D-372` 的记账口径相关 ⇒ 读到那节要重点比对。
> >
> > ## 现状速览（压缩后以此为事实，细节见下列路径，不要从对话里找）
