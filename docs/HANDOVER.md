...
>
> # ▶▶ 压缩后**先读这 6 行**（2026-09-21 13:4x；工作树干净，HEAD=`612cbcc`，本地=远端）
> - 客户端 jar = `2e96d17a22a87952…`（P0+G1+G2+G3+P1 已同步）；源码已镜像 Windows。
> - **第六轮真机已完成**：拾取**已修好**（6/7 簇 `collected=N/N`）；但**绕远仍在**，根因已定位 =
>   **锚点静默回退到不可站的物品自身格**（1 格高夹缝）⇒ 一条 **19 段 / 破 6 格 / ≈10 s** 的挖掘回环
>   ⇒ 烧满 200 tick 簇预算 ⇒ 最坏 `collected=0/13`。证据与 4 条修法见
>   `docs/reviews/2026-09-21-掉落物在洞里被瞬退.md` **§10**（§0=图缺口根因，§8=P0–P3 原规划）。
> - ⏳ **等用户拍板（上一轮已问、未答）**：① **现在做 P2 吗**（拾取加固：`best==null` 不许静默 +
>   `withinPickupReach` 统一真实谓词 + 收集目标必须可站 + 夹缝夹具 `collected=1/1` 且破块 ≤1）？
>   ② 要不要一起做**决策层信号**（收集阶段耗时/破块超阈值 ⇒ 事件，否则决策层看不见绕远）？
> - ⚠️ **P3 重做受阻**：`新的世界 (2)` 已被第六轮覆盖存盘（13:31）⇒ round-4 地形不存在；
>   要么用户给冻结副本，要么改成**当场建场景**的深矿几何（推荐）。探针已加读数有效性判据。
> - 其余待办：`canWalkThrough` 全局改名（**建议不做**，117 处/37 文件）；3 条「未门禁」红线。
>
