...
>    ⚠️ **生产未接线**，目前只有门禁用它）。
>    ⚠️⚠️ **下半段 = 立刻要做的执行项（`D-337`）**：门禁沿路采样实测 —— 粗目标不被前置拒后，
>    **搜索在扩展时读了未加载区块的方块、把 224→384 的 14 个区块同步加载了进来**
>    （加载半径实测仅 ≈192 格）⇒ ① 违反红线 `D-132`；② 真世界里就是 tick 线程上的同步区块生成/磁盘 I/O
>    ⇒ **「远距离一规划就卡」的又一层真因**（与 `D-331` 同病理）。**此前没暴露是因为精确目标的
>    `GOAL_NOT_LOADED` 前置守卫替真正的加载守卫背了锅**。
>    **修法**：候选位置 `canWalk*` 判定**在读方块之前**先问 `chunkLoaded`，未加载 ⇒ 视为**不可通行**
>    （保守，且正是「到边界就交前缀」的期望语义）⇒ 修完：把 `far_path_bench` 那条断言**改回红绿判据** →
>    复跑 `single:pathing` → **收口 CORE**。
