...
>   （commit `106d65f`；jar 本地=镜像=客户端 `738fa4ec…`；`git status` 干净）。
>
> **▶▶▶ 下一件事 = 第 2 项：给 `survey/27 §3 #1` 一个能红的收口判据**
> - **要证的事**：`P0`（`appendBreakAndEnter` 改查脚位**且**头位）之后，「88 段绕远」是否消失？
>   现状 = `G2` 强烈暗示、`P3` 复测读数无效（真机存档被覆盖 ⇒ 起点 `432,83,428=air` ⇒ 一次真搜索都没发生，
>   探针却输出 `reproduced=true`；见 `docs/reviews/2026-09-21-B-深矿可达性判据实验.md` §P3）。
> - **做法（勘测侧建议② + 我方补充：做成规划级夹具，别再来 73 s 探针）**：当场建深矿几何（孤立点、自建自还原）
>   + 同一批候选跑**两遍**：修复前谓词（注入式临时回退）⇒ 期望 `PARTIAL`/`SEARCH_LIMIT`；
>   修复后谓词 ⇒ 期望小预算内 `REACHED` 且段数接近理论（≈10，而非 88）。读数为 `status/reached/movements/nodes`。
>   顺带量一次勘测侧建议⑤（「周围已有同类矿」是不是只是**更便宜**的混淆项：同批候选的 cost vs 可达性）。
> - **纪律**：夹具必须**自建场景**（`survey/27` 建议①：真机存档必被下一轮覆盖 ⇒ 依赖它 = 结构性失效）；
>   测量型工具必须先断言几何前提（建议③，已由 `MineReachProbeTask` 的"读数无效即响亮失败"落地）。
>
