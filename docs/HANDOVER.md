...
- **本断点已同步的 jar**：文件 `eaf99f17fd1b283d1bd5218e9b4b0b3a7d02c84a885979692685e680b9bd3c11`、**内容摘要 `240aa32c82679942706e23791ae340b25284bcd2d2486be758dbfafb6f667bd6`（761 条目，含 R-2 Phase 1b 编排器）**（2026-09-17；⚠️ **jar 不可字节复现** ⇒ 判「客户端是不是这版」只能用内容摘要：`tools/jar-content-hash.sh --compare <构建产物> <客户端 mods 里的 jar>`；文件哈希只表示「当时同步的是这个文件」）（**含 D-226…D-277 + 队列①②③④⑤⑦ + S-6/S-9(观测+消费) + F1/F2/F3/F4 + 死亡第 1 步(端到端已验)**，客户端与 Windows 仓库内容一致；门禁 **16 PASS + 1 预期 WARN**；CORE **48 步** / FULL **58** 项）**含 D-226…D-277 + 队列①②③④⑤⑦ + S-6/S-9(观测+消费) + F1/F2/F3/F4 + 死亡第 1 步(端到端已验)**，客户端与 Windows 仓库同哈希；门禁 **16 PASS + 1 预期 WARN**；CORE **43 步** / FULL **52** 项）
