...
>    `ALICE_HEADLESS=1 check-all` = **17 PASS / 0 WARN / 0 FAIL**（CORE 51/51）。⚠️ **待确认**：四邻腐蚀下
>    1×1~3×2 区块**没有**内部区块 ⇒ 小基地上"向中心靠"看不到效果；⛔ "改八邻"**已实测证伪**
>    （2×2 两种口径都空；矩形两者相同；8 邻在非矩形上更严）⇒ 出路 = **质心退化** 或 **块级内缩 K 格**。
>    **下一件** = `§5.12` 第 2 件（返程判据切优先级链 + 重写 `safe_return` 断言）。
