...
**阶段 3-B（模组机器适配）已收口；之后做过一次全项目完成度审查，并按 `T0-a → T0-b → T1` 次序落地，
T1 已通过第十七轮客户端验证。下一步是 T2（无头回归）或 T3（接第 3 个模组前冻结数据模型），等用户拍板。**

## 2. 刚做完的（全部已推送，HEAD = `e08e535`）
...
- **R2-残**：R-2（生产入口只搬运不发料）**运行期路径没被走到** —— 第十七轮 LLM 全程未被触发 ⇒ 没起过 Job
  ⇒ `[Job] 生产入口只搬运不发料` 0 条。**只有 `COMPILES` + 门禁断言。**
  补法（10 秒）：手动右键 `alice:goal_director` 或 `alice:job_launcher`。
...
- **T2 无头回归**：**未开始**。`./gradlew runServer` 已实测 `Done (3.675s)!`、Gametest 命名空间已启用。
- **T3 模组 #3 数据模型**：**未开始**。`create` / `ExtendedCrafting` **已在客户端 `mods/` 里**且会被**静默读错**。

## 4. 待用户拍板（恢复后**先问这个**）

1. **先补 R-2 的 10 秒运行期证据**，还是**直接进 T2**？
2. 若进 T2：按已约好的闸门 —— **先做「1 个样板 + 覆盖度评估」**，
   量出"30 步里能自动判红的 = N"，**N < 8 就不铺开**。
3. T3 的时机：`MachineMap` 的 site-kind + 1 方块↔N 类型（修的是**已存在的错事实**，不必等新模组）。

**上下文/文档纪律（AGENTS.md 已机器化，不用背）**：`AGENTS.md + PLAYBOOK + STATE ≤ 1476 行`
（当前 **1472**，余额 4）；新增散文规则必须同时删旧的；任何新验证手段必须挂在
`./gradlew build` 或 `tools/check-all.sh` 上。
...
- **本断点已同步的 jar**：`abed83d244c080d212cb935373f775b962efa4b5f698ac0f21842fb6ee323291`（三处一致）。
- **离线门禁（改完就跑这一条）**：`bash tools/check-all.sh`（**9 道**，三态 PASS/WARN/FAIL；
