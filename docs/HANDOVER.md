...
## 1. 一句话现状（2026-09-14 收工）

**`T0-a → T0-b → T1 → T2` 已全部落地（T2 已客户端验证）。
T3 八步已走完七步：步骤 1 / B3a / A / A2 / C / (A) / **B4** 全部落地并实测；
**只剩"接第 3 个模组本身"（`MachineMap` 加 19 行）与 B3b（等第 3 个模组同期做）。**

**今天 6 个提交（按序）**：`bc2b1aa`（步骤 1）→ `a96ce0a`（B3a）→ `864ba9a`（步骤 A）→
`59857e4`（A2+C）→ `239b3cb`（(A) 落地同步）→ `e23869d`（B4 + (甲)）。
...
**今天又做了两件**（见 §2c）：① 采纳 `survey/07` §3.5 的**模组停止规则**（D-219，已落地）；
② 按修复纪律**先核实**了 `survey/07` 的行动清单 ⇒ **5 条里 2.5 条已过时**，不能照单执行。

**待你拍板的问题**（§4）：① `pathing` 子用例的"起点必填契约"；② 夹具分离要不要做
（**只做包移动**，不要"移出发布 jar" —— 见 §2c 的范围修正）；③ 要不要转向决策层
（`survey/04`–`06` 需先过同样的核实）。
