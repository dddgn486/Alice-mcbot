...
## §7 用户提案（2026-09-19）：**任务队列 + 持久终态 + 历史落盘**（记录 + 评估）

> ⭐ **本节是摘要。完整形态（数据模型 / 状态机 / 触发点 / 逐帧走查 / 防循环三道 / 兜底矩阵 / 验收判据）
> 见 `docs/DECISION_LAYER_FINAL_FORM.md`** —— 状态：**设计定稿 · 未实现 · 未验收**（2026-09-19 落档）。

### §7.1 提案原文要点（用户口述，AI 整理，**不得改写语义**）

1. **LLM 维护一个任务队列**，队列含三段：**当前任务** / **后续任务列** / **最终任务目标（最终状态）**。
2. **最终态是持久目标**：决定做完一堆任务后 bot 最后应处于什么状态 —— 可以是**待机**，可以是**一直持续砍树**，**由玩家决定**。
3. **实际做过哪些任务、什么顺序，要落文件** ⇒ 让 LLM **不遗忘已经做了什么**。
4. 场景（用户原例，砍树）：队列里只有"区域砍伐"这一条**默认持续任务**时，**没有失败/结束就不触发 LLM**；
   斧子耐久没了 ⇒ LLM 依决策手册/系统提示词/知识库，**向队列插入"获取工具"任务**，并**先问玩家**是否允许去
   **指定的 bot 任务资源容器**取工具；
   - **不允许** ⇒ **结束任务队列**（区域砍伐没有进行条件），没有其它默认终态任务 ⇒ **回归位点待机**；
     也可以设定"待机前做什么"（卸货 / 未完成任务收尾）；
   - **允许** ⇒ 走过去取；**取不到** ⇒ 再问是否允许**搜索材料自制工具**；不允许 ⇒ 同上；允许 ⇒ 插入若干任务
     （材料获取 / 制作），**由 LLM 依认知自行编排**；
   - 直到**达到终态**或**终态不可达 ⇒ 待机**，此后 **LLM 不再被触发**。
5. 用户提出的关键问题：**LLM 到底在什么时候被触发做一次队列管理**？以及**玩家交互怎么设计**（"可能就不是一个单纯的任务表单"）。

### §7.2 现有积木（哪些已经能承载，不必新造）
...
| 权限兜底（谁能在哪写） | `WritePolicyMatrix` 等级阶梯 + `ZoneAuthority`（5 处消费）+ `WriteBudget` + 账本 |
| 安全兜底（不依赖 LLM） | 维生中断 / `SafeReturnTask`（D-327）/ 站定 |

### §7.3 AI 主张的契约草案（**要"能保证"就必须有这三样**）
...
2. **不变量破裂**（`TOOL_LOW`/`NO_PROGRESS`/背包满/容器未注册/区域条件不满足）——每类带**冷却**；
3. **升级**（同一条目失败 ≥N / 超预算 / 请示超时）；
4. **玩家指令**（自然语言目标 / 手动触发 / `instruct`）。
⇒ **任务正常推进时零调用**；`MAINTAIN` 稳态可以几小时不叫它。
...
③ **回执/历史**（完成条目进 JSONL + `GoalRecord.history`，失败带码）。

### §7.4 风险与对策

| 风险 | 对策 |
|---|---|
| **制度化循环**（"有终态"变成永远试的借口） | 进展契约 + `maxAttempts` + 总预算 + 失败升级（D 条） |
| **队列陈旧**（世界变了，队列还按旧的走） | 每条目执行前**重验前置条件**（同 `CapabilityGate` 执行期复验哲学）；失败即作废/重规划，不硬跑 |
| **交互爆炸**（每次不变量破裂都问玩家） | **默认策略**（`AUTO/NOTIFY/ASK`，玩家可按不变量类别设默认）——即 §6.4"别让没批准变成卡死" |
| LLM 不在场时队列卡死 | 队列执行**不依赖 LLM**；只有"需要新条目/升级"才叫它；叫不到 ⇒ 按 `onFail` 收尾 ⇒ 待机 |

### §7.5 待用户裁定（三问）

1. **先做哪一刀**：① 只做 `MAINTAIN` 单目标 + 两条链（斧坏取工具 / 欠树收苗补种），先跑通"只在需要时叫 LLM"；
   还是 ② 先做**通用队列框架**（`GoalRecord` + `TaskSpec` + 触发点），链随后补？
2. **请示默认档**：是否接受"**按不变量类别设默认策略**"（AUTO/NOTIFY/ASK），而不是每次都由玩家回答？
3. **终态两型**（`REACH_STATE` / `MAINTAIN`）是否按 §7.3-B 区分？
