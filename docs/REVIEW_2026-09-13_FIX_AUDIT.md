...
3. ✅ ⑤ **终态幂等集中执行点**：电池对每个终态步任务补 tick 两次并打 `idempotent=`（D-177 ⑤）。
   **客户端已验证（12:42）**：22/22 个终态步全部 `idempotent=true`，0 次违规。
4. ✅ ⑦ **客户端已复测通过（12:42）**：四个 exec 用例全部 `foreignOk=true(另有残留1件不计入)`
   ⇒ D-168"残留不计入"分支**在全部用例上被真实执行并通过**（此前唯一"只靠推理"的断言已闭环）。
   期间修掉一处**我自己的**播种点错误（`start+Z1` 落在 target 为中心的测量盒外）。
