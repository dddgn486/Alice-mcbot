...
| **S3 能力闸门** | 新类型进**单一映射处**（现为 `RecipeDump.stationFor`），让"查询"知道去哪台机器；缺料/无站点仍如实拒绝 | 映射表 + 闸门用例 | 正例给路线、负例给码 | 零 |
| **S4 单机最小闭环** | 一台机器 + 一个配方：放料 → 等 → 取产物 | 夹具 + 场景 | **只看世界事实**（产物 +N、材料 −N、机器内不留东西） | 需显式授权 + 预算 |
