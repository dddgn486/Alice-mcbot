...
| **S3 能力闸门** | 新类型进**单一映射处** = `decision/MachineMap.java`（**唯一真源**：`typeId → 方块/菜单/能力`；`RecipeDump.stationFor` 只判"需不需要机器"，**不回答"哪台"**），让"查询"知道去哪台机器；无站点/未登记**如实回落**、缺料仍如实拒绝 | `MachineMap` + 生成视图 `docs/MACHINE_MAP.csv` + 双向防漂移 `tools/check-machine-map.sh` | 表 == 上游全部类型（**按命名空间分别双向断言**：Mekanism 27 = 23 有站点 + 4 无站点；Thermal 32 = 26 有站点 + 6 无站点；共 59 行）；写错 id / 删行 ⇒ 报红（已反向验证）；机器路线 `station` = 方块 id | 零（今天 0 行 `EXECUTABLE`） |
