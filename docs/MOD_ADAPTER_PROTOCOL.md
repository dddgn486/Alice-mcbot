...
**✅ 本判据的可执行部分（T3/B4，2026-09-14）**：上面三条是**散文**，靠人记得；现在它有一条**能红的**断言
挂在 `tools/machine-map.py --check`（即门禁 `check-machine-map`）上：**上游访问器能力登记的双向对账**
（`CAPABILITY_GROUPS`）——
① 适配文件（`CAPABILITY_FILES`）里的访问器形字面量（`"getXxx"`/`"isXxx"`/`"hasXxx"`，**先剥注释**）
必须逐条声明；② 声明的每个名字必须真的被用；③ 有 jar 的组还要在**上游 jar 的 class 字节**里找得到
（缺 jar ⇒ `unverified`，不记成通过）。当前 `capabilities=31 组=5`，上游已取证 4/5（`forge` 的
`IItemHandler` 不随模组目录分发 ⇒ 结构上不做 jar 取证）。负测试：改一个名字 ⇒ **3 条独立报错**。
证据与两个洞（`grep` 分不出注释；"0 处字面量"≠通用）见
`docs/reviews/2026-09-14-B4能力登记与起步位置确定性.md` §1。

⚠️ **旧取证方式作废**：原先"各类里 `mekanism` 字面量出现次数（`grep -c`）"**不再作为判据**
（它把注释算成依赖、又漏掉写死的上游访问器名）。§6 表的"证据"一栏仍可保留字面量计数，
但**它只是说明**，判据以本断言为准。
