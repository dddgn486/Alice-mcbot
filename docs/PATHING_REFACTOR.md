...
4. **安全地面原语与成本模型**：复用现有 `MovementHelper`/A* 的支撑和碰撞语义，只增加成熟方案缺失的适配层。当前受限曲面成本固定为：水平（直走/对角）=1、上阶=2、侧向下阶=1、原地向下=3；因对角同价，曼哈顿 heuristic 会高估，`AStarPathfinder` 使用零 heuristic 的 Dijkstra 出队顺序，并在发现较低成本时重开 closed 节点，保证此成本模型下的最短路径。headless `PATHING_REGRESSION` 在小世界中断言等高对角、上阶、下阶与“上后下比低层绕行便宜”的成本/路径段，不作为假人物理证据。再由 `SurfacePathfinder` 输出连续脚位段后做转向、单格上台阶和下台阶。落地验收必须比较实体实际脚底与目标脚位下方 `VoxelShape` 的支撑顶面，不能只比较 `blockPosition()` 的整数 Y；这兼容下半砖、台阶等非整格碰撞形状，同时不把障碍边缘接触误判为到达。单目标直线探针不得自行猜测或跨越障碍；每个路径段在进入和完成后均需验证脚位、支撑和落地，复杂碰撞交给客户端测试。
