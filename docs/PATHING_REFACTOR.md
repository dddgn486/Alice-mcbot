...
4. **安全地面原语与成本模型**：复用现有 `MovementHelper`/A* 的支撑和碰撞语义，只增加成熟方案缺失的适配层。当前受限曲面成本固定为：水平（直走/对角）=1、上阶=2、侧向下阶=1、原地向下=3；因对角同价，曼哈顿 heuristic 会高估，`AStarPathfinder` 使用零 heuristic 的 Dijkstra 出队顺序，并在发现较低成本时重开 closed 节点，保证此成本模型下的最短路径。水平对角必须同时通过两侧格和连续玩家扫掠空间；一格下降同样检查玩家尺寸的连续 AABB 扫掠，只忽略起点/终点脚下支撑接触。等主成本相同时，open set 稳定优先少转弯，再按坐标消除残余平局。headless `PATHING_REGRESSION` 现在断言等高对角、上阶、下阶、高差替代、对角侧向阻塞、下降扫掠阻塞和直线择优，不作为假人物理证据。`SoftPathProbeTask` 对重验失败返回 `soft_path_blocked_segment`，异常路径关系仍返回 `soft_path_invalid_segment`。下降段到达水平半径后进入显式 settle 状态，逐 tick 记录 `actualY/supportTopY/onGround/settleTicks`，只有实际脚位、目标支撑顶面与 `onGround` 同时成立才完成；超时返回 `soft_path_unsettled`，不放宽校验。专项 headless 回归覆盖整块边缘下降、下半砖顶面 `59.5` 和无支撑拒绝。再由 `SurfacePathfinder` 输出连续脚位段后做转向、单格上台阶和下台阶。落地验收必须比较实体实际脚底与目标脚位下方 `VoxelShape` 的支撑顶面，不能只比较 `blockPosition()` 的整数 Y；这兼容下半砖、台阶等非整格碰撞形状，同时不把障碍边缘接触误判为到达。单目标直线探针不得自行猜测或跨越障碍；每个路径段在进入和完成后均需验证脚位、支撑和落地，复杂碰撞交给客户端测试。
