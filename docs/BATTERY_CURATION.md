...
- ⭐ `fishbone_slice2`（EXTRA，`D-437`+`D-439`+`D-441`+`D-442`，切片 2 第二步 + **切片 4**）：（`D-442` 追加 1 条「单向爬升陷阱」：走廊上方 2 格必须能回到起点且用到 `FALL` —— 真机 `return_failed` 的离线复现；红臂 R2 = 拿掉 `FALL` ⇒ 红）（`D-441` 追加 2 条：真机入口 `originFor` **= bot 自己的脚位** + 反证「抽掉地板 ⇒ 必须 `null`」⇒ 入口连同「站不住就拒绝开工」一起被钉住；红臂 R1 = 去掉那条判据 ⇒ 红）**支巷 + 支巷放弃 + 露头矿顺手挖 + 收集 + 矿簇追挖**的行为级判据 —— **四臂**一次跑完，场景 = **整盒填实心石**（⇒「模板外改动 = 0」是**逐格枚举**，不是"没超预算"）：① 主巷 6 + 2 条支巷 ×3 格 ⇒ `C1`（24 格全空）/ `C3`（零 `SEARCH_LIMIT`）/ `C4` + **每条支巷各退回主巷一次**；② 第 1 条支巷第 2 格换**基岩** ⇒ ⭐**新终态 `template_complete_spurs_abandoned=<n>` + DONE**（`§10.2` 用户裁定的两档：子巷放弃 / 主巷如实失败），**基岩格零改动**且主巷与第 2 条支巷照常挖完；③ 顶棚 3 铁矿 + 1 诱饵 ⇒ `C2`（**夹具独立清点**背包原铁，不信任务自报）/ `C5`（破坏数精确 = 模板 + 露头矿）/ 顺手挖**不带位移** / 诱饵原封不动 + 条件①②③**逐条隔离**判据。④ **臂④ `ORE_VEIN_CHASE`（`D-439` + `D-440`）**：整盒实心石 + 一条伸进主巷南侧壁 3 格的矿脉（前两列两格高）+ ⭐ **一条「地板矿脉」**（支巷第 1 格的正下方 —— 矿簇就在地板里；`D-440`：挖空之后**规划器自己补一块再走**，`[Pillar] placed` 补回的正是那一格，`spursAbandoned=0`、`places=1`）⇒ ⭐ **矿脉 7/7 格已不再是矿**（`oreMined=5 collectedProducts=5 auditOutsideExpected=0`）—— 这是「矿簇只挖贴壁那一层」的**正面判据**（红臂 C：暴露面退回只有模板格 ⇒ `2/5 failures=5`；红臂 D：拿掉破矿后扫邻域 ⇒ 同样红）。⚠️ 与 `fishbone_slice1` **完全分开**（`D-437 §一`：老绿臂一字不动）⇒ 两片可独立跑、独立红。判据落在**世界事实**上（逐格期望表 + `WriteAudit` + 背包实物）。自建孤立石盒 ⇒ 不依赖真机存档、也不依赖前序模块。
...
| 日期 | CORE | FULL | 说明 |
|---|---|---|---|
| 2026-09-26 | **42** | **102** | ⭐ **`1.4x` 重力方块（沙砾/沙）的「挖掘期暂停」**（`survey/35 §9` 桶3-4 = 缺口清单 `K1` **前半**）：目标格里有在飞的 `FallingBlockEntity` ⇒ 本 tick **不挖**（逐字对照 Baritone `Movement.java:157-160`）。病灶 = 挖通一格后沙砾 **2 tick** 才落完 ⇒ 落体把刚挖的格填回、回程路当场消失。新步 `mine_falling_pause`（**MAIN，进 CORE**：自建 11×11×6 石盒 + 3 次真破坏 ≈ 150 tick ⇒ 便宜；且它护的行为**静态门禁咬不到** —— `pausedForFallingBlock` 只在执行期产生 ⇒ 放 EXTRA 等于"下次退化没人知道"，理由同 `break_enter_head_blocked`）。四臂 = 暂停（**直接证据** `pausedForFallingBlock` + 窗口内一格没挖动）/ 恢复（清掉落体后必须挖穿 ⇒ 暂停不许永久）/ 两个**负对照**（落体在目标上方 2 格、侧向 2 格 ⇒ 不许暂停）。⇒ BASELINE 15 + MAIN **27** + EXTRA 60 = **102**，CORE **42**。 |
| 2026-09-26 | 43 | 103 | ⭐ `fluid_mine` 进 MAIN（CORE 42→43）：**挖到水 ⇒ 硬拒** —— 接进 `FluidRiskPolicy` 唯一判据（岩浆那条同处），鱼骨既有两档处置自动生效。同时把此前**只挂游戏内物品、没进电池**的流体夹具接进回归，并补水两档臂（上方 / 水平邻格）+ 正对照。 |
