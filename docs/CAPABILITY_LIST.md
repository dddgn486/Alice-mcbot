...
| 自检模块 | 21 | §4.1 |
| 自检步（能力清单用它证明自己） | 101 = BASELINE 15 + MAIN 26 + EXTRA 60（**CORE 实跑 41**） | §4.2 |
| 机器类型（上游已登记） | 59 行 | §5 |
| 玩家能调的开关 | 3 | §6 |
| 底线（**任何玩家入口都不许出现**） | 3 | §7 |
...
### §4.2 电池步（101 步；`#` = CORE 运行序，`—` = 只在 FULL 跑）
...
| — | `channel_place_guard` | EXTRA | pathing |
| — | `corridor_skip_predicate` | EXTRA | pathing |
