...
> ⚠️ **2026-09-20 起**：**任务设计**部分已由 **`docs/MINE_TASK_DESIGN.md`**（阶段 0 设计文档，
> 上游 `D-329` 路线图）接替 —— 那份写**扫描契约 / 作业区意图 / 通道能力 / 失败归因 / 每阶段判据**。
> 本文件保留为**迁移实施记录**（批次 1/1.5/4 已落地：`BotMiner` 已删、破坏走 `BlockBreakSession`；
> §6「寻路红线」的表述仍是**现行**判据，改动要同步两处）。
