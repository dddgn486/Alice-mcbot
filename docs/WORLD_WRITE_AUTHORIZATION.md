...
| **A14** | ⭐ **鱼骨"补一块再走"**（`PathRequest.withPlacement` + `FishboneJob.walkRequest`，`D-440`；`D-442` 补 `FALL`） | `PLACE_STEP_AND_TRAVERSE` 的 **`to.below()`**（目标格正下方补一块再踩上去）+ `PILLAR`（跳起在**自己脚下**补一块）—— **只放不拆**：集合刻意**不含** `BREAK_*` / `DOWNWARD`（能修路，不能开路）。⚠️ `D-442`：本集合**另含 `FALL`**（落差 2~3 格往下），但 `FALL` 是**纯通行、不改世界** ⇒ **不扩大本行的写入权限**；加它的原因是"只有 `PILLAR` 会让一次爬升变成**单向**"（真机实测 `return_failed`，见 `D-442`） | `FishboneJob.placeBudget()` = **按形状推导**：`max(8, advanceCells/10)`（真机默认 404 单元 ⇒ 40 块）；用完**如实退回** `PathRequest.of` 纯通行（并打 `place_budget_exhausted` 一次）；另受执行期 `WriteBudget`（`D-106`） | `fishbone` / `fishbone-spur` / `fishbone-chase` / `fishbone-return` | `STEP_PLACEMENT` / `SUPPORT_PLACEMENT`（由放置原语决定） | ✅ `WriteAudit`（`action=place`，按 `requester=fishbone` 计数）+ 夹具断言"放置格 ⊆ 矿脉地板格 ∪ 其下方" |
