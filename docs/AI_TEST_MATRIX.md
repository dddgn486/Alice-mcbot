...
| D-116 收集复用加高（树冠掉落物） | `/function alice_test:lumber_course` → 右键 `alice:lumber_job`；另跑 `alice:mine_regression` | 零参数右键 | 掉落物停在**头顶树冠**够不到时，① 就地扫尾用"原地加高"搭上去拿（日志 `[CollectDrops] gain_start/gain_done`），其放置由随后的建拆同权收回；③ 落地扫尾不给加高 | `SERVER_TESTED`（2026-09-11 22:44 / 22:46 两轮：**① 被静默跳过** —— 零 `sweep_up_start`，末根破完 150 ms 直接 `restore_start`（②），结算 `28,64,208:product_not_collected gained=6/7`。根因两处：① 用 job 全局 `ticks` 当阶段预算（首帧即超时，且该分支无日志）；`collectPhase()` 收完不置空 `collector` ⇒ 下一棵树的 ① 把上一棵的僵尸任务当"已开工"。已修，见 D-116 附注），`待复测`（判别点：① 开工日志的 `live_drops=N`） |
