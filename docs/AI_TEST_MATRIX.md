...
| T6 离线可规划性自检（D-125） | 本机 `tools/check-scene-connectivity.py --all`（+ `--selftest`） | 命令行（无需客户端） | 每个原木/矿石目标至少有一个**可达**的合法站位；`expect_gap` 场景必须仍不可达；依赖破坏/放置/加高的场景只报统计不判目标 | `本机自证`（2026-09-12）：`--selftest` 双向断言通过；`--all` 22 场景零假报；对 `a5901f5` 旧版本文件复跑 ⇒ 9 目标里 **8 个判无站位**（含 `19,64,213`）——即在离线复现并拦下 D-117 |
