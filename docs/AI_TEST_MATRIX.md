...
| WalkTo 迁移（D-060） | `alice:walk_to_runner` + `/function alice_test:walk_course` | 普通右键 | `[WalkTo] SUMMARY walk_flat=PASS walk_over_wall=PASS walk_unreachable=PASS walk_unsafe=PASS`（新内核规划+执行；失败码保持 legacy 语义，SEARCH_LIMIT ≠ UNREACHABLE） | 待测（D-060） |
