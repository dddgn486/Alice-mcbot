...
| 路线偏好（Q7 验收） | `alice:pathing_dip_route` + `/function alice_test:dip_course` | 普通右键 | 聊天/日志 `first=TRAVERSE movements=4`（成本按实测标定后）；`first=DESCEND movements=2` 为标定前基线 | 待测（先采基线，Q7 落地后复测） |
