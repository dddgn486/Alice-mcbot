...
| 挖掘动作/收集重写（D-071/D-072/D-073） | `/function alice_test:scene_a` + 右键 `alice:mining_scene_tester`；伐木物品应提示已禁用 | 右键 | `[MineRunner] walk_start/break_start/done`；挖完后 `[CollectDrops] SUMMARY collected=n/m`；`alice:auto_lumberer`/`alice:lumber_planner` 只输出"已禁用" | 待测（D-071/D-072） |
