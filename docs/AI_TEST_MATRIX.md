...
| **串联回归（常规入口）** | `alice:pathing_regression` + `/function alice_test:pathing_course` | 普通右键一次 | **11 项**依次自动建地形并断言（执行 5 / 拒绝 3 / 路线偏好 1 / 重规划 1 / 重同步 1）；聊天/日志 `[Regression] SUMMARY ...=PASS/FAIL` | 待测（D-054） |
