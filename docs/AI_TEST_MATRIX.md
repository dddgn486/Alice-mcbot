...
| T5 配额随场景推导（D-125） | 右键 `alice:lumber_job`（`lumber_course` 场景） | 零参数右键 | 配额不再写死 4，而由 `LumberCandidateSource` 数出的可行树数决定；日志 `[Job] lumber 场景可行树=N ⇒ 配额=N` | `待测`（预期 `场景可行树=4 ⇒ 配额=4`，终态仍 `trees 4/4`） |
