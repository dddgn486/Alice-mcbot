...
| **串联回归（常规入口）** | `alice:pathing_regression` + `/function alice_test:pathing_course` | 普通右键一次 | **12 项**依次自动建地形并断言（执行 6 / 拒绝 2 / 岩浆路线安全 1 / 路线偏好 1 / 重规划 1 / 重同步 1）；聊天/日志 `[Regression] SUMMARY ...=PASS/FAIL` | `WINDOWS_CLIENT`（2026-09-09：12 项中 11 项 PASS，`lava_course` 按 D-057 改断言后待复测） |
...
| 岩浆路线安全（D-057） | `alice:pathing_lava_guard` + `/function alice_test:lava_course` | 普通右键 | `[LavaGuard] status=... lava_contacts=0 result=PASS`（允许 REACHED：抬高一格 + 岩浆上方搭桥 + 落回平台 B，全程不进入岩浆格） | 待测（D-057） |
