...
| J7 Step 3 崩溃兜底（D-127） | 右键 `alice:scaffold_check`（第二轮：故意不拆 → 判定 → 续做）；另跑 `alice:regression_battery`（`scaffold` 步） | 零参数右键 | 账本里我方未拆除的 `TEMP` 放置：就近（≤16 格）⇒ `RestoreScopeTask` 续做并把账本收尾；远 ⇒ 如实报告不自动走回去（§12.3 拆除须在架上） | `待测`（预期 `[Scaffold] recover_decide … decision=ready` + `RECOVERY SUMMARY decision=ready ledger_before=N ledger_after=0 residue=0 → PASS`）。未覆盖：`too_far` 分支（场景太小无法安全拉开 >16 格）与真实崩溃重启路径 |
| T7 清障预算按棵重置（跨阈值）（D-126） | `/function alice_test:lumber_course` → 右键 `alice:lumber_job`（需先 `/reload`） | 零参数右键 | 场景东扩 + 第 4 棵同型橡树（基底 `33,64,208`）⇒ 累计清障应 **≥9**，跨过 `MAX_CLEAR_PER_TREE(8)`；配额由 T5 自动升到 5 | `WINDOWS_CLIENT`（2026-09-12 10:35）：`场景可行树=5 ⇒ 配额=5`；`DONE quota_met trees 5/5 logs 23/23 cleared=9 inventoryDelta=23 scaffoldLeft=0 ticks=806`；逐树清障 20:3 / 19:2 / 29:1 / 云杉:0 / 新树 33,64,208:3 ⇒ **cleared=9 跨过阈值 8**，job 级计数器会在第 9 格失败 ⇒ 该回归从「压线」变成真断言 |
