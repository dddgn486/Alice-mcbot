...
| BREAK_AND_ENTER（D-068） | `alice:pathing_break_enter` + `/function alice_test:break_enter_course` | 普通右键 | `[BreakEnter] SUMMARY plan_a=PASS plan_b=PASS execute_a=PASS`（目标列 2 格高/1 格高两种；首步必须是 BREAK_AND_ENTER；实跑后脚位落在柱内） | 待测（D-068） |
