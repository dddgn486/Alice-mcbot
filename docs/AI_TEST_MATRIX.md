...
| **串联回归（常规入口）** | `alice:pathing_regression` + `/function alice_test:pathing_course` | 普通右键一次 | **13 项**依次自动建地形并断言（执行 7 / 拒绝 2 / 岩浆路线安全 1 / 路线偏好 1 / 重规划 1 / 重同步 1）；聊天/日志 `[Regression] SUMMARY ...=PASS/FAIL` | `WINDOWS_CLIENT`（2026-09-09：12 项中 11 项 PASS，`lava_course` 按 D-057 改断言后 PASS，12/12）；D-058 新增 `fall_course` 后待复测 |
...
| 风险开关（D-059） | `/alice risk` / `/alice risk descend_overshoot on|off` | 一条命令 | 默认 `descend_overshoot=false`（Baritone 原样，不检查过冲列）；打开后 DESCEND 恢复过冲列校验，日志 `[Risk] switch ... all=...` | 待测（D-059） |
| 落差 FALL（D-058） | `alice:pathing_fall` + `/function alice_test:fall_course` | 普通右键 | `[Fall] SUMMARY fall_plan_2=PASS fall_plan_3=PASS no_deep_fall=PASS fall_recover_guard=PASS fall_execute=PASS`（2/3 格落差可规划且首步 FALL；4 格落差不得生成 FALL 且路线无 ≥4 格单段下落；不可回收落点不得生成 FALL，可回收落点必须能生成） | 待复测（D-058 修订） |
