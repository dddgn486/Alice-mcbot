...
| 挖掘站位两模式（D-070） | `alice:mine_course_runner` + `/function alice_test:mine_course` | 普通右键 | `[MineCourse] SUMMARY free=PASS wall=PASS blocked=PASS headroom=PASS buried=PASS`（free/wall/headroom=模式 A；blocked=模式 TUNNEL；buried=found_but_unminable 或 TUNNEL） | 待测（D-070） |
