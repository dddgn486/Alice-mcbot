...
| 路线偏好（Q7 验收 D-040） | `alice:pathing_dip_route` + `/function alice_test:dip_course` | 普通右键 | 基线（标定前，客户端实证）`first=DESCEND movements=3 cost=4.41`（≈34 tick）；标定后应为 `first=TRAVERSE movements=4 cost=4.66`（≈28 tick） | 基线已采；标定后待复测 |
