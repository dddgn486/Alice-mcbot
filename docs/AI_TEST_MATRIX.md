...
| 路线偏好（Q7 验收） | `alice:pathing_dip_route` + `/function alice_test:dip_course` | 普通右键 | 标定前基线 `first=DESCEND movements=3 cost=4.41`（≈34 tick）；标定后应为 `first=TRAVERSE movements=4 cost=4.66`（≈28 tick） | 待测（场景已用 `tools/simulate-scene-plan.py` 穷举校验） |
