...
| 垂直上升 PILLAR（D-055） | `alice:pathing_pillar` + `/function alice_test:pillar_course` | 普通右键 | `[Pillar] SUMMARY pillar_plan=PASS resource_guard=PASS pillar_execute=PASS detail=REACHED/first=PILLAR/pillars=3`（3 格深 1×1 基岩竖井；清空快捷栏方块后应不可规划） | 待测（D-055） |
