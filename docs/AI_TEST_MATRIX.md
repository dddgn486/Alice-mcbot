...
| R2 限次清障换候选（D-121） | 右键 `alice:clear_retry_check`（自带场景，零参数） | 零参数右键 | 一个清障候选失败 ⇒ **换下一个**（不再一次失败就放弃整棵树）；只有候选都用过/预算用尽才闩锁。清障子任务信封 = 父信封子集（`clear=none`、加高 `min(父,1)`、无建拆同权） | `待测`（预期 `[ClearRetry] SUMMARY retry=PASS attempts=4 clear_then_mine=PASS cleared=1 sub_profile=PASS → PASS`） |
