...
| **串联回归（常规入口）** | `alice:pathing_regression` + `/function alice_test:pathing_course` | 普通右键一次 | 7 个场景依次自动建地形并断言；聊天/日志 `[Regression] SUMMARY ...=PASS/FAIL`（含执行与拒绝两类） | 待测（D-042） |
...
| 垂直下落（D-048） | `alice:pathing_downward` + `/function alice_test:vertical_course` | 普通右键 | `[Vertical] SUMMARY downward_execute=PASS downward_guard=PASS`（破坏脚下掉 1 格 / 竖井被拒） | 待测 |
