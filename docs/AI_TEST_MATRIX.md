...
| 落差 FALL（D-058） | `alice:pathing_fall` + `/function alice_test:fall_course` | 普通右键 | `[Fall] SUMMARY fall_plan_2=PASS fall_plan_3=PASS drop4_guard=PASS recover_guard=PASS fall_execute=PASS`（2/3 格落差可规划且首步 FALL；4 格落差与"落点上方封顶→PILLAR 无法返回"必须被拒） | 待测（D-058） |
