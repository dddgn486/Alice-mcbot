...
| L3 伐木 Job（切片 **J3**：策略可替换性 + 按棵预算真回归） | `/function alice_test:lumber_course` → 右键 `alice:lumber_policy_check` | 普通右键（零参数） | **只规划不执行**；同候选集对比 `NearestPolicy` 与 `NearestExposedPolicy` | `待测`（预期 `differ=true explainable=true exposedHonest=true → PASS`；`nearest=tree@20,64,208`、`exposed=tree@28,64,208`） |
| L3 伐木 Job（配额 **3** 棵 = 按棵预算真回归） | 同上场景 → 右键 `alice:lumber_job` | 普通右键 | 3 棵同型橡树各需清障 3 格 ⇒ 累计 **9 > MAX_CLEAR_PER_TREE(8)**；云杉顶格仍不可达（如实 partial） | `待测`（预期 `DONE quota_met trees 3/3`、`cleared≈9`、`logs 18/19`、`unknown=0`；若预算非按棵重置，第 3 棵会 `clear_budget` 失败） |
