...
| D-116 收集复用加高（树冠掉落物） | `/function alice_test:lumber_course` → 右键 `alice:lumber_job`；另跑 `alice:mine_regression` | 零参数右键 | 掉落物停在**头顶树冠**够不到时，① 就地扫尾用"原地加高"搭上去拿（日志 `[CollectDrops] gain_start/gain_done`），其放置由随后的建拆同权收回；③ 落地扫尾不给加高 | `待测`（预期高云杉 `gained=7/7`、`scaffoldLeft=0`；`mine_regression` 仍 10/10） |
