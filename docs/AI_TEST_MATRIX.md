...
| L3 伐木 Job（配额 **4** 棵 = 按棵预算真回归） | 同上场景 → 右键 `alice:lumber_job` | 普通右键 | 3 棵同型橡树各需清障 3 格 ⇒ 累计 **9 > MAX_CLEAR_PER_TREE(8)**；云杉顶格仍不可达（如实 partial） | `WINDOWS_CLIENT`（2026-09-10 21:26，**配额 3 棵**轮：`DONE quota_met trees 3/3 logs 12/12 ``cleared=8 inventoryDelta=12 unknown=0 ticks=474` —— **但 8 格正好压线，未触发按棵预算回归**，见 D-092 附注）。现配额提到 **4 棵**：`待复测`（预期 `trees 4/4`、`cleared≥9`；若预算非按棵重置，第 3~4 棵必 `clear_budget` 失败）。**2026-09-11 根因修正（D-117）**：09-10 21:28 的 `a5901f5` 加第 4 棵橡树时，其树冠占住了 `x=22` 列 —— 而 y=65 的 z=212..214 是 x=17..31 的树冠墙、可行走台地只有 x=18..30 ⇒ **x=22 是全墙唯一通道**（工作基线 21:26 的成功路线正是"北上→东到 x=22→南下穿墙"）。封死后 19/24 自 21:28 起每轮 `no_valid_standing_point`（与代码改动无关）。已删除该列 7 个树叶并写入"通道红线"；客户端数据包已同步（**需先 `/reload`**）。`待测`：19/24 的 `clear_start` 能清掉 `19,65,215`、`trees 4/4` |
