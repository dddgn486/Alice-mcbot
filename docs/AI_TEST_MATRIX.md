...
| **串联回归（常规入口）** | `alice:pathing_regression` + `/function alice_test:pathing_course` | 普通右键一次 | **12 项**依次自动建地形并断言（执行 6 / 拒绝 3 / 路线偏好 1 / 重规划 1 / 重同步 1）；聊天/日志 `[Regression] SUMMARY ...=PASS/FAIL` | 11/11 PASS（D-054，2026-09-09）；D-055 新增 `pillar_course` 后待复测 |
...
| 垂直上升 PILLAR（D-055） | `alice:pathing_pillar` + `/function alice_test:pillar_course` | 普通右键 | `[Pillar] SUMMARY pillar_plan=PASS resource_guard=PASS pillar_execute=PASS detail=REACHED/first=PILLAR/pillars=3`（3 格深 1×1 基岩竖井；规划器实测选 2×PILLAR+1×ASCEND cost=11.68；清空快捷栏方块后应不可规划） | 首测 FAIL（D-056 空中假成功）→ 已修复待复测 |
| COLUMN 完成判定（D-056） | 任意执行场景（PILLAR/ASCEND/DESCEND/DOWNWARD/PLACE/BREAK 全量） | 一次串联回归 | 所有 `segment_done` 都必须发生在 bot **落地后**（日志 `ticks=` 不再出现 4 tick 假成功；PILLAR 必先出现 `[Pillar] placed`） | 待复测（D-056） |
