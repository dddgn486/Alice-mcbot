...
| T4 作用域重开继承掉落物（D-124） | 右键 `alice:mine_regression`（第 12 例 `scope_reopen_keeps_drops`）；另跑 `alice:regression_battery`（`scaffold` 步已去掉 D-108 手工收养） | 零参数右键 | 会话内**重开作用域**应继承「仍活着且在新区间内」的我方掉落物归属（跨任务由 `end()` 清空）；判据：挖出掉落物（未收集）→ 重开 → `liveDrops()` 仍 ≥1 | `待测`（预期 `mine_regression` **12/12**、`scope_reopen_keeps_drops=PASS … liveDropsAfterReopen=1`；电池仍 9/9）。首测曾假失败：破坏当 tick 查账恒为 0（`pending` 在 tick END 才确认，见 D-124 附注） |
