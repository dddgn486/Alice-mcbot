...
**② 挖矿任务设计路线图（用户"可采纳"）**：阶段 0 设计文档 → 1.1/1.2 扫描契约+分片扫描器 → **1.5 作业区/意图 + 推进式扫描** →
...
⇒ 它是**收紧**不是放宽。**真正的两条出路**（待用户拍板）：⒜ **质心退化**（空集时目标改为"离区域**质心**最近的
认领格"：2×2 ⇒ 向内约 16 格、1 区块 ⇒ 向内约 8 格；零新几何、形状无关）；⒝ **块级内缩 K 格**
（"离最近未认领区块 ≥ K 格"的格集：2×2 且 K=8 ⇒ 中心 16×16，但 1 区块仍为空 ⇒ 仍需 ⒜ 兜底；代价 = 不能
物化成区块集合，判据变函数 + 最近格搜索）。
**复核触发**：① 用户基地 ≤3×2 区块且希望有内缩 ⇒ 按上面两条之一改（**不要再提八邻**）；
② 第 2 件（返程判据）落地时若发现"内部区块为空"的退化路径没被消费方正确处理 ⇒ 补断言语义（当时 `safe_return` 夹具要改写）。
