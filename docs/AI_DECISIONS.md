...
### D-403：⭐ **简化 —— 保留自动拆除；条件不满足就当场放弃**（用户 2026-09-22 修正 `D-402` 的冗余）

**用户原话**：「算了，现在这样实现反而**有点冗余**了，先**保留自动拆除**，**不满足条件就放弃**。」

#### 一、模型（取代 `D-402` 的"条件门 + 待办"两段式）

| 项 | 结论 |
|---|---|
| **保留** | **自动拆除照旧**（任务内原位 A1 / 任务收尾 A2，`MineTask:452`、`LumberJob:269/551` 已接） |
| **不满足条件** | ⭐ **当场放弃**：**不延后、不重试、不留待办**（`D-402` 的"进账本待办 + 显式回收入口"**判定为冗余 ⇒ 不做**） |
| **但** | **必须如实归因**（`skipped` + `notes` 带码），**绝不静默** —— 这是唯一保留的硬要求 |
| **账本** | 条目**照旧保留**（`D-401` R3 未变）⇒ 它是**记录**，不是"待办队列"；**没有**任何后台机制去收它 |

**保留的自动拆除条件（就这三条，判起来便宜）**：① 区块已加载（不强制加载，T4）② 现场仍是我方方块（`not_ours` ⇒ 销账）
③ **拆了不会把 bot 摔下去**（`D-399`）；其余（越界/块数/tick 预算）**不再作为门**，遇到就放弃并记码。

#### 二、代码（本轮已落）

`task/RestoreScopeTask.java`：**删掉** `deferred` / `deferPass` / `MAX_DEFER_PASSES` 与"延后重试"整段
（那正是被判定为冗余的一层）；不满足 ③ ⇒ 立即 `skipped++` + `unresolved` + `notes` 记 **`underfoot_unsafe`**。

**判据实测**：`single:restore_underfoot_safety`（C2）**PASS**，读数
`skipped=3`、`notes=4002,101,2600:underfoot_unsafe | 4001,100,2600:… `（**当场放弃这条路真的被走到**）、
`biggestFall=1`、`endSupported=true`。
...
### D-405：⭐ 真机实测锁定 —— **挖的脚下那格是"我们自己放的垫脚方块"，高度无关、100% 复现**（用户 2026-09-22）

**用户原话**：「挖的脚下那格**就是放的踮脚的方块**，实测认为**跟高度无关**，**百分百复现**」
+ 两问：「**为什么守卫没生效**」「**为什么会自动回收**」。

#### 一、两个问题的答案（都在代码里，逐条）

**Q1 为什么守卫没生效** ⇒ 因为**守卫的判据不是"不许拆自己踩着的格"**，而是"**拆完下面有没有落脚面**"：

- `D-399` 守卫的条件是 `pos.equals(foot.below()) && !canWalkOn(level, pos)`；
  而 `canWalkOn(level, pos)` 读的是 **`pos.below()`** ⇒ 只要**下面还有实心**（地面高度、阶梯、柱子仍有下一格），
  判据就是**真** ⇒ **守卫放行** ⇒ 拆掉脚下那格 ⇒ bot 下落 1 格。
- ⚠️ 更根本的是：**"站上去 → 向下拆"本来就是回收的设计流程**
  （`RestoreScopeTask.pickNext()` → `startStage(pos.above())`，注释原文「自上而下：站上去 → 向下拆」）
  ⇒ **原地拆自己正踩的格是它的正常路径**，我的守卫只防"**摔进虚空/深坑**"，**不防"1 格自落"**。
- ⇒ 与用户实测"**跟高度无关**"完全一致：**只要有下一格实心，就都会放行、也就都会掉 1 格**。

**Q2 为什么会自动回收** ⇒ 因为 `D-403` 保留的**自动拆除**是接在**任务本身/任务收尾**上的
（`MineTask:452`、`job/lumber/LumberJob.java:269/551`、夹具包装 `CleanupWrappedTask`），
而**这套流程里没有任何一条"bot 还站在上面"的条件** ⇒ 它就在 bot 踩着垫脚方块时把它拆了。

#### 二、⇒ 真缺陷的形状（一句话）

**"回收"在拆到自己脚下的那一格时，没有"先把 bot 挪开"这一步**；
而这**不是** `D-399` 守卫能拦的（守卫问的是"拆完有没有落脚面"，不是"bot 是否正踩着"）。

#### 三、三条修法（待用户裁定，未实施）

| 方案 | 内容 | 代价 |
|---|---|---|
| **A. 先撤离再拆**（推荐） | 拆"bot 正踩着的那格"之前，先让 bot 走到**安全处**（地面/非目标的实心格），再拆 | 需要"撤离点"判定；可能 ≤N 格走位（与 `B3`「回收不许产生新债」不冲突：走位不写入即可） |
| **B. 一律放弃并记码** | 凡是 bot 正踩着的目标格 ⇒ **不拆**（`skipped` + 归因 `underfoot_occupied`） | 后果：**脚手架会残留**（等于拆不动） |
| **C. 改从侧面拆（不站上去）** | `APPROACH` 改为站在**非目标**的支撑上（侧面 `SIDE_BREAK`），从根上避免"站上去" | 需要 reach/LOS 成立；可能拆不到某些几何（那就落回 B） |

### D-406：⭐ 撤回 `D-405` 的 Q1 解释；真根因两条（守卫**时机**错 + **`D-398`/Z1 未落地**）（2026-09-22）

**用户实测证据（`latest.log` 22:05–22:08，逐字）**：
```
place -84, 90, 147 cobblestone by=mine-runner:attempt0:STEP_PLACEMENT tick=57570   ← 我们放的垫脚方块
break -84, 90, 147 cobblestone by=RestoreScope:SCAFFOLD_RESTORE  tick=57700        ← 130 tick 后「自动回收」拆掉它
break -85, 89, 147 gold_block   by=command:EXPECTED_TARGET
```
用户纠正：「脚下那格是原来的踮脚方块，**再下面十格内一个方块都没有**，这守卫难道不会拦住？」
⇒ 按 `D-399` 判据（`pos == foot.below()` 且 `!canWalkOn(level,pos)`）**本来应该拦住** ⇒ 我 `D-405` 的解释
（"下面还有实心所以放行"）**不成立 ⇒ 撤回**。

#### 一、Q1（守卫为什么没生效）真根因：**检查的时机错了**

守卫在 `RestoreScopeTask.pickNext()` —— **队列取件那一刻**检查；而 bot **是之后才站上去的**：
`pickNext()` → `startStage(pos.above())`（= **站上去**）→ 再拆。
⇒ 取件时 `pos != footCell(bot).below()` ⇒ **第一个子句为假 ⇒ 守卫直接放行**；
等到真正破块时 bot 已经站在它上面了 —— **危险是 `APPROACH` 这一步自己造出来的**，守卫看不到。
⇒ 也解释了为什么夹具 `restore_underfoot_safety` 会绿：那个几何里 bot **取件时就站在目标上**（巧合命中第一个子句）。

**修法方向**：① 判据挪到**破块前**再判一次；② 或改流程（**不站上去**＝侧面拆 / **先撤离**再拆）。
两条都要等 Z1 之后再做（见下）。

#### 二、Q2（为什么会自动回收）真根因：**`D-398` 未落地（Z1 未做）**

- 实测：**这里不是保护区**（全会话仅 2 处提到"保护区"；`作用域开启: center=-84, 89, 148 radius=8` 是**任务 scope**，不是保护区）；任务 = `MineTask`（玩家命令指派目标，`command:EXPECTED_TARGET`）。
- 账本**记了 7 条** `[Ledger] place` ⇒ 按用户 `D-398`（**区外不记账、不恢复、无限制修改**）**本就不该有**；
- 自动回收也**本就不该发生**（`D-398` R2：区外一定不恢复）。
⇒ ⭐ **结论**：这不是"该不该自动回收"的参数问题，而是**整个账本 + 回收机制在区外根本不该存在**；
**`Z1`（账本/回收收窄到保护区内）做完 ⇒ 用户这条 100% 复现场景从根上消失**（脚手架留在野外、不回收）。

#### 三、行动

1. ⭐ **立即做 `Z1`**（账本/回收收窄到保护区内）—— 本轮真正的修复方向；
...
### D-412：⭐⭐ 真 infra 缺陷 —— **场景数据包被同命名空间的旧副本盖住**（静默，2026-09-23）

**怎么发现的**：`C6` 搬迁（+32 东移）后连跑 3 次全部 `FAIL ticks=0 no_reachable_candidate`，
而 `scene rc=855/186` 与移位前**逐字相同**（看起来"场景跑成功了"）。完整取证 =
`docs/reviews/2026-09-23-C6搬迁否决与数据包遮蔽缺陷.md`。

#### 一、事实（每一条都是当场复算/探针，不是推断）

| 环节 | 观测 | 排除 |
|---|---|---|
| 位移正确性 | 1072 个坐标三元组**恰好 +32**、y/z 未动、行数不变 | 改坏场景文件 |
| 离线自检 | `check-scene-connectivity.py` 输出**去掉数字后与基线逐字节相同** | 几何/连通性变化 |
| 数据包同步 | `$WORLD/datapacks/alice_test/...` 确为新文件（`fill 49 58 203 63 …`） | 没同步进世界 |
| 区块与命令层 | `(55,207) loaded=true`；`/setblock 55 101 207` **rc=1 且方块出现** | 区块未加载／命令层对新坐标不生效 |
| ⭐ **函数重跑** | **`function alice_test:lumber_course_terrain` rc=855，而 `(55,63,207)`/`(52,64,208)` 仍是 Air** | 夹具/Job 读错世界 |

#### 二、真因

`datapacks/` 下**每个子目录都是一个活数据包**。`sync-windows-artifact.sh:97` 把场景包备份放在
**`datapacks/` 里面** ⇒ `alice_test.bak.<ts>` 同样提供 `alice_test` 命名空间，且字典序在
`alice_test` **之后** ⇒ **后加载者胜** ⇒ 服务端执行**旧副本**（逐字核对过 `fill 17 58 203 31 116 231`）
⇒ 方块被写到**旧坐标**，而那里的区块此刻**没被握住**（bot 在新址）⇒ 静默 0 改动。

⚠️ **它能骗过全部现成护栏**：`rc` 照样 855（命令确实执行了）；`D-251`/`D-408` 的 `scene rc=` 判据看不见；
`check-scene-connectivity.py` 读的是**仓库**那份 ⇒ 离线自检全绿。

#### 三、影响面（**已核实，比"可能很严重"窄**）

- 母本建于 **2026-09-20 23:51**，而 `.bak` 是 23:10/23:19 造的 ⇒ **自那以后每轮跑的都是 `.bak` 那份**。
- ⭐ 但 `git log --since="2026-09-20 23:51" -- tools/test-scenes/ | grep -c '\.mcfunction'` = **0**
  ⇒ **该窗口内没有任何已提交的场景改动** ⇒ **历史实际损失 = 0**；
  唯一被吞掉的是**今天那次未提交的 +32 位移**（正是它暴露了本缺陷）。
- 客户端存档里的 `.bak`（09-21）与仓库 `diff -rq` = **0 处不同** ⇒ 它不是"内容陈旧"，是**同名遮蔽**。

#### 四、修（两处，已落地）

| 位置 | 改动 |
|---|---|
| `tools/headless-battery.sh` | 复制新包**之后**清掉一切**提供 `data/alice_test` 的同名旧包**；并**断言提供者恰好 1 个**（宁可响亮失败，不要静默用旧的） |
| `tools/sync-windows-artifact.sh` | 备份**移出 `datapacks/`**（改放 `<world>/.alice-datapack-backups/`）；顺手清历史遗留的同名活备份 |

---

### D-413：`C6` **搬迁否决** ⇒ 改为「清掉夹具世界的第三方认领」（2026-09-23）

`C6`（把伐木课程搬出玩家 FTB 认领）**试过、否决**；`D-412` 修好后搬迁的真实效果立刻现形：

| 观测 | 含义 |
|---|---|
| `premise … 已落地（等了 2 tick）` | ✅ 平台落地、bot 站得住（无认领 + 场景在位） |
| `rejected=[tree@52,64,208:trunk_too_tall(unreachable=4,hard=2), tree@60,…:…(unreachable=4,hard=1), …]` | ❌ **每棵树都"无可站立点 + 硬遮挡"** ⇒ `candidates=0` |

**原因（读场景文件即可见）**：这个场景是**连着宿主地形一起抓下来的、不是自足的** ——
清空盒只到 `x17..31` 而树长到 **`x35`**（东边几棵**站在没被清过的真实地形上**）；文件头那条
"树冠通道红线"也写着 bot 南北穿行**靠宿主地形**（`x=22..26` 唯一缺口）。
⇒ **纯平移把场景搬进了「未被验证的宿主」**；而且**基地附近没有空位**（扫遍所有偏移：避开 6 个其它场景
+ 2 处认领后，`dx=±32,dz=0` 已是最小代价，其余只会更远）。

**真修法**（与电池**早已在做**的"清 `alice_*.dat`"逐字同源 —— 夹具世界是给 Alice 的**代码**用的，
不是给第三方模组状态用的）：
...
**安全性核对**：全仓只有两处碰 FTB —— 我新加的 `FixtureThirdParty`（只读）+ **`break_refused`（CORE 步）
在运行期自己造认领** ⇒ **没有任何夹具依赖"预存认领"** ⇒ 清掉安全。

#### 结果（场景坐标**已还原**）

| 检查 | 结果 |
|---|---|
| `single:lumber_job` × **4** | **4/4 PASS**（确定性；此前的"刀尖 flaky"同样是旧副本造成的） |
| `module:lumber` | **`6 failures=0 skipped=0 → PASS`**（此前 `skipped=4`） |
| ⭐ **CORE** | **`passed=41/41 skipped=0 ticks=4173 → PASS`** —— **35+ 轮以来第一次全绿** |
| ⭐⭐ **CORE 缓存** | 紧接再跑：`verdict=PASS 用时=0s 缓存复用（指纹=996a85b8521e）` ⇒ **反馈 ~200 s → 秒级** |

⭐ **口径修正**：`D-410` 说"CORE 反馈 分钟→秒"的乘法级杠杆**只归 `A2`** —— 那句在"**红项不修**"的
前提下成立；本轮把红项**从根上修掉**后，`D-352` 的"只写 PASS"缓存**自己就命中了**。
⇒ **两条路都能到终点**，而"修根"这条**不需要新机制**；`A2` 仍值得做（它对付的是**红项必然存在**的时期）。
...
### D-417：⭐ `A2` 逐步缓存**可行性验证的结论 = 放弃**（"跳过执行 + 复用判决"形态）；并登记替代杠杆 `A2′`（2026-09-23）

**触发**：用户 2026-09-23 选定队列首项 `A2`（`survey/30 §5`：先把 `CORE` 反馈从分钟级打到秒级）。
整份取证 = `docs/reviews/2026-09-23-A2-逐步缓存可行性.md`（判据逐字引用、两个方向的实测、诚实边界）。

#### 一、判据的口径必须先修（照字面做是恒真的）

`survey/30` 写的判据是"同一提交跑一次全量、再跑一次逐步缓存，逐条判决必须完全一致"。
⭐ **字面照做无法证伪**：若每步指纹覆盖 `src/`（`D-352` 的全局指纹就是这样），一轮真跑后**每一步**都命中
⇒ 第二轮的"逐步缓存"是**拿缓存和自己比**。⇒ 本注只承认两种有信号的形式：
① **自足性**（该步**无前序**时的判决 = 它在 CORE 位置的判决）；② **消融**（**跳掉**某步后**其它**步的判决变不变）。

#### 二、实测（两个方向都做了）

| 方向 | 结果 |
|---|---|
| **自足性**（41 步 `single:<step>` vs 它在 CORE 的位置） | **判决 41/41 一致（0 翻转）**；ticks **36/41 逐字相同** |
| **消融**（归档 `20260922-215010`（53 项）→ `20260922-224359`（41 项）独立复算） | **2 步翻**：`craft_table` PASS→FAIL、`survival_exit` PASS→FAIL。⚠️ 窗口内 `Z1` + "电池 `scenes→provision` 改序"同批落地 ⇒ **该对不干净**；但 `craft_table` 的根因（`D-408 §三`）**确实是隐含前置**（旧的绿靠前序步把区块捂住），且它是靠**改电池顺序**修好的、不是靠把步加回来 |

**ticks 侧的关键控制**（否则会误判）：先用 6 轮 CORE 建立**同上下文噪声底** ⇒ **36/41 步 spread = 0**（跨越 `Z1`–`Z4` 源码改动仍逐字相同）、
只有 `clear_guard`（27）与 `lumber_job`（391，`C4`/`C5` 已知）会动。用它判 solo 差 ⇒
**唯一站得住的真差 = `pathing`（CORE `684`，spread 0 → solo `714`/`714`，两个独立样本）**，
与"它不传送、course 是绝对坐标、起点由前序步决定"（`task/RegressionBatteryTask.java:597` + `task/PathingRegressionTask.java:109`）逐字吻合。
⚠️ **两次当场自我更正已留痕**：`clear_guard` 的 Δ28 与 `mine_regression` 的首个越界样本（142）**都不是证据**（前者在自身 27 的 spread 内、后者复样 134 落回区间）。

#### 三、结论：**放弃**（三条支柱，各自独立成立）

1. **成本账不成立（实测）**：逐步缓存能省的只有"**被测的那 21%**"，而**每轮 79% 的引导照付**
   —— 41 步 solo 的 wall 451 s 里 **358 s 是冷启动/调度**（单步固定 20–26 s；`mine_menu` 1 tick 却 24 s）。
   ⇒ 「CORE 反馈 分钟→秒」**与逐步缓存无关**；即使机制完全可信，收益上限也够不到它承诺的东西。
2. **可用粒度不存在**：逐步缓存要求"该步的指纹保守完备"。CORE 的 41 步**绝大多数要走内核**
   （寻路/挖掘/合成/容器/事件）⇒ 保守完备的逐步指纹会**退化回 `src/` 全量**，即 `D-352` 已有的全局指纹；
   人为放宽粒度换来的就是"**用陈旧判决顶替真跑**" = 假绿本身。
3. **退化不可见**（`survey/30` 所担心的"假绿"的**具体落点**）：`write_policy` 判的是**整轮前缀**
   （`RegressionBatteryTask.java:595`：它审计"本次电池此前所有写入"），而它的两条判据是 `isEmpty()`
   （`WritePolicyCheckTask.java:442-449`），两个 map（`WritePolicyMatrix.java:888,893`）**只累计违规、全仓没有人口计数**
   ⇒ 跳过前序会让它**静默变成空集真**，而**判决表面照样"逐条一致"**（矩阵第 36 步就是 `write_policy PASS / PASS`）。
   ⭐ 即：`A2` 自己那条判据（"逐条判决完全一致"）**看不见**这一类退化。

#### 四、正向产出（不是白跑）

- **`single:<step>` 通道拿到可信基线**：41/41 判决一致 ⇒ 在当前提交上"单步判决"可代表"该步判决"；
  `run/a2-solo-matrix.tsv` + `run/a2-solo-logs/*.json` 可直接当对照基线复用。
- **替代杠杆 `A2′`（已登记进 `docs/OPEN_ITEMS_LEDGER.md §11-G`，未实施）**：**「一次引导跑多个点名步」**（`single:<步名>[,<步名>…]`）。
  `headless/HeadlessBattery.java:138` 现在是 `setOnlySteps(List.of(name))`，**只收一个名字**。
  判据 = 与**逐条 solo**判决一致（**对照就是本次那张表，现成**）；收益 = 省 N−1 次引导（每次 20–26 s）。
  ⭐ **它不跳过任何执行** ⇒ 不引入"陈旧判决"这一类风险；对 `write_policy` 反而**更强**（前缀人口变大）。
...
另裁定 **`craft_table` 保留在 CORE**（`survival_exit × D-398` 当轮待裁、不阻塞；⭐ **已于 2026-09-23 由 `D-420` 裁定：不改，区域外逃生不记账**）。
...
### D-419：**能力清单 = 从代码生成 + 跨出处双向门禁**（`B3` / `Q-22`；2026-09-23）

**触发**：用户 2026-09-23 选定「①②③ 一批」里的 `B3`（本批四项里唯一剩项）；
出处 = **`survey/29 §3.8⑥`**（`Q-22` 用户已同意）：
「把『它知道的自己』做成一张**生成的表**，而不是记忆……照项目四个现成模子（**单一出处 + 双向防漂移 +
解析不到就响亮失败 + 全挂 `check-all`**）⇒ **『它以为的自己』和『真实的自己』结构上不可能分叉**。」

**落地**（`docs/CAPABILITY_LIST.md` = 生成物，264 行）：
`tools/capability-list.py`（8 个出处解析器 + 8 族断言）· `tools/check-capability-list.sh`（挂 `check-all`）·
`docs/BATTERY_CURATION.md` §2 的手写清单**降级为"理由叙述"**（消灭第二真相源）。
...
| # | 断言 | 为什么今天值得存在 |
|---|---|---|
| A1 | 步声明（模块 `CheckStep` ∪ 电池内联 `step(...)`）↔ `CURATION` 键（**双向，无豁免**） | 此前只在**运行期**断言（`prepareSteps`，要起服务端 ≈20–26 s 才响） |
| A2 | 模块 `CheckProfile` ↔ `CURATION` 档位 | ⭐ **此前零断言**，而且模块档位**今天被 `fromCheckSteps` 丢弃**（零消费者）⇒ `Phase 1b` 落地那一刻才会现形 |
| A3 | `CheckModules.ALL` ↔ 电池成员（缺席者须带理由，豁免反向核对） | `module:<id>` 的入口比电池大 ⇒ 新模块可能"登记了却从不进 CORE" |
| A4 | `JobRequest.Kind` ↔ `JobKindContract` 表（双向） | 既有门禁只查 `Kind → 表` 单向 |
| A5 | `MovementType` 常量 ↔ `changesWorld()` 名单 | 写入口径的第二处声明 |
| A6 | `RiskSwitches.OPTIONAL` 键 ↔ `KNOWN` | 解析自检（强门禁是 `risk-surface.py`，不重复） |
| A7 | 人口下限 + 每节的**独立计数**交叉核对 | **解析不到就响亮失败**（`Z4` 的空集真教训）：少解析一行会表现为"对面多一条"⇒ 红，而不是少一行悄悄过 |
| A9 | 手写文档**不许再自称清单/计数** | 防"又长出第二个会漂的家" |

**豁免表也双向**：豁免项若**真的**出现在对面 ⇒ 过期豁免 ⇒ 红（豁免不是许愿池）。

#### 二、活体样本（这条门禁不是假想需求）

`docs/BATTERY_CURATION.md` §2 的小节标题写着 `BASELINE（15）` / `MAIN（35）` / `EXTRA（15）`，
**真值 = 15 / 26 / 52**；同一张清单里 `decision_contract` 还被**重复列了两次**（MAIN 段 + EXTRA 段）；
文件自己留着一句"本节标题里的计数**长期漂移**"的认领。⇒ 分叉早已发生，且**靠人读才发现**。

#### 三、验证

- 门禁**首跑绿**（**没有先放宽断言**）；`tools/check-all.sh` = `pass=21 warning=1 failed=0`（新增本门禁记分）。
- **注入矩阵 13 臂全红**（A1 两向 / A2 / A3 两向 / A4 两向 / A5 / A6 / A7（解析崩塌）+ A7b（人口下限）/ A8 / A9），每臂跑完立刻还原。
- `ALICE_HEADLESS=1 tools/check-all.sh` = **`PASS: pass=22 warning=0 failed=0`**；因本轮改了 `tools/`（`D-352` 指纹失效）电池是**真跑**：CORE **41/41 PASS**（251 s，`run/headless-logs/20260923-220529-core.log`）⇒ **本批没动到任何判决**；绿缓存重建为指纹 `0f0f4b99f345…`。
- 取证全文 = `docs/reviews/2026-09-23-B3-能力清单生成与防漂移门禁.md`。

#### 四、⚠️ 明写"未达到"（不许用"部分完成"含糊）
...
### D-420：**区外逃生**同样**不记账** —— `D-398` 不开口子（用户 2026-09-23 裁定）

**用户原话**：「**不改，区域外逃生不记账**」

**裁的是什么**（`§11-F 待用户拍板 ②` 的收口）：`D-245`（逃生放置不自动回收，但条目留 `TEMP`、交玩家回收）
与 `D-398` R1/R2（区外不负责任、一定不恢复）在**地理范围**上撞车 —— 野外逃生垫按 `D-398` **连条目都不生成**。
本轮裁定：**不给维生逃生开例外**，`D-398` 保持**无条件 + 可判定**。

#### 一、裁决的直接后果（写清楚，避免下次又被读成"漏了"）

| # | 后果 |
|---|---|
| 1 | ⭐ **`D-245` 的"记账交玩家回收"这半边，适用范围 = 保护区内**（区外只剩"不自动回收"那半边，由**世界事实**断言守着） |
| 2 | 野外逃生垫：**不进账本、不恢复、无回收义务**（`D-398` R1/R2/R4 不变）；它净消耗一次性方块（`THROWAWAY`，通常是圆石） |
| 3 | **可见性并未丢失**：每次区外放置留 `[Ledger] skip <坐标> place=<方块> by=survival-escape:…（区外…累计 N 次）`；`Z2` 的 `Closure` 读数会明说「本窗口没有记账：这 N 次全在区外」 |
| 4 | 代码**一行不动**（今天的实现已经就是这个裁决）；改的只是**口径的归属**（从"待裁"变成"已定"） |

**现场实证**（2026-09-23 CORE `41/41 PASS`，`run/headless-logs/20260923-220529-core.log`）：
```
22:05:22 [Ledger] skip 330, 99, 306 place=minecraft:cobblestone
         by=survival-escape:attempt0:STEP_PLACEMENT（区外：D-398 R1/R2 不记账、不恢复；累计 12 次）
22:05:24 [Ledger] 闭合 step=survival_exit inZone=0 wildInLedger=0 recorded=+0 wildSkipped=+1（全在区外）
```
坐标 `330, 99, 306` 与 `D-245` 的反向对照（2026-09-16）**是同一格** ⇒ `D-245` 那天验的行为，今天仍然成立。

#### 二、⭐ 裁决暴露的一个**覆盖缺口**（登记，不阻塞）

`SurvivalExitCheckTask` 那条判据是**按运行时 `pillarProtected` 选期望值**的一个断言两个期望值：
区内 ⇒ 要求 `TEMP` 条目；区外 ⇒ 要求**无条目**。实测走的是**区外**那半（`inZone=0`），
⇒ **区内那半今天没有任何场景会走到** —— 而本次裁决把 `D-245` 的记账半边**只**留在保护区内
⇒ ⭐ **它现在是"唯一幸存的那半边"，却恰恰是没被执行过的那半**。
修法（零概念成本）：给 `survival_exit` 补一条**区内臂**（把逃生竖坑放进自建认领区再逃生，断言 `TEMP` 条目存在）。
⚠️ 它要动 CORE 步的 tick 预算 ⇒ 按纪律**先登记**（`§11-F`），做的时候要复跑 CORE 逐步 diff。

#### 三、复核触发（若将来要翻案，从这里开始，不许凭感觉得出）

1. `D-245` 复核触发 ①：**真的**出现"野外逃生垫长期堆积、影响游玩或卡路"；
2. `[Ledger] skip …（区外）` 的 `outsideSkips` 累计数在真实存档里**明显长起来**（那是量出收益的时刻）；
3. 出现"想一键清掉野外逃生残留"的实际需求。
⇒ 真到那时，形状**必须是**：**有界**（只记逃生档 + 每世界上限）+ **不构成闭合义务**
（`D-398 §二` 已预留这条路：「区外条目不进账本，**或进也不构成闭合义务**」）+ 用**新 `WriteReason` 值**判定
（**不许**用 `requester` 字符串匹配 —— `D-329 M4` 的教训）。

---

### D-421：上下文改按**两条线**管，**goal 不再用轮数当边界**；压缩前**暂停 goal**（用户 2026-09-24 裁定）

**用户原话**：「算了，256轮上限没啥意义，还不如按上下文压缩阈值为上限，压缩前要暂停，50%是提醒我的线，
80%是你要准备断点的线，不要自己估算上下文用量，也不要一点一点去接近线，每次做完看一线过没过线就行，
不要一边顾虑一边做，如果不小心触发自动压缩算作可接受的损失误差」

#### 一、裁定内容

| # | 规则（可执行写法） |
|---|---|
| 1 | **不看轮数**：`maxGoalRounds` 只是防跑飞的安全网，**不是任务边界**；任务的真实边界 = 上下文断点线 / 需要用户证据 / 用户主动决策 |
| 2 | **不估算用量**：只认 `bash tools/dsh-context-usage.sh` 的真读数（来源 = 会话 JSONL 的 `usage.totalTokens`）；不猜、不心算占比 |
| 3 | **不做逼近式管理**：**每做完一件事跑一次**脚本，过线 ⇒ 动作，没过线 ⇒ 继续干活；**不为上下文改变输出详略、不提前收尾、不"再撑一点"** |
| 4 | **50%（= 256,000）= 提醒线**：告诉用户"可以挑干净断点压了"，并附"事实已落盘" |
| 5 | **80%（= 409,600 = 自动压缩阈值）= 断点线**：**先落盘事实**（HANDOVER/台账/commit）→ **`update_goal pause`**（压缩期间禁止自动续行）→ 请用户敲 `/compact`；用户说"继续"后 `resume` |
| 6 | **误触自动压缩 = 可接受的损失误差**（不算事故，不追责、不做补救仪式） |

#### 二、与旧规则的关系

- **取代**：`AGENTS.md` 旧条目里"① ≥0.5×阈值 / ② ≥0.25×阈值 才提醒"的提醒闸门，以及"≈0.9×阈值才落盘 HANDOVER"的落盘线
  ⇒ 现在只有 **50% / 80%** 两条线，**80% 强制先 pause 再压缩**（旧文本没有 pause 这一步）。
- **不变**：`D-214` 的**窗口 512K / 阈值 409,600 / 压缩后保留 ≈81,920**；`/compact` 仍**由用户敲**（AI 撞 `ManualCompactionError("busy")`，不自行触发）；
  压缩后取回原文仍用 `node tools/dsh-session-log.mjs --shadowed/--seq`；事实仍**先落盘**。
- **口径澄清**（若用户本意不是这个基准，改这一行即可）：百分比基准 = **窗口 W = 512,000**
  ⇒ 50% = **256,000**、80% = **409,600**（后者恰好等于 `0.8×W`，即自动压缩触发点，所以"断点线"与"自动压缩线"重合是设计意图）。
- **与"到边界就 `complete`、不再 `pause`"（2026-09-21）不冲突**：那条管的是**任务终点**；本条管的是**压缩窗口期**
  ⇒ 为了不让自动续行在压缩中途再开一轮，**压缩前 pause、压缩后 resume** 是本条允许的唯一 pause 场景。
...
### D-423：⭐ **作业级收集授权**（`P3` —— 范围内 + **只认本作业产物**）与四个夹具陷阱（2026-09-24）

**裁定出处**：用户 2026-09-22 裁定（原文见 `D-394` §五的待办行）：「给 mine 作业一个**『本作业声明范围内』**
的收集授权（`provenance=GRANTED_AREA`）」。本项落地。

#### 一、要解决的问题

mine 作业的落物今天只能靠「破坏事件配对」拿归属（`OURS_DIRECT` 直配 / `OURS_INDIRECT` 松窗）。
**一旦没配上**（连锁模组缓冲后重放、生成登记被推迟过窗口），它就是 `FOREIGN` ⇒ 被动闸门拦下、
收集阶段也看不见它 ⇒ **自己挖出来的东西捡不起来**（`A3` 的击杀缺口是同一族的另一个入口）。

#### 二、形态（四条结构性约束，都是刻意的）

1. **授权必须带产物过滤**（本项的核心）：`GRANTED_AREA` 的策略是 `AUTO` 且**被动吸附也放行** ⇒ 若只按
   **坐标**放行，等于把「这片地上的东西我都准你捡」（玩家授权的语义）悄悄搬给作业 ⇒ 作业范围内的
   **玩家丢的东西**会被一起吸走。⇒ 查询入口 `CollectGrants.coveringJobScoped(server, pos, ItemStack)`
   **要求传物品**，范围 **且** 产物同时成立才给 `GRANTED_AREA`。
2. **只在内存**：作业级授权**不进** `GrantsData`（`D-344` 同口径：作业不能悄悄改玩家的持久授权）；
   进程重启即无 —— 这是由「没有持久化入口」这个结构保证的，不是靠纪律。
3. **签发与撤销成对**：`MineJob` 开工签发（`productFilter::matches` 当产物判据）、`finish()` 撤销
   （四条终态路径都过那里）；10 分钟 TTL 只是「撤销路径被漏掉」的兜底，**不是**正常期限。
4. **单一判定入口不变**：`DropPolicy.effectiveProvenance` 仍是唯一入口（玩家授权 → 作业级授权 →
   `FOREIGN`），主动与被动两条路共用（`D-138` 的口径不许分叉）。

**门禁** `rule_job_area_grant_scoped`（四条臂）+ **五处注入即红**：A 去掉作业级咨询 · B 咨询只传坐标 ·
C 查询去掉产物判定 · D `MineJob` 去掉撤销 · E 把 `JobGrant` 塞进持久化 `save()`。

#### 三、⭐ 顺带抓出的四个**夹具陷阱**（比「功能绿了」更值钱；全部写进夹具 javadoc）

| # | 陷阱 | 现象 | 修法 |
|---|---|---|---|
| a | **两套时钟混用**：权威路径用 `server.getTickCount()`，我把 `level.getGameTime()` 传进查询 | 授权刚签发就被当过期销账（日志里有、查询说没有），`revoke` 也返回 false | 查询 API **自己取钟**（不给调用方留这种错法） |
| b | **`new AABB(pos).inflate(r)` 的半边长是 `r+0.5`** | 写 1.5 想要「这一格」，实际覆盖 ±2 格 ⇒ 相距 2 格的两个落点互相进框 ⇒ 臂② 读到**邻格的铁**（而闸门日志明明是 `diamond … FOREIGN`） | 判据 = `blockPosition()` **相等**；落点也拉开 ≥4 格 |
| c | **落物「可查」需要时间**（新建方块 + 生成实体，登记会被推迟；`D-345` 实测 19 tick） | 固定 3 tick ⇒ 三条落点全报「没有落物」 | **等条件 + 上限**（`A3` 的同一条教训） |
| d | **「东西还在吗」不能按格子判**：`ItemEntity` 是**可推动**实体 | 站在它上面 20 tick ⇒ 被玩家 AABB 推出原来那格 ⇒ **同代码两次判决不同**（剩 1 堆 / 剩 0 堆） | 按 **UUID 身份**判（`level.getEntity(id) != null && !isRemoved()`） |

**门禁/夹具**：`single:job_area_grant`（EXTRA，`PASS checks=15 failures=0`，连跑三次全绿）· 生产路径实证 =
CORE 日志里的 `[Grant] 作业级授权 …` / `[Grant] 撤销作业级授权 …`（`MineJob` 真的在签、也真的在撤）。

#### 四、复核触发

1. 出现「作业范围内、但**不属于任何作业产物**的东西想被收」的真实需求 ⇒ 那是**玩家授权**的场景
   （`/alice grant`），不许把作业授权的产物过滤放宽成「整片」；
2. 出现「作业级授权在 `finish()` 之后仍放行」的实例 ⇒ 说明有终态路径绕过了 `finish()`，先补那条路径；
3. 模组产物（作业产物不在 `forge:ores/*` / `forge:raw_materials/*` 族里）需要作业级授权时 ⇒
   走 `GoalSpec.productTag`（`MineProductFilter` 已支持指定标签），不要硬编码物品清单。

---

### D-424：⭐ **"无解"是当时的证明，邻域变了必须重评**（`PL-1` 切片 1 **实测改道**）+ 两条预算事实（2026-09-24）

#### 一、发生了什么（全部是实测读数，不是推断）

夹具 `single:mine_vein_propagation`（石壳 + 3×2×5 铁矿脉 30 格，bot 在脉外平台上）基线：
`PASS` / 121 s / **`mined 12/30`**，剩余 18 格**全部** `found_but_unminable`
（`direct=no_valid_standing_point` · `tunnel=no_reachable_tunnel_standing_point` ·
`enter=enter_target_unreachable`）。设计文档
（`docs/plans/2026-09-24-P5a-两格高走廊设计.md` §十一/§十二）据此把缺口归成
**「缺一个便宜的执行器」**（`PL-1` 路线 A：父任务内联破坏 + 2 格高走廊 + 作业级预算）。

⭐ 但先补了一个**只读**读数（作业终态后，把每一格剩下的矿石**重新规划**一遍；探针见夹具
`probeOrphans`）：**孤儿=18 → 现在可规划=17**（9 格 `CURRENT`＝原地就能挖、8 格 `TUNNEL`），
只剩 1 格是暂时性的 `search_incomplete`。取证日志
`run/headless-logs/20260924-115423-single_mine_vein_propagation.log`。

⇒ **卡点不是执行器贵，而是过早的永久了结**：失败那一刻的证明（邻格还是矿石 ⇒ 没有可站格 /
没有视线）是**真的**，但作业继续挖下去把西列挖空之后，**同一批目标全都有方案了**，
而 `attempted` 把它们当成"永远无解"。与 `P1-b`/`D-388`（`SEARCH_LIMIT ≠ UNREACHABLE`）同族，
这是它的姊妹条款：**当时无解 ≠ 永远无解**。真机同病：
`MiningPlanner:221` 注释里那句「目标 `336,62,190` **从未被挖**却已 `already_attempted`」。

#### 二、裁定（做什么）

**切片 1 = 有界的「过期证明重评」，不是执行器**：

1. **失败时记下这一刻的邻域证明** = **26 邻域**通行性位串
   （`MineJob.neighbourhoodWitness`，用 `MovementHelper.canWalkThrough` ⇒ 与规划器**同一份**判据）；
2. **只有当前邻域 ≠ 记录**时才允许重评（`proofExpired`），且每格最多
   `MAX_STALE_PROOF_RETRIES = 3` 次 ⇒ **有界、不空转**；
3. **硬拒绝不记证明**（保护区 / 不可破坏 / 流体风险：换邻域也改变不了"这格不许碰"，`D-323`）
   ⇒ 复用**同一份**名单（`MineTask.isHardTargetRefusal` 因此由包可见改成 `public`；
   `J-6`：拒绝清单只能有一处定义）；
4. **读数与痕迹**：`MineJob.staleProofRetries()` + `DecisionTrace "RETRY"` +
   `[MineJob] stale_proof_retry …`（能力有没有真的被触发，日志里看得见）。

#### 三、实测（判据）

| 读数 | 改前 | 改后 |
|---|---|---|
| `mined` | **12/30** | **30/30**（连跑两次：4421 / 4540 tick，258 / 264 s） |
| `staleRetries` | 0（无此机制） | **18** |
| 复评探针「剩余矿石格」 | **18** | **0** |
| 夹具判决 | PASS（旧下限 12） | PASS（新下限 **28**） |

- **红臂**（关掉重评 = `proofExpired` 恒 `false`）：落回 **`mined 12/30` · `staleRetries=0`**
  —— 与 `PL-1` 之前**逐字一致** ⇒ 差距确实由这条能力带来，不是夹具/环境漂移。
- `MIN_MINED` **按实测 12 → 28**（实测 30，留两格余量：每格的收集阶段时长随掉落物落点波动）；
  复核触发：连续三次 30/30 ⇒ 抬到 30（判据只许紧、不许松）。
- 门禁 `rule_stale_proof_replan`（六条臂）+ **逐条注入即红**：A 过滤处不问过期 · B 判据不比较 ·
  C 无上限 · D 失败时不记证明 · E 不排除硬拒绝 · F 邻域位串不用 `canWalkThrough` · G harness 单项预算 ≤ 夹具预算。

#### 四、两条预算事实（顺带纠正设计文档的一处归因）

1. ⚠️ 设计文档 §九 把切片 2 的 `2801 ticks ⇒ TIMEOUT` 归因成「顶穿**作业** tick 预算」。
   **实测纠正**：那是 **harness 的单项预算 2800**（`MiningModule` 为这一步声明），
   既不是作业预算（`JOB_MAX_TICKS`）也不是夹具的 `BUDGET_TICKS`（4600）——
   后者在旧配置下**永远轮不到**。本次就撞上了：能力已经把 `mined` 推到 ~19，
   但读数**根本没机会打印**（`TIMEOUT（单项预算用尽）`，`run/headless-logs/20260924-115901-*`）。
   ⇒ 新门禁臂 ⑥：**harness 单项预算必须 > 夹具自己的 `BUDGET_TICKS`**（兜底不许取代判据）。
2. 每格的**收集阶段 ≈130 tick**（`CollectDrops`：落物掉到 y=67，而矿脉悬空在 y=100 ⇒
   `too_far`/`policy_blocked` 白等）⇒ 挖穿 30 格 ≈ 4400 tick。夹具/作业预算据实抬到
   `BUDGET_TICKS=5400` / `JOB_MAX_TICKS=5200`，让终态由**作业自己达成配额**决定，
   而不是由时钟落点决定。⚠️ **未做、单独登记**：给场景加地板让落物可收（会把每格成本砍到
   ~15 tick）—— 那是**夹具几何**改动，会动 `D-389`/`D-391` 的历史读数，不在本切片里混。

#### 五、边界（本切片有意不覆盖）

- 只看 **26 邻域**：更远处的世界变化（例如 5 格外的通路被打开）**不改判决**；
- **执行器路线仍然留档**（设计文档 §十一 路线 A：内联破坏 + 2 格高走廊 + 作业级预算）——
  但**当前没有任何夹具要求它**，且它属 **D 段产品线**（鱼骨/探洞），不塞进本项；
- `MIN_MINED` 只许按实测抬；不许为了让夹具过而放宽。

#### 六、复核触发

1. 连续三次 `30/30` ⇒ `MIN_MINED` 抬到 30；
2. 出现「邻域没变但确实该重评」的实例（例如 5 格外的通路打开）⇒ 扩大证明范围到"影响面"，
   并在夹具里加对应场景（先补读数再改）；
...
### D-429：每 tick 搜索预算的 A/B 台架（`P4′` 的判据仪器）—— 九格读数与裁定建议（2026-09-24）

新增**电池步 `tick_budget_bench`**（EXTRA，`TickBudgetBenchTask`）+ 评审 `docs/reviews/2026-09-24-P4′台架与读数.md`。
**本片只造仪器与读数，不裁定 `P4′` 的方向**（那是能力决策，见 §五）。

#### 一、为什么以前没有"可复现的读数"

`P4′` 的原文判据要"注入 400 ⇒ `Can't keep up` 出现；收到 ≤60 ⇒ 消失"，但**没人能复现"红"**：
CORE 的搜索最大只 4–5 ms（`P2` 备忘记过），而真机那三次 `Can't keep up`（2035/2632/2232 ms）
既不可控也不可重复。台架把这件事变成**一次运行、九格读数、同轮对照**：

- `SearchTickBudget` 本来就有**夹具专用口**（`setLimits` / `resetForFixture` / `restoreDefaults`）
  ⇒ 三种配置**在同一轮里**跑，不用改源码重编译（这比"改常量 + 重编 + 跑两轮"可信得多：
  同轮对照消掉了机器状态漂移）；
- 每格 = **一个 tick 内**跑完的负载，读数取该 tick 的账（`tickMillis/tickSearches/tickExpensiveSearches/tickRefusals`）。

#### 二、九格读数（`run/headless-logs/20260924-140303-single_tick_budget_bench.log`，16 checks 全绿）

| 配置 | `MINING`=13×**50 ms 上限**搜索（生产形态） | `CHEAP`=30 短搜索 | `LEGACY`=13×**无时间上限**搜索（历史形态） |
|---|---|---|---|
...
| `UNGATED`（闸门全关） | 650 ms（13 / 0） | 1500 ms（30 / 0） | ⭐ **2578 ms**（13 / 0） |

⭐ **红臂在电池日志里真的出现了**（全日志唯一一条，来自 `UNGATED/LEGACY` 那格）：
`Can't keep up! Is the server overloaded? Running 2198ms or 43 ticks behind` —— 与真机三次同形。

#### 三、⭐ 三条结论（读数，不是推断）

1. **收益**：毫秒兜底轴 400 → 60 把**生产形态**的最坏单 tick 搜索耗时从 **402 ms 压到 100 ms**（廉价链同向 400 → 100）。
2. **代价**：同一个 tick 里**被拒的搜索数 5 → 11**（廉价链 22 → 28）⇒「一个 tick 连做一串搜索」的正常批处理链
   首当其冲 —— `mine_menu` 的 161 ms 链当初正是为此把毫秒轴定在 400（`P4` 评审：第一版用 150 ms 就把该步判红）。
3. ⭐⭐ **原判据的后半句（"收到 ≤60 ⇒ `Can't keep up` 消失"）在算术上不成立**：闸门开着时生产形态最坏 ~402 ms，
   连原版告警阈值 2000 ms 的一半都不到 ⇒ 400 与 60 **都不产生**它；它只在"闸门全关 + 历史形态"下出现。
   ⇒ 判据必须先承认「红只在闸门关闭时可复现」（台架已把它做成可复现读数）。
4. ⭐ **真正决定最坏 tick 的不是毫秒轴，而是"第一个搜索永远放行"**：`LEGACY` 两格（PROD 761 / TIGHTENED 358，
   都是**单次**搜索的时长）说明 —— 单次搜索本身很贵时（`SearchBudget` 的 `maxMillis=0` 那一档），
   收毫秒兜底轴几乎不改变结果。⇒ 若目标是"任何 tick 都别超过 X ms"，杠杆是**给重消费者自己的预算加时间上限**。

#### 四、为什么台架不判"该不该收紧"

因为这一步同时是**能力决策**（§三.2 的代价），不是纯技术问题；而且它动的是**内核行为常量**
（`D-368`/`P4` 评审都强调过：内核行为变更不属于"尺子线"）。台架的职责是把两侧代价摆平，让裁定有据。

#### 五、裁定建议（三选项，**待用户拍板**）

1. ⭐ **推荐（若目标是"消掉 `Can't keep up` / 缩短最坏 tick"）= 不动毫秒轴，改给重消费者的搜索加时间上限**
   （`SearchBudget` 里 `maxMillis=0` 的那一档；今天进入生产的正是挖矿候选那条链）。理由：读数显示
   最坏 tick 由"单次搜索的时长"决定，而毫秒兜底轴只影响"一个 tick 能塞几次搜索"。
   ⇒ 新项 `P4″`（判据 = 本台架的 `LEGACY` 格在生产形态下退到 ≤50 ms/次，同时 `MINING` 格拒绝数不增）。
2. **维持 400 不动**（若优先级是"别打断正常批处理链"）：代价 = 最坏 tick ~400 ms 留在那里（客户端手感：
   一次 0.4 s 卡顿，但不足以触发原版告警）。
3. **收紧到 60**（原 `P4′` 的字面动作）：收益 = 最坏 tick 402 → 100 ms；代价 = 拒绝数翻倍。
   选它就必须**先跑 CORE 逐步 diff**（`mine_menu` 那类批处理步是首当其冲的回归点）。

#### 六、复核触发

1. 出现 `tick_budget_bench` 的 `TIGHTENED`/`PROD` 格**读数反转**（收紧后不再更省）⇒ 说明"第一个搜索放行"
   成了唯一主导项 ⇒ 直接走选项 1；
2. 生产日志出现 `Can't keep up` ⇒ 拿本台架复算"当轮最坏 tick 应该多少"，再用 §五 选项 1 的杠杆；
3. `SearchTickBudget` 的夹具专用口被移除/改名 ⇒ 本台架会**响亮失败**（它读的就是那三个方法），同步本决策。

#### 七、⚠️ 更正（2026-09-24 核对，**裁定不变**）

⭐ **§二 的"红臂来自 `UNGATED/LEGACY` 那格"与 §三.3 的"红只在闸门关时可复现"两句都不成立。**
取证与复算 = `docs/reviews/2026-09-24-survey32-核对.md §3`。要点：

1. **行位置（硬证据）**：`run/headless-logs/20260924-140303-single_tick_budget_bench.log` 唯一那条告警在 **`:190`**，
   即 `PROD(400)` 三格之后，**比 `:321` 的 `UNGATED/LEGACY` 早 5 秒** ⇒ 时间上不可能来自它。
   当天三次运行（`…135848` / `…140135` / `…140303`）**全部同形**（都紧跟在闸门开着的格子之后）；
   "全日志唯一一条"也**不是观测**：该轮总时长 ≈11 s < 15 s ⇒ **限流保证最多一条**。
2. **阈值语义（本轮读字节码，升级掉旧注的"系统常识级判断"）**：`net/minecraft/server/MinecraftServer.class`
   的分支 = `l1 = now − checkpoint; if (l1 > 2000ms && (checkpoint − lastReport) >= 15000ms) { ticks = l1/50; warn(...);
   checkpoint += ticks*50; lastReport = checkpoint; }` ⇒ ① **累计**量、不是单 tick 量 ② **两次上报至少隔 15 s** ③ 上报后计数前移归零。
   ⇒ §三.3 的"402 ms 连 2000 ms 阈值的一半都不到 ⇒ 400 与 60 都不产生它"是**算术错**：5 个连续超支 tick 就够跨过**累计**阈值
   （台架自己正是这样报的：`PROD` 三格连着跑 ⇒ 2198 ms）。
3. **结论怎么变**：`P4″`（`D-431`）**不受影响、反而更硬** —— 闸门限的是"一 tick 塞几次搜索"，限不了累计落后；
   只有把**单次搜索**压到 ≤50 ms（`P4″` 的判据本身）才能让每 tick 都不超支。
4. **新增附带项（登记进 `P4″` 落地时一起做）**：本台架"同轮连跑三配置"**与 15 s 限流相冲** ⇒ **它无法判定"闸门关 ⇒ 出红"**。
   要让红臂可判，须改成**一个配置一次 server 启动**，或**直接读累计落后量**（不依赖被限流的 warning）。
5. §二 九格的 `UNGATED/LEGACY` 一栏，**同一张表里混用了口径**：其余八格引 `account`，该格引 `wall`（2578 vs `account`/`SUMMARY` 的 **2575**）⇒ 引用时统一取 `account`。

---

### D-430：⭐ **内核关门线**（用户 2026-09-24 裁定「同意关门」）

**裁定原话**：「同意关门，保存工作状态后，我去找勘测员聊聊计划，**根据勘测报告再决定之后的排期**」。

#### 一、裁定内容（三条）

1. **内核只接受三种输入**（其余一律不进内核队列）：
   - ⭐ **真机实测红**（客户端/电池日志里能指名的失败、崩溃、`Can't keep up`、账目/closing 异常）；
   - ⭐ **用户报障**（"我操作了 X，看到 Y，期望 Z"）；
   - ⭐ **CORE 回归**（41 步里任何一步判决变化，或 `check-all` 出红）。
2. **暂停四件"自证型"内核工作**（转入 §11-E 观察项，**不是删除**）：
   ① **名词登记**（`CAPABILITY_UNRESOLVED_BUDGET` 剩下的 10 个码）· ② **门禁人口/对称性扩写** ·
   ③ **死码/同源化审计**（`D-425`/`D-426`/`D-427`/`D-428` 那一族）· ④ **Baritone 逐行对照切片**（`P2` 余下）。
   每件都在 §11-E 写了**复活条件**。
3. **主力转能力线**：三天挖矿任务线（③ 鱼骨 → ① 跟随 → ② 探洞）为主战场，产品面（能力清单进 prompt、只读命令面）
   次之。**但 J-1 之后的排期不现在定档** —— 以**勘测报告**为输入再定（用户本轮明确）。

#### 二、依据（为什么现在关门，而不是"感觉够了"）

`AGENTS.md` 给内核定的两个目标指标**都已达到**：
- 「从改一行到知道对不对」＝ **1 条命令**：`tools/check-all.sh`（20 道门禁）+ `headless-battery.sh core`（41 步）✓
- 「规则被违反时」＝ **构建红**：20 道门禁**全部**做过"注入即红"验证 ✓

⇒ 仪表够用。此后内核每往里投一分，买的都是**对称性/自证**（名词、人口、死码、逐行对照），
边际收益递减而成本恒定 —— 这就是"修内核花了很多时间"的机制（`§11-J` 的立表理由）。

#### 三、关门线的**解除条件**（写死，防止"关门"变成"忘掉"）

出现下列任一情形 ⇒ 对应项**自动复活**并可立即开片：
1. `CORE` 任一步判决变化 / `check-all` 出红（⇒ 内核回归工作立刻开工，永远是最高优先级）；
2. 真机日志出现能指名的失败（含 `Can't keep up`、`SEARCH_LIMIT` 族误报、账本/恢复异常）；
3. **能力线撞到内核边界**：例如鱼骨挖掘需要某个被冻结的 Movement 能力/准入，而读数证明缺口真在那边
   （此时**由能力线的判据**驱动，而不是由"对称性"驱动 —— 这是允许的复活路径）；
4. 用户直接要求恢复某一件（一句话即可）。

#### 四、与既有裁定的关系

- **`D-333`（守卫线冻结）保持一致**：不重设计守卫、不做任务预设、只加测量过的调试接口 —— 本裁定把它推广到
  "所有自证型内核工作"。
- **§11-J 排期表**：J-0.3 由本裁定 ✅ 关闭；J-0.1/J-0.2 两个开口决策点**仍未裁**
  （`P2c` = 按推荐"先不改"归档；`P4′` = 待用户选 `P4″`/维持 400/收紧 60，读数已在 `D-429`）。
- **不改变**任何既有红线：`D-076`（寻路默认纯通行）、`D-132`（不读未加载区块）、
  `D-026`（合法位置集）、`K-4`（可规划即可执行）与全部门禁**照旧生效** —— 关门只是**不再主动加新门禁/新名词**。

---

### D-431：`P4′` 方向裁定 = 选 **`P4″`**（给重消费者的 `SearchBudget` 加时间上限，**不动**每 tick 毫秒兜底轴）（用户 2026-09-24 拍板）

📋 **用户裁定原话**：「照推荐定 `P4″`」（回答 `D-429 §五` 的三选一）。

#### 一、裁了什么

- **采纳** `D-429 §五` 选项 1 ⇒ **新项 `P4″`**：`SearchTickBudget.DEFAULT_MAX_MILLIS_PER_TICK` **保持 400 ms 不动**，
  改为**给重消费者自己的 `SearchBudget` 加时间上限**（今天进入生产的是挖矿候选那条链，`maxMillis = 0` 那一档）。
- **不采纳**选项 2（维持 400 = 什么都不做）与选项 3（收紧到 60）：选项 3 的收益（最坏 tick 402 → 100 ms）
  抵不过代价（同 tick 拒绝数 5 → 11 翻倍；`mine_menu` 那条 161 ms 批处理链首当其冲，`P4` 评审有先例），
  且原判据后半句（"收到 ≤60 ⇒ `Can't keep up` 消失"）在算术上不成立（闸门开着时生产形态最坏仅 ~402 ms ≪ 2000 ms 告警阈值）。

#### 二、`P4″` 的验收判据（沿用 `D-429 §五`，此处定为落地口径）

1. 台架 `tick_budget_bench` 的 **`LEGACY` 格**（13 × 无时间上限搜索）在**生产形态**下退到 **≤50 ms/次**；
2. 同时 **`MINING` 格的拒绝数不增**（当前 PROD 放行 8 / 拒 5）；
...
- **`P2c`**（`FALL_NOT_ON_GROUND`：起点不落地的 `FALL` 边「规划得到、执行不了」）—— 用户本轮**未表态**，
  两侧建议均为「**先不改**」（`D-428 §六` 推荐 + `survey/32 §5` 同向）⇒ 仍列 **待用户拍板**，
  **不得**按"推荐"自动落地（裁定权在用户，`AGENTS.md` §协作与决策权）。

#### 四、复核触发

1. `P4″` 落地后台架 `LEGACY` 格**读不出 ≤50 ms**（说明"第一个搜索永远放行"才是主导项 ⇒ 该回到闸门轴谈）；
2. `P4″` 落地后 `MINING` 格**拒绝数上升**（说明它反而打断了正常批处理链 ⇒ 回滚该上限并重估）；
3. 生产日志出现 `Can't keep up!`（原版阈值 2000 ms）⇒ 先用本台架复算"当轮最坏 tick 应该是多少"，
   再判断是否属于本裁定的杠杆范围。
