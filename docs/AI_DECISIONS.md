...
**建议（②′，推荐）**：**计划级自洽校验 + 有界重搜** —— 搜完对计划回放一遍写入（与执行器**同一口径**：
直接调 `collectBlockers`），逐边检查"支撑是否落在本计划早先挖掉的格子上"；发现冲突就**禁掉那条"清空者"边**
（按**具体边**禁，不是禁整个 Movement 类）后重搜，最多 K=3 次；K 次仍冲突则把最后一次交出去 + 计数
（执行期 `futureTargetBlocked` 仍是兜底 ⇒ **不新增"可解变不可解"**）。
- 优点：**不碰 A\* 状态模型**（D-036 风险最小）、不受节点合并损失影响、最终计划**可证明自洽**、
  该不变量本身可以在夹具里做**常绿断言**（反向对照天然存在：切片 B 下的灌水坑）；
- 代价：命中冲突时多跑 1~K 次搜索（今天只在挖掘类场景出现，计数可观测）；
