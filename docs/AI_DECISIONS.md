...
**⚠️ 待你确认（会影响体感）**：四邻腐蚀下 **2×2 区块（32×32 格）及以下没有内部区块** ⇒ 如果你的基地就是
2×2 区块，那么"向区域中心靠"这件特性**看不到效果**（退化成"进区即到"）。要放宽成**八邻**（对角也算）
只需改 `internalChunks` 一处（3×3 与 2×2 都会得到内部区块）。**先按你选的"四邻"落地了**。
**复核触发**：① 用户基地 ≤2×2 区块且希望有内缩 ⇒ 改八邻；② 第 2 件（返程判据）落地时若发现
"内部区块为空"的退化路径没被消费方正确处理 ⇒ 补断言语义（当时 `safe_return` 夹具要改写）。
