...
- 二次修复（用户 2026-09-09 手动验证）：**封口必须 ≥3 格高（y=63..65）**。2 格高仍会 `REACHED`——
  因为该检查允许世界修改，规划器可以先在 `(1,63,61)` 放一个方块造出脚位 `(1,64,61)`，
  再 ASCEND 到封口顶面 `(2,65,61)` 沿墙绕行；3 格高时上升目标 `(2,65,61)` 是墙块 → 绕行封死。
