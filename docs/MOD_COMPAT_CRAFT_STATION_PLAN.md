...
| **Sophisticated Backpacks** | `1.20.1-3.26.3.2157` | `efc831296f60a59792af142534d1c6dd1dff7f3f` | **已装**（用户截图后加装）；要 `sophisticatedcore [1.5.1.+,)` ✓ |
| （误装，保留）Refined Storage | `1.12.4` | `014bf4de4975f6310d22a6ed06c6a7e1e0805641` | §4；用户裁定**不撤**（AE 兄弟模组，暂不做兼容，无冲突） |
...
### 1.6 客户端截图实测确认（2026-09-13 16:44–16:45，7 张）

用户截图（`<client>/screenshots/2026-09-13_16.44.2x–16.45.03.png`）核对结果 —— **三条描述全部成立**：

| 截图 | 看到的事实 | 对应我们的判断 |
|---|---|---|
| 16:44:28 | **丛林木钻石箱子** GUI：左侧是**升级槽**列，右侧边缘有标签按钮；存储区空 | 容器 = 方块型站点；升级槽与存储区在**同一个菜单** |
| 16:44:34 | JEI 提示 **合成升级** `sophisticatedstorage:crafting_upgrade`："在升级标签页中添加一个工作台" | 与源码 tooltip 一致（§1.1）✓ |
| 16:44:36 | 升级**已装**（左侧升级槽里出现带工作台图标的项），右侧出现"合成"标签 | L2 装配 → L3 标签页**是两个可分辨的动作** ✓ |
| 16:44:43 | **点开"合成"标签后**：右侧出现 **3×3 网格 + 结果槽**（下方带向下箭头），悬停提示 **"不自动重填合成方格"** | 界面事实：**点开标签才显示网格**；`refill_crafting_grid` 开关默认关 ✓（§1.2/1.3） |
| 16:44:44 | 按钮提示 **"Shift右击将成品放入容器"** | `shouldShiftClickIntoStorage` 默认 **true** = 进容器 ✓（§1.3） |
| 16:44:47 | 同一按钮切到 **"Shift右击将成品放入玩家物品栏"** | **QuickMove 目标确实可切换** ✓（用户第 3 条，界面级证据） |
| 16:45:03 | 另一个按钮提示 **"自动重填合成方格"** | 与"不自动重填"互为两态 ✓ |

⇒ **界面上"点开标签才显示槽位"是真的**（渲染层）；而源码显示服务端槽位**常驻菜单且未被 `isActive` 关掉**（§2.6.2）。
两者不矛盾，**L3 是否必须发包留给 S1-3 探针用"打开前后差分"实测**（不猜）。
...
| **精妙背包（Sophisticated Backpacks）** | ✅ **已装** `3.26.3.2157`（sha1 `efc83129…`，要 Core ≥1.5.1 ✓）；它有**自己的** `sophisticatedbackpacks:crafting_upgrade`，tooltip 与标签页机制**与存储完全一致**（实现都在 Core） |
